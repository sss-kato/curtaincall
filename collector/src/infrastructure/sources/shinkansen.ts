...
const COMPANY_ID = "shinkansen";

/** カテゴリ判定表（§4.6）。tagMap は RSS の category 要素と照合。"NEWS" と作品名は写像しない */
...
/**
 * 劇団☆新感線の RSS Source。取得先は company.sources[0].url（blog.vi-shinkansen.co.jp/?feed=rss2）のみ。
 * 公式ドメイン vi-shinkansen.co.jp のフィードは空のためブログを参照する（R-9、調査レポート §2）。URL はコードに持たない
 */
export class ShinkansenSource implements Source {
  readonly id: string;
  ...
  constructor(
    ...
    private readonly tables: CategoryTables = SHINKANSEN_TABLES,
  ) {
    this.id = company.id;
  ...
  /** §5.3 の抽出規則を 1 項目に適用する。title → url → publishedAt → category → thumbnail の順に確定する */
  private extractOne(item: FeedItem): ExtractResult {
    ...
    // §5.3 url：item.link を絶対 URL として検証するだけ（?p=NNNNN のクエリを変更しない。R-9）。
...
export function createShinkansenSource(
  company: Company,
  deps: SourceDeps,
  options: SourceOptions,
): Source {
  return new ShinkansenSource(company, deps, options);
}
