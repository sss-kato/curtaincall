...
describe("送信の基本", () => {
  ...
  it("1 通の失敗で他は送られる", async () => {
    ...
    expect(
      logger.entries.some((e) => e.level === "error" && e.message === "notification failed"),
    ).toBe(true);
  });
...
describe("全団体が reject", () => {
  ...
  async function executeWithAllTopicsRejected() {
    const companyA = buildCompany({ id: "takarazuka", fcmTopic: "takarazuka" });
    const companyB = buildCompany({
      id: "shiki",
      name: "劇団四季",
    ...
    const gateway = new FakeNotificationGateway(new Set(["takarazuka", "shiki", "toho"]));
    const logger = new RecordingLogger();
    const useCase = new NotifyNewArticles(gateway, logger);
  ...
  it("error ログが団体数分出る", async () => {
    ...
    const errorEntries = logger.entries.filter(
      (e) => e.level === "error" && e.message === "notification failed",
    );
