...
| F-08 通知の団体別 ON/OFF | `settings` テーブルの `notification.<companyId>`（既定 ON）と `companies.json` の `fcmTopic` を突合し、`SyncPushSubscriptionsUseCase` が購読・解除を冪等に同期する。起動時と D-05 のトグル操作後に実行する | §4.4 / §5.6 |
...
| §6.1 取得タイミング（起動時・Pull to Refresh・フォアグラウンド復帰） | `SyncTrigger` 5 値（launch / foreground / pullToRefresh / retry / notificationTap）を `SyncController.sync()` に渡す。launch と foreground は本書の `AppLifecycleSync` が起動し、残り 3 つは D-05 が起動する。同時実行は 1 つに抑止（S-01 §8 #12） | §5.3 |
...
| S-01 §8 #23 フォアグラウンド復帰時の取得 | `AppLifecycleSync` が `paused → resumed` の遷移で `sync(foreground)` を呼ぶ。`inactive → resumed`（通知許可ダイアログ・アプリ内ブラウザから戻った）では呼ばない | §5.3 |
...
S-00 のすべての操作 A-nn と状態 ST-nn を載せる。本書は共通部品の Widget・状態の判定関数・DB 列を提供し、個別画面での配置と操作の UseCase は D-05 が持つ。
...
| S-00/ST-10 | 状態 | `ListStatus.full == FullView.loading`（判定は §5.4） | §5.4 |
...
| S-00/E-10〜E-17 | 要素 | `ArticleCell`（入力は `ArticleCellModel`。団体名・既読状態・保存状態は D-05 が組み立てる） | §5.9 |
...
依存は presentation / app → application → domain、infrastructure → domain の一方向のみ。具象クラスを import してよいのは `lib/core/di/providers.dart` と `lib/app/bootstrap.dart`（DB を開き、アセットを読み、Provider を override する）だけ。presentation は `core/di` の Provider を通じて UseCase・Repository の**インターフェース型**を受け取り、infrastructure を import しない（CLAUDE.md）。

```mermaid
flowchart TB
  subgraph APP["lib/app（ルート Widget・起動）"]
    BOOT["bootstrap.dart（具象 import 可）"]
    ROOT["app.dart / root_tabs.dart / app_lifecycle_sync.dart"]
  end
  subgraph PRES["features/*/presentation"]
    SC["SyncController / listStatus / ArticleCell"]
  end
  subgraph DI["core/di/providers.dart（具象 import 可）"]
    PV["Provider 群"]
  end
  subgraph APPL["features/*/application"]
    UC["SyncArticlesUseCase / SyncPushSubscriptionsUseCase / RequestNotificationPermissionUseCase"]
  end
  subgraph DOM["features/*/domain"]
    ENT["Article / Company / compareArticles"]
    IF["ArticleRepository / SettingsRepository / CompanyRepository / ArticlesFeed / PushGateway"]
  end
...
feature 間の依存は次の 1 方向だけを許す。`articles` は他の feature を import しない（`Article` の所有者）。

| 依存元 | 依存先 | 理由 |
|---|---|---|
| `saved` / `settings` / `notifications` の presentation（D-05） | `articles/domain`・`articles/presentation`（`ArticleCell`） | 保存一覧も同じ記事セルを使う（S-00 §8 #1） |
...
app/
  ...
  lib/
    ...
    app/
      bootstrap.dart            DB open・companies 読み込み・（fcm 時）Firebase 初期化・ProviderScope の overrides（§5.1）
    ...
    features/
      articles/
        domain/
          ...
          articles_file.dart        ArticlesFile（schemaVersion・generatedAt・articles）
          article_order.dart        compareArticles（D-01 §4.5 の移植）
          article_repository.dart   ArticleRepository・FeedApplyPlan・FeedApplyResult（§4.4）
          articles_feed.dart        ArticlesFeed ポート・FeedFetchResult・FeedOfflineException・FeedErrorException（§4.3）
        ...
        infrastructure/
          ...
          drift_article_repository.dart ArticleRepository 実装（applyFeed は 1 トランザクション）
        presentation/
      ...
      notifications/
        domain/
          push_gateway.dart          PushGateway・PushPermissionStatus・NotificationTap（§4.6）
        application/
  ...
  test/
    ...
    features/
      ...
      articles/domain/article_order_test.dart              compareArticles の直接テスト（§7 純粋関数の例外）
      ...
      articles/presentation/published_date_format_test.dart formatPublishedDate の直接テスト（同上）
...
| `articles` の行の寿命 | 配信に含まれる間は `in_feed = true`。配信から外れたら、保存済みなら `in_feed = false` で残し、未保存なら削除する（§5.2 手順 7）。保存を解除した記事が `in_feed = false` なら、その時点で D-05 が削除する（S-02 §7.3「解除したとき」。§8.1） |
...
abstract interface class ArticlesFeed {
  /// [etag] が非 null なら If-None-Match を送る。
  /// 304 → FeedNotModified、200 → FeedFetched。それ以外は例外
  Future<FeedFetchResult> fetch({required String? etag});
}
...
final class FeedErrorException implements Exception {
  const FeedErrorException(this.reason, {this.cause});
  final SyncFailureReason reason;   // §4.4
  final Object? cause;
}
```
...
| 項目 | 規則 |
|---|---|
| リクエスト | `GET articlesFeedUrl`。ヘッダ `User-Agent: appUserAgent`、`Accept: application/json`、`etag != null` のとき `If-None-Match: <etag>`。`feedTimeout` で打ち切る。クエリ（キャッシュバスティング）は付けない（D-01 §8 #17） |
| 304 | `FeedNotModified` |
| 200 | 本文を UTF-8 で decode → `ArticlesFileCodec.decode`（下記）→ `FeedFetched(file, etag: headers['etag'])` |
| その他のステータス | `FeedErrorException(SyncFailureReason.httpStatus)` |
| `SocketException`・`http.ClientException` | `FeedOfflineException`（DNS 解決失敗・接続拒否・機内モード。§8 #7） |
| `TimeoutException` | `FeedErrorException(SyncFailureReason.timeout)` |
| JSON でない・トップレベルが object でない・`articles` が配列でない・記事 1 件でも必須フィールド欠落や型不一致 | `FeedErrorException(SyncFailureReason.malformed)`。**ファイル全体を失敗**とし、部分的に取り込まない（§8 #8） |
| `schemaVersion != 1` | `FeedErrorException(SyncFailureReason.unsupportedSchema)`（D-01 §4.6） |
...
各インターフェースは本書の UseCase が必要とするメソッドだけを持つ（CLAUDE.md I）。D-05 が閲覧・更新用のメソッドを同じインターフェースに追加する（§8.1）。
...
abstract final class SettingKeys {
  static const String feedEtag = 'feed_etag';                     // §4.3。ArticleRepository が読み書きする
...
/// 取得成功。notModified = true は 304（差分なし）
final class SyncSucceeded extends SyncResult {
  const SyncSucceeded({required this.inserted, required this.updated, required this.deleted, required this.notModified});
  final int inserted;
  final int updated;
  final int deleted;
...
### 4.6 PushGateway（`features/notifications/domain/push_gateway.dart`）
...
| `FcmPushGateway`（フェーズ 5。§10 T-G） | `firebase_messaging`：`getNotificationSettings().authorizationStatus`（`provisional` は `authorized` 扱い）／`requestPermission(alert: true, badge: false, sound: true)`／`subscribeToTopic` / `unsubscribeFromTopic`／`getInitialMessage()`／`onMessageOpenedApp`。`RemoteMessage.data['companyId']` が `String` のときだけ `companyId` に入れる。フォアグラウンド受信時も通知を表示する（`setForegroundNotificationPresentationOptions(alert: true, badge: false, sound: true)`。S-01/ST-15 はフォアグラウンドでも同じ動作） |

切り替えは `const String pushBackend = String.fromEnvironment('CURTAINCALL_PUSH', defaultValue: 'none')`。`'fcm'` のときだけ `bootstrap` が `Firebase.initializeApp()` を呼び、DI が `FcmPushGateway` を注入する（§8 #12）。

### 4.7 Riverpod / DI（`lib/core/di/providers.dart`）

`riverpod_annotation` + `riverpod_generator`（Riverpod 3.x。`Ref` 型）。基盤の Provider はすべて `keepAlive: true`。`appDatabaseProvider`・`companiesProvider`・`loggerProvider` は `bootstrap` が `ProviderScope(overrides: [...])` で実体を与え、override 無しで読むと `UnimplementedError` を投げる（テストでも必ず override する）。

```dart
// lib/core/di/providers.dart（抜粋）
@Riverpod(keepAlive: true)
AppDatabase appDatabase(Ref ref) => throw UnimplementedError('bootstrap で override する');

@Riverpod(keepAlive: true)
List<Company> companies(Ref ref) => throw UnimplementedError('bootstrap で override する');

@Riverpod(keepAlive: true)
Logger logger(Ref ref) => throw UnimplementedError('bootstrap で override する');
...
@Riverpod(keepAlive: true)
ArticleRepository articleRepository(Ref ref) => DriftArticleRepository(ref.watch(appDatabaseProvider));
...
| 具象の import | `providers.dart` と `bootstrap.dart` のみ。presentation・application は型としてインターフェースだけを参照する |
...
`logger` パッケージ（CLAUDE.md。`print` 禁止）。`bootstrap` が 1 つ生成して `loggerProvider` に override する。レベルは debug ビルドで `Level.debug`、release で `Level.warning`。domain・application は `Logger` を import しない（UseCase は結果型で失敗を返す）。ログを出すのは infrastructure（`HttpArticlesFeed`・`DriftArticleRepository`・`NoopPushGateway`・`FcmPushGateway`）と presentation（`SyncController`・`AppLifecycleSync`）。記事本文・通知本文・サムネイルの内容はログに出さない（URL・id・件数・例外の型とメッセージのみ）。
...
- **ステップ**：
  ...
  2. `Logger` を生成（§4.9）
  3. `pushBackend == 'fcm'` のときだけ `Firebase.initializeApp()`（T-G 以降。それ以外は呼ばない）
  4. `AppDatabase(driftDatabase(name: 'curtaincall'))` を生成する（実際の open は最初のクエリで行われる。`schemaVersion` の migration もそのとき）
  5. `AssetCompanyRepository().loadAll()` を await して `List<Company>` を得る（§4.5）
  6. `runApp(ProviderScope(overrides: [appDatabaseProvider.overrideWithValue(db), companiesProvider.overrideWithValue(companies), loggerProvider.overrideWithValue(logger)], child: const CurtainCallApp()))`
- **出力**：画面（`CurtainCallApp` → `RootTabs`。初期タブはホーム = S-00/ST-20）
- **失敗時**：手順 5 の失敗（アセット欠落・`schemaVersion` 不一致・`id` 重複）はビルドの不備なので `StateError` をそのまま投げてクラッシュさせる（黙って空のタブで起動しない。§6）。手順 4 は例外を投げない（open の失敗は最初のクエリで `SyncFailed(storage)` または各画面の Stream エラーとして現れる）

取得は `runApp` の後に `AppLifecycleSync`（§5.3）が始める。DB の記事は取得を待たずに表示される（要件 §5 起動速度）。

### 5.2 SyncArticlesUseCase（配信の取得と端末 DB への反映）

- **入力**：`SyncTrigger`（ログ用。処理は契機によって変わらない）
- **依存**：`ArticlesFeed`、`ArticleRepository`
- **ステップ**：
  1. `etag = await articles.feedEtag()`
  2. `result = await feed.fetch(etag: etag)`。`FeedOfflineException` → `SyncOffline` を返す。`FeedErrorException(reason)` → `SyncFailed(reason)` を返す（DB には触れない）
  3. `result` が `FeedNotModified` → `SyncSucceeded(inserted: 0, updated: 0, deleted: 0, notModified: true)` を返す（D-01 §6「304 は取得成功・差分なし」）
  ...
  6. **判定**（D-01 §5.2 の app 側判定表）：`incoming` の各記事 `r` について、`local = existing[r.id]`
     ...
     | `local.updatedAt != null && r.updatedAt != null && local.updatedAt != r.updatedAt` | 更新 | `updates` に `r` |
     | `local.updatedAt != null && r.updatedAt == null` | 変化なし（`updatedAt` が無しに戻った。押し出し後の再出現。D-01 §8 #34） | `refreshes` に `r` |
     | 上記以外（`updatedAt` が両方 null、または同値） | 変化なし | `refreshes` に `r` |

     「更新」の判定は `updatedAt` の値の変化だけで行い、`contentHash` は比較しない（D-01 §8 #5）。`refreshes` では `fetchedAt`・`updatedAt` を端末の値のまま保つ（D-01 §5.2）。`updates` では全列を配信値にする（collector は更新時に `fetchedAt` を引き継ぐため通常は端末の値と一致する。§8 #6）
  7. `plan = FeedApplyPlan(inserts, updates, refreshes, etag: fetched.etag)` を `articles.applyFeed(plan)` に渡す。`DriftArticleRepository.applyFeed` は次を **1 トランザクション**で行う：
     ...
     6. `settings` の `feed_etag` を `plan.etag` で upsert（null なら行を削除）
...
- **出力**：`SyncResult`
- **失敗時**：手順 2 の分類どおり `SyncOffline` / `SyncFailed`。手順 7 の例外（drift のエラー）はトランザクションが戻された上で `SyncFailed(SyncFailureReason.storage)`。UseCase は例外を投げない（`SyncController` が結果型を状態に写すだけで済む。§8 #10）
...
sequenceDiagram
  ...
  participant R as ArticleRepository（drift）
  C->>U: execute(trigger)
  U->>R: feedEtag()
  R-->>U: etag
  U->>F: fetch(etag)
...
### 5.3 取得の契機と `SyncController`（`features/articles/presentation/sync_controller.dart`）
...
class SyncController extends _$SyncController {
  ...
  @override
  SyncStatus build() => const SyncStatus(inProgress: false, lastResult: null, sequence: 0);

  /// 実行中なら新たに始めず、実行中の Future を返す（S-01 §8 #12）
...
- **入力**：`SyncTrigger`
- **ステップ**：
  1. `_inFlight != null` → それを返す（新しい取得を始めない）
  2. `state = state.copyWith(inProgress: true)`
  3. `_inFlight = ref.read(syncArticlesUseCaseProvider).execute(trigger)`
  4. 完了したら `state = SyncStatus(inProgress: false, lastResult: result, sequence: state.sequence + 1)`、`_inFlight = null`。`SyncFailed` / `SyncOffline` は `logger.w`、成功は `logger.i`（契機・件数）
  5. 手順 3 の Future に `.timeout(feedTimeout + const Duration(seconds: 5))` を掛け、完了しない場合（バックグラウンドで OS に止められた等）は `SyncFailed(SyncFailureReason.timeout)` として手順 4 を行う（`HttpArticlesFeed` 自身のタイムアウトが先に効くため、通常はここに来ない）
- **出力**：`SyncResult`（呼び出し側は通常待たない。Pull to Refresh のインジケータを閉じるために D-05 が await する）
- **失敗時**：UseCase は投げないが、万一の未捕捉例外は `SyncFailed(storage)` として状態に写し、再スローしない（画面を止めない）
...
| フォアグラウンド復帰 | `foreground` | `AppLifecycleSync.didChangeAppLifecycleState`：直前の状態が `paused` または `hidden` で、新しい状態が `resumed` のとき。`inactive → resumed`（通知許可ダイアログを閉じた、アプリ内ブラウザ（SFSafariViewController）を閉じた、コントロールセンターを閉じた）では呼ばない | S-01 §8 #23、§8 #18 |
| Pull to Refresh | `pullToRefresh` | D-05（S-01/A-04） | F-10 |
| 再試行 | `retry` | D-05（S-00/A-12 → S-01/A-08） | S-00 §6 |
| 通知タップ | `notificationTap` | D-05（S-01/A-07・ST-15） | S-01 §8 #20 |

`AppLifecycleSync` は `CurtainCallApp` の直下に置く `ConsumerStatefulWidget`（`WidgetsBindingObserver`）で、子として `RootTabs` を返すだけの透過的な Widget。起動時の手順は「`sync(launch)` を開始（await しない）→ `RequestNotificationPermissionUseCase`（§5.7）を await → `SyncPushSubscriptionsUseCase`（§5.6）を await」。通知許可ダイアログの表示中も取得は続く（S-01/ST-18）。フォアグラウンド復帰時は `sync(foreground)` に加えて `pushPermissionStatusProvider` を invalidate する（S-03/A-07 で設定アプリから戻ったときに許可状態を読み直す）。
...
/// 判定の入力。hasArticles = 端末に記事が 1 件以上ある（in_feed を問わない）
...
FeedStatus feedStatus(Ref ref) {
  ...
  return FeedStatus(
    hasArticles: switch (count) { AsyncData(:final value) => value > 0, _ => false },
...
@Riverpod(keepAlive: true)
Stream<int> articleCount(Ref ref) => ref.watch(articleRepositoryProvider).watchCount();
```

判定表（上から順に最初に一致した行を採る）：

| `hasArticles` | `inProgress` | `lastResult` | `hasVisible` | `full` | `refreshing` | `offlineBanner` | `errorNotice` | S-00 |
|---|---|---|---|---|---|---|---|---|
| false | true | 任意 | — | `loading` | false | false | false | ST-10 |
| false | false | `SyncOffline` | — | `offline` | false | false | false | ST-16 |
| false | false | `SyncFailed` | — | `error` | false | false | false | ST-14 |
| false | false | null / `SyncSucceeded` | — | `empty` | false | false | false | ST-12（取得成功で 0 件、または取得を伴わない画面） |
| true | 任意 | 任意 | true | `content` | `inProgress` | `lastResult is SyncOffline` | `lastResult is SyncFailed` | 記事一覧 + ST-11 / ST-13 / ST-15 |
| true | 任意 | 任意 | false | `empty` | `inProgress` | `lastResult is SyncOffline` | `lastResult is SyncFailed` | ST-12 + ST-11 / ST-13 / ST-15（S-00 §8 #24） |

- `articleCountProvider` の初回値が届く前（`AsyncLoading`）は `hasArticles = false` として扱う。DB の count は数ミリ秒で返るため、初回フレームで E-20 が一瞬出ることは許容する
...
- **出力**：`PushSubscriptionSyncResult(subscribed: List<String>, unsubscribed: List<String>, failed: List<String>)`（トピック名。ログとテスト用。画面には出さない。S-03 §8 #4）
- **失敗時**：例外を投げない。`loadAll` の失敗（起動時に検証済みのため起きない）だけは `StateError` をそのまま伝播する
- **呼び出し**：起動時（§5.3）と、D-05 の通知トグル操作（S-03/A-01）で設定を保存した直後（§8.1）。毎回全団体を冪等に同期し、前回の購読状態を端末に持たない（§8 #13）
...
| `ArticleCell` | `features/articles/presentation/widgets/article_cell.dart` | `ArticleCellModel { Article article; String companyLabel; ReadDisplayState readState; bool isSaved; }`、`onTap`、`onToggleSaved`、`now`（E-14 の判定基準。D-05 が渡す） | S-00 §3.2 の配置。1 行目は `Row` で E-11（`Flexible` + `TextOverflow.ellipsis`）→ E-12（`CategoryIcon`）→ E-16（`readState == updated` のときだけ）。E-13 は `maxLines: 3`・`ellipsis`、空文字なら S-00 §7.7 の代替文言。E-14 は `formatPublishedDate(article.publishedAt.toLocal(), now)`。E-15 は `Image.network(thumbnail, fit: BoxFit.cover)` を `AspectRatio(1)` + `ClipRRect` で描き、`loadingBuilder` / `errorBuilder` と `thumbnail == null` でプレースホルダー（ST-05・ST-06）。既読（ST-02）は `CupertinoColors.secondaryLabel` と `Opacity(0.5)`。E-17 は `CupertinoButton`（`star` / `star_fill`）で `onToggleSaved`、セル本体は `GestureDetector` で `onTap`（E-17 のタップはセルへ伝播させない。S-00/A-11）。画像はメモリ内キャッシュ（`ImageCache`）だけで、ディスクに書くパッケージ（`cached_network_image` 等）は使わない（CLAUDE.md。§8 #16） |
| `CategoryIcon` | 同 `widgets/category_icon.dart` | `Category` | S-00 §7.2 の対応：`sparkles` / `ticket` / `play_fill` / `calendar` / `person_2` / `person` / `info_circle`。`Semantics(label:)` に §7.2 のラベル |
| `formatPublishedDate` | `presentation/published_date_format.dart` | `DateTime local`、`DateTime now` | S-00 §7.3：同じ暦日 → 「今日」、前日 → 「昨日」、同じ年 → `M/d`、それ以外 → `yyyy/M/d`。時刻は出さない |
...
| ケース | 期待する振る舞い | 担当する層 |
|---|---|---|
| 機内モード・DNS 解決失敗・接続拒否（`SocketException` / `http.ClientException`） | `SyncOffline`。DB に触れない。取得済みの記事があれば ST-13、無ければ ST-16 | infrastructure（`HttpArticlesFeed`）→ application |
...
| `feed_etag` があるのに `articles` が 0 件 | 起きない：`feed_etag` は記事の反映と同じトランザクションでしか書かれない。万一（手動で DB を消した等）は 304 が返り続けるが、`articles` が 0 件のときは `feedEtag()` が null を返す実装にして防ぐ（`SELECT` で `articles` の件数が 0 なら null） | infrastructure（`DriftArticleRepository`） |
| `schemaVersion: 2`（対応外） | `SyncFailed(unsupportedSchema)`。端末内の記事・既読・保存を消さない（D-01 §4.6） | infrastructure |
...
| 取得中に再度取得を要求（Pull to Refresh 連打・復帰直後の通知タップ） | 新しい取得を始めず実行中の Future を返す（S-01 §8 #12） | presentation（`SyncController`） |
| 取得中にアプリがバックグラウンドへ | 取得は続く。iOS が停止させた場合は次の復帰で `foreground` の取得が走る。中断された取得の `Future` が完了しない場合に備え、`SyncController` は `feedTimeout + 5 秒` で `_inFlight` を諦めて `SyncFailed(timeout)` にする | presentation |
...
| `companies.json` が読めない・`schemaVersion != 1`・`id` 重複 | 起動時に `StateError` でクラッシュ（ビルドの不備。collector の契約テストが CI で先に検出する。D-01 §7.1） | app（`bootstrap`） |
...
| `subscribe` / `unsubscribe` が失敗（APNs トークン未取得・ネットワーク） | その団体だけ `failed` に入れて続行。画面には出さない（S-03 §8 #4）。次回起動・次回トグル操作で再度同期される | application |
...
| `pushBackend == 'fcm'` だが `GoogleService-Info.plist` が無い | `Firebase.initializeApp()` が失敗し起動時にクラッシュ（設定の不備を黙って Noop に縮退させない。D-02 §8 #32 と同じ考え方） | app（`bootstrap`） |
| 通知ペイロードに `data.companyId` が無い・文字列でない | `NotificationTap(companyId: null)`。「すべて」で開くのは D-05（S-01 §8 #15） | infrastructure（`FcmPushGateway`） |
| `data.count`・`notification.title/body` | 読まない（D-01 §8.1） | infrastructure |
...
| サムネイルの取得失敗・オフライン | `errorBuilder` でプレースホルダー（ST-06）。再試行の操作は出さない。ディスクに保存しない | presentation（`ArticleCell`） |
...
| `inactive → resumed`（アプリ内ブラウザを閉じた・許可ダイアログを閉じた） | 取得しない。`paused → resumed` のみ取得する（§5.3） | app（`AppLifecycleSync`） |
| 起動直後に `resumed` が届く | `initState` で `launch` を起動済みなので、`AppLifecycleSync` は直前の状態を `resumed` として初期化し、最初の `resumed` を無視する | app |

## 7. テスト方針

テストの単位は UseCase（CLAUDE.md）。Widget テスト・個別クラスのユニットテストは書かない（例外は本節末尾の純粋関数 4 つだけ。§8 #28）。差し替えるのは外部 I/O（HTTP・FCM・アセット）で、drift は `NativeDatabase.memory()` の**実物**を使う（`DriftArticleRepository` の SQL、特に保存済み記事の保護とトランザクションを UseCase 経由で検証するため）。HTTP は `package:http` の `MockClient` で `HttpArticlesFeed` を実物のまま通し、`ArticlesFileCodec` の検証（D-01 §7.3「読み取り（D-04）」）も UseCase から到達させる。ネットワークへは一切アクセスしない。`Firebase`・`FcmPushGateway`・`bootstrap`・`SyncController`・Widget はテストしない。
...
| `SyncArticlesUseCase` | **保存の失敗**：`applyFeed` が途中で例外を投げる（テスト用に `ArticleRepository` を実物のラッパで包み 2 件目で投げる）と `SyncFailed(storage)` で DB が変わらず `feed_etag` も更新されない | in-memory drift + 例外を注入するラッパ |
| `SyncPushSubscriptionsUseCase` | 設定行が無い → `companies.json`（実アセット）の 5 団体すべてを配列順に `subscribe`／`notification.toho = '0'` → `toho` だけ `unsubscribe`、他 4 つは `subscribe`／すべて `'0'` → 5 つとも `unsubscribe`／`companies.json` に無い団体の設定行は無視／`shiki` の `subscribe` が例外 → `failed: ['shiki']` で残り 4 団体は処理される／トピック名は `fcmTopic`（`id` と同じ値）／設定を `'1'` に戻して再実行すると再び `subscribe`（冪等） | `FakePushGateway`、in-memory drift（`SettingsRepository` 実物）、`AssetCompanyRepository` 実物（`rootBundle`。`TestWidgetsFlutterBinding.ensureInitialized()`） |
...
**純粋関数の直接テスト（例外）**：状態判定と並び順の純粋関数 4 つは UseCase を経由しないため、CLAUDE.md「テスト方針」の例外として関数ごとにテーブル駆動テストを置く（§8 #28。CLAUDE.md への追記は §8.1）。対象は「入出力が値だけで、I/O・Provider・Widget に触れない関数」に限り、それ以外の個別クラステストは引き続き書かない。
...
| `resolveListStatus`（`articles/presentation/list_status.dart`） | S-00 §5.2 の判定表 6 行 × 4 出力（`full`・`refreshing`・`offlineBanner`・`errorNotice`）と、`hasVisible = false` で `FullView.empty` になる行（§5.4） | `test/features/articles/presentation/list_status_test.dart` |
| `formatPublishedDate`（`articles/presentation/published_date_format.dart`） | S-00 §7.3 の 4 書式（「今日」・「昨日」・同年の `M/d`・前年以前の `yyyy/M/d`）と、端末タイムゾーンで日付が変わる境界（UTC の同じ瞬間が `now` の日付をまたぐ場合）。`now` と対象日時は引数で与える | `test/features/articles/presentation/published_date_format_test.dart` |
...
| 7 | オフライン（S-00/ST-13・ST-16）の判定は `SocketException` / `http.ClientException` の捕捉で行い、タイムアウトはエラー（ST-14・ST-15）に分類する。接続状態を監視するパッケージは入れない | `connectivity_plus` で失敗後に接続状態を照会する／タイムアウトもオフラインにする | S-00 §5.2 の区別は「ネットワーク不通」か「ネットワークはあるが失敗」かで、`dart:io` の `SocketException`（DNS 失敗・接続拒否・機内モード）がそれに対応する。`http` の `IOClient` はこれを `ClientException` に包むことがあるため両方を捕捉する。タイムアウトは回線が細い場合にも起きるためオフラインとは言えない。接続監視は依存が増え、Wi-Fi 接続済みでインターネットに出られない状態を正しく判定できない |
...
| 16 | サムネイルは `Image.network` のメモリ内キャッシュだけを使い、ディスクに書くキャッシュパッケージを使わない | `cached_network_image` 等でディスクキャッシュする | CLAUDE.md「画像（サムネイル）を端末にコピー保存しない。URL 参照のみ」（調査レポート §6.2）。オフライン時に画像が出ないのは S-00/ST-06 が許容している |
...
| 21 | `companies.json` は `bootstrap` で 1 度読み、`companiesProvider` に同期値として override する | `FutureProvider` でアセットを非同期に読む | 上部タブ（S-01）・通知トグル（S-03）は取得前にも描く必要があり（D-01 §8 #28）、非同期だと全画面が `AsyncValue` の分岐を持つ。アセットの読み込みは数ミリ秒で起動速度（要件 §5）に影響しない。読めない場合はビルドの不備なので起動時に落とす（§6） |
...
| 28 | 状態判定と並び順の純粋関数（`resolveListStatus`・`resolveReadDisplayState`・`compareArticles`・`formatPublishedDate`）は CLAUDE.md「テスト方針」の例外として直接テストし、`test/features/articles/` に関数ごとのテーブル駆動テストを置く（§7） | 規約どおり書かず、UseCase テスト（`compareArticles` は `SyncArticlesUseCase` 経由）と D-05 の Provider 経由の検証に任せる | S-00 §5.2 の判定表（6 行 × 4 出力）は本書の中心的な規則で、UseCase を経由しないため UseCase テストでは網羅できない。D-02 §8 #30 で infrastructure の例外を認めた前例と同じく、対象を「入出力が値だけの純粋関数」に限定すれば規約の趣旨（Widget・個別クラスの実装詳細を固定しない）は保てる。開発者確認済み（2026-09-19、旧 Q-5）。CLAUDE.md は D-04 承認後に司令塔が更新する（§8.1） |
...
| D-05 | ホームの表示対象は `articles.in_feed = true` の記事のみ（S-01 §8 #10）。保存画面は `saved_articles` に行がある記事すべて（`in_feed` を問わない。S-02 §7.3）。`ArticleRepository` に `watchInFeed()` 等の閲覧メソッドを追加する | §4.2、§4.4 |
...
| D-05 | 通知トグル（S-03/A-01）は `SettingsRepository.setNotificationEnabled` の後に `SyncPushSubscriptionsUseCase` を呼ぶ（差分ではなく全団体を同期）。S-03/ST-03 の判定は `pushPermissionStatusProvider`（`authorized` 以外で E-09）。設定アプリから戻ったとき（S-03/A-07）の読み直しは本書の `AppLifecycleSync` が invalidate する | §5.6、§5.7 |
...
| D-05 | `SettingsRepository` に `browser` / `unreadFilter` の get・set・watch を追加する。キーと既定値は `SettingKeys`（§4.4）。未読フィルタは S-01 §8 #19 のとおり永続化する | §4.4 |
...
| CLAUDE.md（司令塔。D-04 承認後に更新。§8 #26） | `app/` の「ディレクトリ（feature-first × クリーンアーキテクチャ）」のツリーに次を追記する。`lib/` 直下に `app/`（ルート Widget・起動処理・下部タブ・ライフサイクル監視。feature に属さない）、`core/` に `logging/`（logger の生成）と `ui/status/`（S-00 の共通状態表示 Widget）、`features/` に `companies/`（同梱 `companies.json` の読み込み）、`test/` に `helpers/`（in-memory DB・MockClient・FakePushGateway・記事ビルダ）と `fixtures/`（`articles.sample.json`）。あわせて `core/di/` の説明を「具象の import を許可する唯一の場所（`lib/app/bootstrap.dart` を含む）」にする | §3.2 |
| CLAUDE.md（司令塔。D-04 承認後に更新。§8 #28） | 「テスト方針」の「Widget テスト・個別クラスのユニットテストは書かない」の直後に例外を 1 項目追記する。文言：「例外：入出力が値だけで I/O・Provider・Widget に触れない**純粋関数**（状態判定・並び順・日時書式。app では `resolveListStatus`・`resolveReadDisplayState`・`compareArticles`・`formatPublishedDate`）は、関数ごとのテーブル駆動テストを `test/features/<feature>/{domain,presentation}/` に置いてよい。対象は設計書 §7 に列挙したものに限る」 | §7 |
...
| T-B1 | articles / companies の domain | `features/articles/domain/article.dart`・`category.dart`・`articles_file.dart`・`article_order.dart`・`article_repository.dart`・`articles_feed.dart`・`read_display_state.dart`、`features/articles/application/sync_result.dart`、`features/companies/domain/company.dart`・`company_repository.dart`、`test/features/articles/domain/article_order_test.dart`・`read_display_state_test.dart`。計 12 | `lib/features/articles/{domain,application}/`、`lib/features/companies/domain/`、`test/features/articles/domain/` | T-A | `flutter analyze && flutter test` が通る。`compareArticles`・`resolveReadDisplayState` の直接テスト（§7 純粋関数の例外）を含む。CI の `flutter test` の条件を外す |
| T-B2 | settings / notifications / saved の domain | `features/settings/domain/settings_repository.dart`・`browser_choice.dart`、`features/notifications/domain/push_gateway.dart`、`features/saved/domain/saved_article_repository.dart`。計 4 | `lib/features/{settings,notifications,saved}/domain/` | T-A | 同上。T-B1 と並行可（触るディレクトリが互いに素） |
| T-C | drift と Repository 実装、DI | `core/database/tables.dart`・`app_database.dart`（+ 生成物）、`features/articles/infrastructure/drift_article_repository.dart`、`features/saved/infrastructure/drift_saved_article_repository.dart`、`features/settings/infrastructure/drift_settings_repository.dart`、`features/companies/infrastructure/asset_company_repository.dart`、`core/di/providers.dart`（Repository の Provider まで。UseCase の Provider は T-D・T-E が追記）、`test/helpers/in_memory_database.dart`。計 8 | `lib/core/{database,di}/`、`lib/features/*/infrastructure/`、`test/helpers/` | T-B1、T-B2 | `flutter analyze` 警告ゼロ。テストは T-D・T-E が UseCase 経由で行う |
| T-D | 配信の取得と SyncArticlesUseCase | `features/articles/infrastructure/articles_file_codec.dart`・`http_articles_feed.dart`、`features/articles/application/sync_articles_use_case.dart`、`core/di/providers.dart` 追記、`test/fixtures/articles.sample.json`（collector からコピー）、`test/helpers/mock_feed_client.dart`・`test_articles.dart`、`test/features/articles/application/sync_articles_use_case_test.dart`。計 7 + 追記 1 | `lib/features/articles/{application,infrastructure}/`、`lib/core/di/`、`test/` | T-C | `flutter analyze && flutter test` が通る。§7 の `SyncArticlesUseCase` 5 行のケースをすべて含む |
| T-E | 通知の Noop 実装と UseCase | `features/notifications/infrastructure/noop_push_gateway.dart`、`features/notifications/application/sync_push_subscriptions_use_case.dart`・`request_notification_permission_use_case.dart`、`features/notifications/presentation/notification_tap_providers.dart`、`core/di/providers.dart` 追記、`test/helpers/fake_push_gateway.dart`、`test/features/notifications/application/sync_push_subscriptions_use_case_test.dart`・`request_notification_permission_use_case_test.dart`。計 7 + 追記 1 | `lib/features/notifications/`、`lib/core/di/`、`test/` | T-C | 品質ゲート通過。§7 の 2 UseCase のケースをすべて含む。T-D と並行可（`providers.dart` の追記箇所が重ならないよう、T-D は articles 節、T-E は notifications 節を別ブロックで追加する） |
| T-F1 | アプリの骨格（起動・下部タブ・取得の契機・状態判定） | `lib/app/bootstrap.dart`・`root_tabs.dart`・`tab_reselect.dart`・`app_lifecycle_sync.dart`、`lib/app/app.dart` 改修、`features/articles/presentation/sync_controller.dart`・`list_status.dart`（+ 生成物）、`test/features/articles/presentation/list_status_test.dart`。計 7 + 改修 1 | `lib/app/`、`lib/features/articles/presentation/`、`test/features/articles/presentation/` | T-D、T-E | 品質ゲート通過。シミュレータで起動し、3 タブが切り替わり、起動直後に `sync(launch)` が走ってログに結果が出る。`resolveListStatus` の判定表 6 行の直接テスト（§7）を含む |
| T-F2 | 共通 Widget（記事セル・状態表示） | `core/ui/status/` の 6 ファイル、`features/articles/presentation/widgets/article_cell.dart`・`category_icon.dart`、`features/articles/presentation/published_date_format.dart`、`test/features/articles/presentation/published_date_format_test.dart`。計 10 | `lib/core/ui/`、`lib/features/articles/presentation/`、`test/features/articles/presentation/` | T-B1 | `flutter analyze && flutter test` が通る。`formatPublishedDate` の直接テスト（§7）を含む。T-F1 と並行可（テストファイル名も互いに素） |
| T-G | FCM 実装（フェーズ 5。**Apple Developer Program 加入後**） | `features/notifications/infrastructure/fcm_push_gateway.dart`、`pubspec.yaml` に `firebase_core`・`firebase_messaging`、`bootstrap.dart` の `fcm` 分岐、`providers.dart` の `fcm` 分岐、`ios/Runner` の Push Notifications / Background Modes（remote-notification）の capability、`.gitignore` に `GoogleService-Info.plist`（§8 #27。開発者が手元の `app/ios/Runner/` に置く）。計 1 + 改修 5 | `lib/features/notifications/infrastructure/`、`lib/app/`、`lib/core/di/`、`app/ios/` | T-F1、要件 §10 フェーズ 5 | 品質ゲート通過（`none` ビルド）。`--dart-define=CURTAINCALL_PUSH=fcm` の実機ビルドで、collector（D-02）からの通知が届き、タップで `companyId` が `notificationTapStreamProvider` に流れる。新規テストは無し（`FakePushGateway` で UseCase は検証済み） |
