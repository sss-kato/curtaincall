...
| S-00/A-12 | 操作 | `FullErrorView.onRetry` コールバック。**再試行は 2 系統**（S-00 §6 A-12）で、D-05 が全面表示の種別（`ListStatus.full`）で分岐する：`error` / `offline`（ST-14 / ST-16）→ `SyncController.sync(SyncTrigger.retry)` **と** `CountRetryRequested.retry()`（`retryRequested` を立てて `articleCountProvider` を invalidate する 1 メソッド）の両方（取得のやり直し + 端末の読み出しのやり直し）。`unavailable`（ST-17）→ `retry()`（と D-05 が持つ一覧 Stream の Provider の invalidate）だけを行い、**`sync()` を呼ばない**（取得は行わない）。**どちらの系統も `retry()` を呼ぶ**理由（`ref.invalidate` は直前値を捨てないため、明示的な再試行であることを `retryRequested` として写像に渡さないと ST-17 へ移れない）は §5.4 の表と §8 #63 | §5.3 / §5.4 / §5.9 |
...
| S-00/ST-12 | 状態 | `ListStatus.full == FullView.empty`（**記事件数を読み出せたうえで**表示対象 0 件。`ArticleCountState.empty` / `present` のときだけ成立し、読み出せないとき（`unavailable`）は ST-17 に落ちる。S-00 §8 #36。文言は配置画面 = D-05）。**件数は読み出せたが一覧の中身を組み立てられなかったとき**（D-05 が `hasVisible: false` を渡す）もここに落ちる（S-01 §5.1 ST-04。§5.4 の判定表の末尾行） | §5.4 |
...
| S-00/ST-17 | 状態 | `ListStatus.full == FullView.unavailable`（`resolveArticleCount` が `ArticleCountState.unavailable`＝記事件数 Stream が**アプリの起動から一度も**値を得ずにエラーで終わった。一度でも件数を得た後の**受動的な**エラーは直前の件数で判定し本状態にならない。**例外は利用者が明示的に押した再試行**（A-12 / A-08）で、`retryRequested == true` の間は直前値があっても失敗を本状態に写す（S-00 §6 A-12 の移り先。S-00 §5.2 ST-17 の限定・S-01 §5.1 ST-19。§8 #63）。判定表の先頭行で、`lastResult`・`inProgress` を問わず成立し、`refreshing`・`offlineBanner`・`errorNotice` はすべて false（裏の取得の成否で抜けず、E-21・E-23・E-25 を重ねない。S-00 §5.2 ST-17・§8 #36、S-01 §5.1 ST-19）。全面は `FullErrorView(kind: FullErrorKind.unavailable, onRetry: …)`（§5.9）。抜ける経路は A-12 の再購読だけ | §5.4 / §5.9 |
...
| S-01/A-08（再試行。ST-19 からの再試行を含む） | ST-07・ST-08 からは `SyncTrigger.retry` と `CountRetryRequested.retry()`、ST-19（= S-00/ST-17）からは `retry()` のみ（`sync()` を呼ばない）。`retry()` はどちらも `retryRequested` を立てて再購読するため、読み出しに失敗すれば ST-19 へ移れる（S-01 §6 A-08 の移り先に ST-19 が含まれる。`ref.invalidate` だけでは直前値が残って移れない。§8 #63）。分岐は S-00/A-12 の行と §5.4 | §5.3 / §5.4 |
...
| S-02/ST-01・ST-02・ST-06（保存一覧の content / empty / 読み出せない）・A-05 | `ListStatus`・`FullView`（`unavailable` を含む 6 値）の**型だけ**（`lib/core/ui/status/list_status.dart`）と `FullErrorView`（`FullErrorKind.unavailable`。§5.9）。判定は D-05 が保存一覧の `AsyncValue` から直接 `content` / `empty` / `unavailable` を組み立て、A-05 は `savedArticlesProvider` の再購読のみを行う（取得は行わない。S-02 §8 #1・#11）。**本書の `resolveListStatus` はホーム（S-01）専用**で S-02 は通さない（§8 #51。S-02 は「やり直し中も E-24 を出したまま」で S-01 と写し方が異なる。S-02 §5 ST-06） | §5.4 / §5.9 / §8.1 |
...
app/
  ...
  test/
    ...
    features/
      ...
      articles/presentation/list_status_test.dart          resolveListStatus・resolveArticleCount・shouldLogCountError の直接テスト（同上）
...
**`unavailable` になるのは「アプリを起動してから一度も件数が分からない」ときに限る**（S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06）。ここで言う「一度も」は**受動的な失敗**（利用者の操作を伴わずに Stream がエラー終了した、など）についての限定で、一度でも件数を得た後に読み出せなくなったときは ST-17 にせず、直前に表示していた内容（一覧または ST-12 の空表示）をそのまま保ち、利用者への知らせも出さない（一度見えていた一覧を全面のエラー表示に置き換えないため）。本書はこれを次の 2 つで満たす（§8 #63）：(1) `resolveArticleCount` の分岐 1 が `hasValue: true`（`AsyncError` / `AsyncLoading` が直前値を保持している場合を含む）を**最優先**で拾い、件数の値がある限り `unavailable` を返さない（`retryRequested == false` のとき。下記の例外を参照）。(2) `articleCountProvider` が `keepAlive: true` で、アプリの生存中は状態（＝一度得た件数）を捨てない。下部タブでホームを離れて戻っても再購読は起きず、「起動してから一度も」の単位と一致する。

**例外は「利用者が明示的に押した再試行」（S-00/A-12・S-01/A-08・S-02/A-05）だけ**：この 3 つでは `ref.invalidate` で購読を張り直し（新しい件数を得る唯一の手段）、**あわせて写像の入力 `retryRequested` を true にする**（下の「全面表示からの復帰」）。押した時点の画面はいずれも全面表示（ST-14・ST-16・ST-17）で**一覧は出ていない**ため、上の限定が守ろうとしている「見えていた一覧を消さない」とは衝突しない。
...
/// 記事件数の 4 状態（写像は resolveArticleCount）。
...
/// **状態クラス（AsyncData / AsyncError）ではなく hasValue / isLoading / hasError で判定する**（下記の理由）。
/// [retryRequested] = 利用者が A-12 / A-08 を押してから、次の件数が届くまでの間（countRetryRequestedProvider）。
/// ref.invalidate は直前値を捨てない（§5.4 本文）ため、明示的な再試行かどうかを入力として受け取る（§8 #63）
...
/// hasValue だけでは判定できない（§5.4 本文・§8 #63）
...
/// **ホーム（articleCountProvider）専用**。保存画面（S-02/A-05）はこの Provider を使わない（§5.4 本文）。
...
class CountRetryRequested extends _$CountRetryRequested {
  ...
  /// invalidate を伴わずに立てると下りる契機が二度と来ない（§5.4 本文・§8 #63）
  void retry() {
    state = true;
...
**全面表示からの復帰（再試行の 2 系統）**：S-00 §6 A-12 は再試行の意味を発生元の状態で分けている。本書はそれを `ListStatus.full` の値で分岐させる（D-05 が `FullErrorView.onRetry` に組み込む。§8.1）。
...
**2 系統の違いは `sync(retry)` を伴うかどうかだけで、`retry()` はどちらも呼ぶ。** ST-14 / ST-16 でも `retry()` を呼ぶ理由：(1) **これが無いと回復できない経路がある**。記事 0 件の端末で件数 Stream が「0 を 1 度 emit した後」にエラー終了すると、`resolveArticleCount` は分岐 1（`hasValue: true`）で `empty` を返し続け（自動リトライは切ってあり（§8 #61）、`keepAlive: true` で破棄もされない）、`sync(retry)` だけで取得に成功しても件数は `empty` のままで ST-12 から抜けられない（§6 の該当行）。invalidate は**購読を張り直して新しい件数を得る唯一の手段**なので必須である。(2) S-00 §6 A-12 は ST-14 / ST-16 からの再試行の**移り先に ST-17 を挙げている**。ただし invalidate だけでは直前値が残る（上記のとおり `copyWithPrevious` が引き継ぐ）ため、`retry()` が `retryRequested` を立てることによって、**再購読が飛行中は `pending`（ST-10）・再び失敗したときは `unavailable`（ST-17、S-01 では ST-19）**へ移り、画面定義書の移り先の列挙と一致する。(3) 取得のやり直しに失敗しても、読み出しが成功していれば一覧（または ST-12・ST-14・ST-16）へ通常どおり移る（新しい件数が届いた時点で `shouldClearRetryRequested` が true になり `retryRequested` は false に戻る）。§8 #63 の「直前値を保つ」限定は**受動的な失敗**についての規則で、利用者が明示的に押した A-12 / A-08 / A-05 はその例外（押した時点で画面に一覧は出ていない）。**復帰した後も限定は効き続ける**：一覧に戻った端末で件数 Stream が受動的にエラー終了しても、`retryRequested` は既に false なので分岐 1 が直前値を使い、一覧は覆われない。下ろし損ねるとこの限定が壊れるため、下ろす条件は `shouldClearRetryRequested` として切り出し、§7 のテーブル駆動テストで「早すぎる下ろし」「下ろし損ね」の両方を検出する。

`unavailable` は `retryRequested == false` のとき `articleCountProvider` が**値を 1 度も得ないままエラーで終了**した場合（起動直後の DB の一時ロック・migration の一過性失敗・破損）に成立し、`lastResult` が何であっても判定表の先頭行で拾われる（S-00 §8 #36「読み出せないときは ST-12 にしない」）。**記事が表示できていた後の受動的な I/O エラーはここに入らない**（上記 (1)(2)。§8 #63）。この「直前値を残す」挙動は Riverpod の `copyWithPrevious` に依存する（3.4.3 のソースで確認済み。§8 #63）ため、**版を上げたときは、シミュレータで「一覧が出た後に件数の読み出しを失敗させても一覧が残り、全面の E-24 に置き換わらない」ことを目視で確認する**（§10 T-F1・T-F3 の完了条件）。**この状態から抜ける経路は A-12 による再購読だけ**で、押すと `retryRequested` が true になるため再購読の実行中は `pending` → ST-10（E-20。S-01 §5.1 ST-02・§8 #31）、失敗すれば再び `unavailable` に戻って何度でも再試行できる（ST-14 / ST-16 から押した場合も、直前値 0 を持っていても同じ移り方になる）。再購読が無いと、DB が正常に戻ってもアプリを再起動するまで全面表示が一覧を覆い続ける（要件 §5「オフライン時：取得済みの記事は閲覧可能」に反する）。

**再購読が成功して 0 件だったとき**の移り先は、判定表の `empty` × `hasVisible = false` の 5 行（3 行目の `hasVisible = true` → `content` は含まない）がそのまま S-01 §8 #31 の 4 分岐（取得成功・未実行 → ST-12／オフライン → ST-16／エラー → ST-14／取得継続中 → ST-10）になる。ここで `lastResult == null`（取得を一度も実行していない）は `launchPending` の行に落ちて ST-10 になる。**この組み合わせは起動直後の 1 フレームに限らず、`sync(launch)` が完了するまで（最長 `overallTimeout` = 20 秒。§5.3）成立する**：起動直後に件数の読み出しが失敗して ST-17 になり、`launch` が飛行中のうちに A-12 を押して再購読に成功し 0 件だった場合がこれにあたる。表示は ST-10（E-20）で、S-01 §8 #31 の「読み出しに成功した時点で取得がまだ続いている → ST-02」と一致する（S-00 §8 #4・S-01 §5.1 ST-02 も ST-10 を求める）。**したがって 4 行目（`launchPending`）は到達可能で、テストから外さない**（§7）。
...
**Riverpod 3 の自動リトライ**（T-23 の実測）：`ProviderContainer` の既定 `retry`（`maxRetries = 10`、指数バックオフ上限 6.4 秒）のままだと、`Stream` がエラーを出しても状態は直ちに `AsyncError` にならず、リトライが尽きるまでの約 50 秒は `AsyncLoading(hasError: true, ...)` として現れる。これは **A-12 の再購読中とまったく同じ形**なので、両者を `AsyncValue` から区別できない。`articleCountProvider` は自動リトライを切る（上記 `_noRetry`。§8 #61）ことでこの重なりを無くし、「値なし × `isLoading`」を**利用者が再試行を押した結果の再購読**に限定する。`AsyncData` / `AsyncError` の状態クラスだけを見る `switch` は記事があるのに全面ローディングになるため、写像は引き続き `hasValue` / `isLoading` / `hasError` で行う。

判定表（上から順に最初に一致した行を採る）。**この表はホーム（S-01）にだけ適用する**（保存画面の `content` / `empty` は D-05 が `hasVisible` から直接組み立てる。§8 #51）：

| `count` | `inProgress` | `lastResult` | `launchPending` | `hasVisible` | `full` | `refreshing` | `offlineBanner` | `errorNotice` | S-00 |
|---|---|---|---|---|---|---|---|---|---|
...
| `empty` | 任意 | 任意 | 任意 | true | `content` | `inProgress` | `lastResult is SyncOffline` | `lastResult is SyncFailed` | 記事一覧（**件数 Stream の更新が一覧 Stream より遅れている過渡状態**。一覧に行があるのに E-22 を重ねない。S-00 §8 #4。§8 #65） |
| `empty` | 任意 | 任意 | true | — | `loading` | false | false | false | ST-10（起動時の取得が未開始・実行中。§8 #36） |
| `empty` | true | 任意 | false | — | `loading` | false | false | false | ST-10 |
...
| `present` | 任意 | 任意 | 任意 | true | `content` | `inProgress` | `lastResult is SyncOffline` | `lastResult is SyncFailed` | 記事一覧 + ST-11 / ST-13 / ST-15 |
| `present` | 任意 | 任意 | 任意 | false | `empty` | `inProgress` | `lastResult is SyncOffline` | `lastResult is SyncFailed` | ST-12 + ST-11 / ST-13 / ST-15（S-00 §8 #24） |

- 起動直後の最初のフレームは `sync(launch)` が `initState` の post-frame callback で始まるため `inProgress == false`・`lastResult == null`・`count == pending` である。2 行目（`pending`）と 4 行目（`launchPending`）により、この間に出るのは E-20（ST-10）であり、E-22（ST-12）は一瞬も出ない
- **件数 Stream（drift の `watchCount()`）と `SyncStatus`（`SyncCompleted`）は別経路で更新される。** 初回起動で 100 件を反映した直後は、`SyncStatus` が先に `SyncSucceeded` になり、件数の再クエリ結果（0 → 100）はコミット後の別マイクロタスク以降に届くため、両者が同じフレームに収まらないと `empty` × `inProgress: false` × `SyncSucceeded` × `launchPending: false` が一瞬成立する。3 行目（`empty` × `hasVisible = true` → `content`）がこの窓を埋め、D-05 の一覧 Stream が既に 100 件を持っていれば E-22（ST-12）は出ない（S-00 §8 #4・S-01 §8 #31 の理由 (4)。§8 #65）。一覧もまだ 0 件なら 8 行目の `empty`（ST-12）に落ちるが、それは「本当に 0 件」の状態と区別がつかず、次のフレームで一覧が届けば `content` に切り替わる
...
| **件数 Stream が値（0 件）を得た後にエラー終了し、その後の再試行で取得に成功した**（記事 0 件の端末で ST-14 / ST-16 が出ている状態） | A-12 が `sync(retry)` **と** `CountRetryRequested.retry()` を行うため、取得で入った記事が件数の読み直しで反映され一覧に戻る（届いた時点で `shouldClearRetryRequested` が true になり `retryRequested` は false に戻る）。invalidate しないと `resolveArticleCount` が直前値（0）を返し続けて `empty` のままになり、**ST-12「記事がありません」から抜けられず回復手段がアプリの再起動だけ**になる（§5.4「全面表示からの復帰」・§8 #63） | presentation / D-05 |
...
| 16 | サムネイルは `Image.network` のメモリ内キャッシュだけを使い、ディスクに書くキャッシュパッケージを使わない。デコードサイズは **`cacheHeight`**（84pt × `devicePixelRatio`。#55）で表示サイズに揃える | `cached_network_image` 等でディスクキャッシュする／デコードサイズを指定せず原寸でデコードする | CLAUDE.md「画像（サムネイル）を端末にコピー保存しない。URL 参照のみ」（調査レポート §6.2）。オフライン時に画像が出ないのは S-00/ST-06 が許容している。原寸デコードは 2000×2000 の OGP 画像（16 MB/枚）が 100 件で `ImageCache` の上限を超え、メモリ警告と再取得の往復を招く（レビュー第 1 周 M-17） |
...
| 59 | 対応外 `schemaVersion` の抑止中（`launch` / `foreground` で通信しない）は、**S-01 §8 #23 の例外として画面定義書に明記された**。この間は ST-03（E-21）も ST-15（E-25）も発生しない（S-00 §8 #31）。本書の抑止規則（#32）と `suppressed`（#44）は変えず、D-05 は `suppressed == true` の `SyncFailed` で `TransientNotice` を呼ばない（§6・§8.1） | S-01 を変えず設計書の申し送りに留める（画面定義書と食い違ったままにする）／抑止をやめて毎回取得する（#32 を覆す） | 画面定義書が上流（CLAUDE.md）で、食い違ったままにすると実装者が #23 を根拠に抑止を外しうる。抑止をやめると起動・復帰のたびに 200 KB を取り直す問題（#32 の根拠）が戻る。**反映済み**（S-01 §8 #23・S-00 §8 #31。開発者決定 2026-09-24、旧 §9 Q-5、推奨案 (A)） |
...
| 61 | 記事件数の写像を 4 値の `ArticleCountState`（`pending` / `unavailable` / `empty` / `present`）に広げ、判定表の先頭行で `unavailable` を ST-17（`FullView.unavailable`）に落とす。あわせて **`articleCountProvider` の自動リトライを切り**（`retry: _noRetry`）、「値なし × `isLoading`」を A-12 による再購読中の合図として使う。関数名は `resolveHasArticles` → **`resolveArticleCount`**（§5.4・§7） | 真偽値のまま `hasError → false` に写し、`lastResult` で ST-14 / ST-12 に分ける（従来）／`unavailable` は足すが自動リトライは既定のまま残す | S-00 §8 #36 が「読み出せず件数が分からないときは ST-17 で、ST-12 にしてはならない」と確定したため、真偽値では表せない（従来は `lastResult` が `SyncSucceeded` のとき ST-12 に落ちる既知の限界があり、再試行の動線が無かった）。自動リトライを残すと「値なし × `isLoading` × `hasError`」が**自動リトライ中（約 50 秒）と A-12 の再購読中の両方**で成立し、S-01 §5.1 ST-02（再試行の実行中は全面ローディング）と ST-19（読み出せない間は E-24 のまま）を区別できない。切れば「値なし × `isLoading`」＝利用者が押した再購読に限定でき、障害の発覚も 50 秒早まる。自動回復の喪失は ST-17 の再試行ボタン（S-00/A-12）が代替する。改名は `bool?` を返さなくなったためで、CLAUDE.md の純粋関数の列挙も 1 語を差し替える（§8.1。`shouldSyncOnResume` → `resolveResumeSync` と同じ扱い）（画面定義書 S-00 §8 #36・S-01 §8 #31 への追随、2026-09-24） |
