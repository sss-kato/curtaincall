...
| S-00/A-12 | 操作 | `FullErrorView.onRetry` コールバック。**再試行は 2 系統**（S-00 §6 A-12）で、D-05 が全面表示の種別（`ListStatus.full`）で分岐する：`error` / `offline`（ST-14 / ST-16）→ `SyncController.sync(SyncTrigger.retry)` **と** `ref.invalidate(articleCountProvider)` の両方（取得のやり直し + 端末の読み出しのやり直し）。`unavailable`（ST-17）→ `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）だけを行い、**`sync()` を呼ばない**（取得は行わない）。**どちらの系統も invalidate する**理由（利用者が明示的に押した操作では件数の読み出しをやり直す）は §5.4 の表と §8 #63 | §5.3 / §5.4 / §5.9 |
...
| S-00/ST-17 | 状態 | `ListStatus.full == FullView.unavailable`（`resolveArticleCount` が `ArticleCountState.unavailable`＝記事件数 Stream が**アプリの起動から一度も**値を得ずにエラーで終わった。一度でも件数を得た後のエラーは直前の件数で判定し本状態にならない。S-00 §5.2 ST-17 の限定・S-01 §5.1 ST-19。§8 #63）。判定表の先頭行で、`lastResult`・`inProgress` を問わず成立し、`refreshing`・`offlineBanner`・`errorNotice` はすべて false（裏の取得の成否で抜けず、E-21・E-23・E-25 を重ねない。S-00 §5.2 ST-17・§8 #36、S-01 §5.1 ST-19）。全面は `FullErrorView(kind: FullErrorKind.unavailable, onRetry: …)`（§5.9）。抜ける経路は A-12 の再購読だけ | §5.4 / §5.9 |
...
| S-01/A-08（再試行。ST-19 からの再試行を含む） | ST-07・ST-08 からは `SyncTrigger.retry` と `articleCountProvider` の再購読、ST-19（= S-00/ST-17）からは再購読のみ（`sync()` を呼ばない）。どちらも再購読を伴うため、読み出しに失敗すれば ST-19 へ移れる（S-01 §6 A-08 の移り先に ST-19 が含まれる）。分岐は S-00/A-12 の行と §5.4 | §5.3 / §5.4 |
...
| S-02/ST-01・ST-02・ST-06（保存一覧の content / empty / 読み出せない）・A-05 | `ListStatus`・`FullView`（`unavailable` を含む 5 値）の**型だけ**（`lib/core/ui/status/list_status.dart`）と `FullErrorView`（`FullErrorKind.unavailable`。§5.9）。判定は D-05 が保存一覧の `AsyncValue` から直接 `content` / `empty` / `unavailable` を組み立て、A-05 は `savedArticlesProvider` の再購読のみを行う（取得は行わない。S-02 §8 #1・#11）。**本書の `resolveListStatus` はホーム（S-01）専用**で S-02 は通さない（§8 #51。S-02 は「やり直し中も E-24 を出したまま」で S-01 と写し方が異なる。S-02 §5 ST-06） | §5.4 / §5.9 / §8.1 |
    ...
    features/
      articles/
        ...
        presentation/
          ...
          list_status.dart              FeedStatus・ArticleCountState・resolveListStatus（**ホーム専用**）・resolveArticleCount・shouldLogCountError、feedStatus / articleCount の 2 Provider（§5.4。ListStatus・FullView は core/ui/status/list_status.dart から import する。§8 #51・#61）。
...
final class SyncCoordinator {
  ...
  /// false なら execute(trigger, isCancelled) を始めて返す。
  ...
  /// **唯一の例外**：抑止された実行では SyncCompleted だけが流れる（SyncStarted は流れない。§8 #64）
...
**`unavailable` になるのは「アプリを起動してから一度も件数が分からない」ときに限る**（S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06）。ここで言う「一度も」は**受動的な失敗**（利用者の操作を伴わずに Stream がエラー終了した、など）についての限定で、一度でも件数を得た後に読み出せなくなったときは ST-17 にせず、直前に表示していた内容（一覧または ST-12 の空表示）をそのまま保ち、利用者への知らせも出さない（一度見えていた一覧を全面のエラー表示に置き換えないため）。本書はこれを次の 2 つで満たす（§8 #63）：(1) `resolveArticleCount` の分岐 1 が `hasValue: true`（`AsyncError` / `AsyncLoading` が直前値を保持している場合を含む）を**最優先**で拾い、件数の値がある限り `unavailable` を返さない。(2) `articleCountProvider` が `keepAlive: true` で、アプリの生存中は状態（＝一度得た件数）を捨てない。下部タブでホームを離れて戻っても再購読は起きず、「起動してから一度も」の単位と一致する。

**例外は「利用者が明示的に押した再試行」（S-00/A-12・S-01/A-08・S-02/A-05）だけ**：この 3 つでは `ref.invalidate` で直前値ごと捨てて読み直す（下の「全面表示からの復帰」）。押した時点の画面はいずれも全面表示（ST-14・ST-16・ST-17）で**一覧は出ていない**ため、上の限定が守ろうとしている「見えていた一覧を消さない」とは衝突しない。この例外が無いと、**件数 Stream が値を得た後にエラー終了した端末では再購読の契機がどこにも無くなり**、ST-14 / ST-16 で再試行して取得に成功しても（件数は直前値の 0 のまま）ST-12「記事がありません」から抜けられず、回復手段がアプリの再起動だけになる（§6・§8 #63）。
...
ArticleCountState resolveArticleCount(AsyncValue<int> count) => switch (count) {
      ...
      // 2. 値がなく読み出しが飛行中 = 初回購読中、または A-12 の ref.invalidate による再購読中。
      //    後者は S-01/ST-02（ST-19 からの再試行の実行中は全面ローディング）にあたる（S-01 §6 A-08）
...
FeedStatus feedStatus(Ref ref) {
  ...
  return FeedStatus(
    count: resolveArticleCount(count),
...
|---|---|---|---|
| ST-14（取得の失敗 × 記事 0 件） | `error` | `SyncController.sync(SyncTrigger.retry)` **と** `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）の両方 | S-00 §6 A-12（移り先に ST-17 を含む） |
| ST-16（オフライン × 記事 0 件） | `offline` | 同上 | 同上 |
| ST-17（記事を読み出せない） | `unavailable` | `ref.invalidate(articleCountProvider)`（と一覧 Stream の Provider）**だけ**。`sync()` は呼ばない | S-00 §6 A-12「取得は行わない」、S-01 §6 A-08、S-02 §8 #1 |

**2 系統の違いは `sync(retry)` を伴うかどうかだけで、`ref.invalidate` はどちらも行う。** ST-14 / ST-16 でも invalidate する理由：(1) **これが無いと回復できない経路がある**。記事 0 件の端末で件数 Stream が「0 を 1 度 emit した後」にエラー終了すると、`resolveArticleCount` は分岐 1（`hasValue: true`）で `empty` を返し続け（自動リトライは切ってあり（§8 #61）、`keepAlive: true` で破棄もされない）、`sync(retry)` だけで取得に成功しても件数は `empty` のままで ST-12 から抜けられない（§6 の該当行）。(2) S-00 §6 A-12 は ST-14 / ST-16 からの再試行の**移り先に ST-17 を挙げている**。invalidate すれば値が消え、読み直しに失敗したときに ST-17（S-01 では ST-19）へ移れて画面定義書と一致する。(3) 取得のやり直しに失敗しても、読み出しが成功していれば一覧（または ST-12・ST-14・ST-16）へ通常どおり移る。§8 #63 の「直前値を保つ」限定は**受動的な失敗**についての規則で、利用者が明示的に押した A-12 / A-08 / A-05 はその例外（押した時点で画面に一覧は出ていない）。

`unavailable` は `articleCountProvider` が**値を 1 度も得ないままエラーで終了**した場合（起動直後の DB の一時ロック・migration の一過性失敗・破損）に成立し、`lastResult` が何であっても判定表の先頭行で拾われる（S-00 §8 #36「読み出せないときは ST-12 にしない」）。**記事が表示できていた後の I/O エラーはここに入らない**（上記 (1)(2)。§8 #63）。この「直前値を残す」前提は Riverpod が `AsyncError` / `AsyncLoading` に直前値を引き継ぐ実装に依存するため、**シミュレータで「一覧が出た後に件数の読み出しを失敗させても一覧が残り、全面の E-24 に置き換わらない」ことを目視で確認する**（§10 T-F1 の完了条件）。**この状態から抜ける経路は Provider の再購読だけ**で、再購読の実行中は `resolveArticleCount` が `pending` を返すため ST-10（E-20）になり（S-01 §5.1 ST-02・§8 #31）、失敗すれば再び `unavailable` に戻って何度でも再試行できる。再購読が無いと、DB が正常に戻ってもアプリを再起動するまで全面表示が一覧を覆い続ける（要件 §5「オフライン時：取得済みの記事は閲覧可能」に反する）。

**再購読が成功して 0 件だったとき**の移り先は、判定表の `empty` の 4 行がそのまま S-01 §8 #31 の分岐（取得成功・未実行 → ST-12／オフライン → ST-16／エラー → ST-14／取得継続中 → ST-10）になる。ここで `lastResult == null`（取得を一度も実行していない）は `launchPending` の行に落ちて ST-10 になる。**この組み合わせは起動直後の 1 フレームに限らず、`sync(launch)` が完了するまで（最長 `overallTimeout` = 20 秒。§5.3）成立する**：起動直後に件数の読み出しが失敗して ST-17 になり、`launch` が飛行中のうちに A-12 を押して再購読に成功し 0 件だった場合がこれにあたる。表示は ST-10（E-20）で、S-01 §8 #31 の「読み出しに成功した時点で取得がまだ続いている → ST-02」と一致する（S-00 §8 #4・S-01 §5.1 ST-02 も ST-10 を求める）。**したがって 4 行目（`launchPending`）は到達可能で、テストから外さない**（§7）。
...
| **件数 Stream が値（0 件）を得た後にエラー終了し、その後の再試行で取得に成功した**（記事 0 件の端末で ST-14 / ST-16 が出ている状態） | A-12 が `sync(retry)` **と** `ref.invalidate(articleCountProvider)` の両方を行うため、取得で入った記事が件数の読み直しで反映され一覧に戻る。invalidate しないと `resolveArticleCount` が直前値（0）を返し続けて `empty` のままになり、**ST-12「記事がありません」から抜けられず回復手段がアプリの再起動だけ**になる（§5.4「全面表示からの復帰」・§8 #63）。読み直しに失敗すれば ST-17（S-01/ST-19）へ移る（S-00 §6 A-12 の移り先） | presentation / D-05 |
...
| 記事件数 Stream が**値を 1 度も得る前に**エラーで終了した（起動時の DB ロック・migration の一過性失敗） | `resolveArticleCount` が `unavailable` → ST-17（E-24「記事を表示できませんでした」）。E-24 の再試行（S-00/A-12）は **`ref.invalidate(articleCountProvider)` だけ**を行う（`sync()` は呼ばない。S-00 §6 A-12）。再購読の実行中は `pending` → ST-10（E-20。S-01 §5.1 ST-02）、DB が正常に戻っていれば件数が届いて一覧に戻り、再び失敗すれば ST-17 に戻る（何度でも再試行できる）。**再購読が無いとアプリを再起動するまで ST-17 から抜けられない**（§5.4。§8.1 の D-05 宛） | presentation / D-05 |
...
| A-12 の再購読に成功し、記事が 0 件だった | 判定表の `empty` の 4 行がそのまま S-01 §8 #31 の分岐になる（取得成功 → ST-12／オフライン → ST-16／エラー → ST-14／取得継続中 → ST-10）。「取得を一度も実行していない」（`lastResult == null`）は `sync(launch)` の完了まで（最長 20 秒）成立しうるので、`launch` の飛行中に A-12 を押した場合はこの行で ST-10（E-20）になる（S-01 §8 #31「取得がまだ続いている → ST-02」と一致。§5.4） | presentation（`resolveListStatus`） |
...
| `resolveArticleCount`（`articles/presentation/list_status.dart`） | `AsyncData(0)` → `empty`／`AsyncData(3)` → `present`／`AsyncLoading`（初回）→ `pending`／`AsyncError`（値なし）→ `unavailable`（ST-17。§5.4）／**`AsyncLoading(hasError: true)` で値なし**（`ref.invalidate` による再購読中）→ `pending`（ST-10。S-01 §5.1 ST-02）／**`AsyncLoading(hasError: true)` で直前値あり** → 直前の件数で判定（`present` / `empty`。全面表示にしない）／`AsyncError` で直前値あり（`AsyncError(…).copyWithPrevious(AsyncData(3))`）→ 直前の件数で判定（**一度でも件数を得た後は `unavailable` にしない**＝ S-00 §5.2 ST-17・S-01 §5.1 ST-19 の限定。§8 #63） | `test/features/articles/presentation/list_status_test.dart` |
...
| 63 | **ST-17（`unavailable`）は「アプリの起動から一度も件数が分からない」ときに限る。** ただしこの限定は**受動的な失敗**（利用者の操作を伴わずに Stream がエラー終了した等）についての規則で、**利用者が明示的に押した再試行（S-00/A-12・S-01/A-08・S-02/A-05）は例外**とし、`ref.invalidate` で直前値ごと捨てて読み直す。**再試行の 2 系統の違いは `sync(retry)` を伴うかどうかだけで、`ref.invalidate` は ST-14 / ST-16 / ST-17 のいずれからも行う**。実現は (1) `resolveArticleCount` の分岐 1（`hasValue: true` を最優先）、(2) `articleCountProvider` の `keepAlive: true`、(3) A-12 の 2 系統がともに invalidate すること（§5.4・§2.1・§6・§7） | 直前値の有無を見ずに `hasError` で `unavailable` に落とす／件数を得た後の失敗も ST-17 にして再試行の動線を出す／**ST-14 / ST-16 からの再試行では invalidate せず `sync(retry)` だけを呼ぶ**（reopen 前の規則） | S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06 が「一度でも一覧または空表示が出た後に読み出せなくなったときは直前の表示を保ち、知らせを出さない」と限定した（一度見えていた一覧を全面のエラー表示に置き換えないため）。`hasError` で落とす案はこの限定に反し、記事 100 件を表示中の一過性の `database is locked` で一覧が消える。`keepAlive` を外すと下部タブでホームを離れて戻るたびに再購読が起き、「起動してから一度も」の単位が画面の再訪単位にずれる。**本決定で振る舞いは変わらない**（既存の分岐順と `keepAlive` がそのまま限定を満たす）ので、画面定義書の限定を根拠として明文化し、Riverpod が直前値を引き継ぐ前提の実測確認を §10 T-F1 の目視項目に足すだけに留める（画面定義書 S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06 への追随、2026-09-24。**再試行の 2 系統がともに invalidate する**ことは reopen 後・第 1 周 [M-1] で確定。司令塔の判断） |
| 64 | **対応外スキーマの抑止判定を `SyncArticlesUseCase` から切り出し、`SyncCoordinator` が `execute` を呼ぶ前に評価する**（`SyncSuppressionPolicy`。§5.2.1・§5.3）。抑止した実行では `SyncStarted` を流さず `SyncCompleted(trigger, SyncFailed(unsupportedSchema, suppressed: true))` だけを流す | UseCase の手順 0 で判定する（従来）／`ListStatus.refreshing` の式に「`lastResult` が `suppressed` なら false」を足して救う | 従来は `_start` が手順 2 で `SyncStarted` を流してから `execute` を呼び、抑止判定はその中（DB 読み。`launch` では DB open / migration を含む）だったため、**通信していないのに `inProgress == true` になり E-21（ST-11）が実時間で見えた**（S-00 §8 #31・S-01 §8 #23 の但し書きに反する。記事 0 件の端末では S-01/ST-02 の全面ローディングも一瞬出る）。`refreshing` の式で救う案は、開始時点の `lastResult` が**前回の結果**なので抑止と判別できず成立しない。判定を前倒しすれば「通信していない = `SyncStarted` を流さない」が構造で保たれ、`lastResult` と `sequence` は従来どおり更新されるので #44（記事 0 件の端末で ST-14 を出す）も保たれる。抑止判定の間もスロット（`_current`）は埋まるので同時実行の抑止（S-01 §8 #12）は変わらない（reopen 後・第 1 周 [M-3]） |
