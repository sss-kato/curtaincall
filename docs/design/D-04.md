...
| S-01 §8 #23 フォアグラウンド復帰時の取得 | `AppLifecycleSync` が「`paused` / `hidden` を経由して `resumed` に戻った」ときに `sync(foreground)` を呼ぶ（判定は純粋関数 `resolveResumeSync`）。`inactive → resumed`（通知許可ダイアログ・アプリ内ブラウザから戻った）では呼ばない。**#23 の但し書き**（対応外の `schemaVersion` を検知している間は起動・復帰で取得しない）は §5.2 手順 0 の抑止で実現し、この間は E-21・E-25 を出さない（§8 #32・#44・#59） | §5.2 / §5.3 |
...
| S-00/A-12 | 操作 | `FullErrorView.onRetry` コールバック。**再試行は 2 系統**（S-00 §6 A-12）で、D-05 が全面表示の種別（`ListStatus.full`）で分岐する：`error` / `offline`（ST-14 / ST-16）→ `SyncController.sync(SyncTrigger.retry)` だけを呼ぶ（取得のやり直し）。`unavailable`（ST-17）→ `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）だけを行い、**`sync()` を呼ばない**（端末の読み出しのやり直し。取得は行わない）。分岐の根拠と各系統の結果は §5.4 | §5.3 / §5.4 / §5.9 |
...
| S-00/ST-10 | 状態 | `ListStatus.full == FullView.loading`（判定は §5.4。起動直後で記事件数が未確定（`ArticleCountState.pending`）、`launch` の取得が未開始・実行中、および **ST-17 からの再試行（A-12）で記事件数 Stream を再購読している間**（S-01 §5.1 ST-02・§8 #31）もここ。§8 #36・#61） | §5.4 |
...
| S-01/A-08（再試行。ST-19 からの再試行を含む） | ST-07・ST-08 からは `SyncTrigger.retry`、ST-19（= S-00/ST-17）からは `articleCountProvider` の再購読のみ（`sync()` を呼ばない）。分岐は S-00/A-12 の行と §5.4 | §5.3 / §5.4 |
...
**`unavailable` になるのは「アプリを起動してから一度も件数が分からない」ときに限る**（S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06）。一度でも件数を得た後に読み出せなくなったときは ST-17 にせず、直前に表示していた内容（一覧または ST-12 の空表示）をそのまま保ち、利用者への知らせも出さない（一度見えていた一覧を全面のエラー表示に置き換えないため）。本書はこれを次の 2 つで満たす（§8 #63）：(1) `resolveArticleCount` の分岐 1 が `hasValue: true`（`AsyncError` / `AsyncLoading` が直前値を保持している場合を含む）を**最優先**で拾い、件数の値がある限り `unavailable` を返さない。(2) `articleCountProvider` が `keepAlive: true` で、アプリの生存中は状態（＝一度得た件数）を捨てない。下部タブでホームを離れて戻っても再購読は起きず、「起動してから一度も」の単位と一致する。
...
/// **自動リトライを切る**（`retry: (_, __) => null`。§8 #61）。切らないと「値なし × isLoading × hasError」が
...
/// リトライを切る（同ファイルのトップレベル。`initialNotificationTapProvider`（§5.7）と同じ書き方）
...
|---|---|---|---|
| ST-14（取得の失敗 × 記事 0 件） | `error` | `SyncController.sync(SyncTrigger.retry)` **だけ**を呼ぶ（取得のやり直し） | S-00 §6 A-12 |
| ST-16（オフライン × 記事 0 件） | `offline` | 同上 | S-00 §6 A-12 |
| ST-17（記事を読み出せない） | `unavailable` | `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）**だけ**を行う。`sync()` は呼ばない | S-00 §6 A-12「取得は行わない」、S-01 §6 A-08、S-02 §8 #1 |
...
**再購読が成功して 0 件だったとき**の移り先は、判定表の `empty` の 4 行がそのまま S-01 §8 #31 の分岐（取得成功・未実行 → ST-12／オフライン → ST-16／エラー → ST-14／取得継続中 → ST-10）になる。ここで `lastResult == null`（取得を一度も実行していない）は `launchPending` の行に落ちて ST-10 になるが、**ホームでは `sync(launch)` が必ず起動直後に走り結果を返す**（失敗でも `SyncFailed` が入る）ため、この組み合わせが成立するのは起動直後の最初のフレームだけで、再購読の成功時点では起こらない（S-01 §8 #31 の「取得を一度も実行していない → ST-04」と競合しない）。起動直後の窓については S-00 §8 #4・S-01 §5.1 ST-02 が ST-10 を求めており、判定も ST-10 で一致する。
...
- 起動直後の最初のフレームは `sync(launch)` が `initState` の post-frame callback で始まるため `inProgress == false`・`lastResult == null`・`count == pending` である。2 行目（`pending`）と 3 行目（`launchPending`）により、この間に出るのは E-20（ST-10）であり、E-22（ST-12）は一瞬も出ない
...
| 抑止により取得せず失敗を返した（`launch` / `foreground` で記録が現在の app の対応外） | `SyncFailed(unsupportedSchema, suppressed: true)`。**通信していないので E-21（ST-11）も E-25（ST-15）も出さない**（S-00 §8 #31 で確定。S-01 §8 #23 の例外）。D-05 は `sequence` の変化を受けても `suppressed == true` なら `TransientNotice` を呼ばない（§8.1）。`ListStatus.errorNotice` の計算自体は変えない（記事 0 件の端末では全面の ST-14 が必要なため `full == error` も変えない。§8 #44） | application / D-05 |
...
| A-12 の再購読に成功し、記事が 0 件だった | 判定表の `empty` の 4 行がそのまま S-01 §8 #31 の分岐になる（取得成功 → ST-12／オフライン → ST-16／エラー → ST-14／取得継続中 → ST-10）。「取得を一度も実行していない」（`lastResult == null`）は起動直後の 1 フレームにしか成立しないため、再購読の成功時点では起こらない（§5.4） | presentation（`resolveListStatus`） |
...
| 63 | **ST-17（`unavailable`）は「アプリの起動から一度も件数が分からない」ときに限る。** 一度でも件数を得た後の読み出し失敗では ST-17 にせず、直前の件数で判定して直前の表示を保ち、知らせも出さない。実現は `resolveArticleCount` の分岐 1（`hasValue: true` を最優先）と `articleCountProvider` の `keepAlive: true` の 2 つで、いずれも既存の設計のまま（§5.4・§2.1・§6・§7） | 直前値の有無を見ずに `hasError` で `unavailable` に落とす／件数を得た後の失敗も ST-17 にして再試行の動線を出す | S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06 が「一度でも一覧または空表示が出た後に読み出せなくなったときは直前の表示を保ち、知らせを出さない」と限定した（一度見えていた一覧を全面のエラー表示に置き換えないため）。`hasError` で落とす案はこの限定に反し、記事 100 件を表示中の一過性の `database is locked` で一覧が消える。`keepAlive` を外すと下部タブでホームを離れて戻るたびに再購読が起き、「起動してから一度も」の単位が画面の再訪単位にずれる。**本決定で振る舞いは変わらない**（既存の分岐順と `keepAlive` がそのまま限定を満たす）ので、画面定義書の限定を根拠として明文化し、Riverpod が直前値を引き継ぐ前提の実測確認を §10 T-F1 の目視項目に足すだけに留める（画面定義書 S-00 §5.2 ST-17・S-01 §5.1 ST-19・S-02 §5 ST-06 への追随、2026-09-24） |
...
| D-05 | **ホーム（S-01）の** S-00 §5.2 の判定は `resolveListStatus(ref.watch(feedStatusProvider), hasVisible: ...)`（引数なしの Provider。§8 #51）を使い、画面側で再実装しない。`hasVisible` は団体タブ・未読フィルタ・保存の有無で絞った後の件数 > 0。E-25（ST-15）は `syncControllerProvider` の `sequence` の変化を `ref.listen` し、**その時点の判定結果（`resolveListStatus` の戻り値）の `errorNotice` が true** かつホームが表示中（`tabControllerProvider.index == RootTab.home.index` かつ `ModalRoute.isCurrent`）のときだけ `TransientNotice` を 3 秒出す。`errorNotice` が true になるのは判定表の下 2 行（`count == ArticleCountState.present`）だけなので、この条件は「**取得済みの記事が 1 件以上あって一覧（または ST-12 の空表示）が出ている**」を含む（S-01 §8 #30）。`lastResult is SyncFailed` だけで判定すると、取得済みが 0 件で全面が ST-14 / ST-16 / ST-10 のときにも帯が重なる（S-00 §5.2 の排他規則に反する）。表示中でなければ捨てる（S-00 §8 #25）。**次の 2 つでも出さない（どちらも捨てる。後から遡って出さない）**：(1) その時点の `ListStatus.full == FullView.unavailable`（ST-17 の間。判定表の先頭行が `errorNotice` を false にするため上の条件で自動的に満たされるが、規則として明示する。S-01 §8 #30 (c)）、(2) `lastResult` が `SyncFailed(suppressed: true)`（取得そのものを行っていない。`errorNotice` の計算自体は変えないため、**これだけは `errorNotice` が true でも出さない**。S-00 §8 #31・S-01 §8 #23。§8 #44・#59）。**index の数値リテラル（`0`）を書かない**（§5.8「index の数値をコードに散らさない」） | §5.4、§5.8 |
| D-05 | **`FullErrorView.onRetry`（S-00/A-12）は `ListStatus.full` で 2 系統に分岐する**（§5.4 の表）：`error` / `offline`（ST-14 / ST-16）は `sync(SyncTrigger.retry)` **だけ**、`unavailable`（ST-17）は `ref.invalidate(articleCountProvider)`（D-05 が一覧の Stream Provider を持つならそれも invalidate）**だけ**で、**`sync()` を呼ばない**（S-00 §6 A-12「取得は行わない」、S-01 §6 A-08）。ST-17 で再購読しないと、DB が正常に戻ってもアプリを再起動するまで一覧に戻れない。逆に ST-14 / ST-16 で invalidate しても害は無いが、S-02/A-05（取得を伴わない）と同じく「再試行の意味を発生元で分ける」規則を守るため呼ばない。保存画面（S-02/A-05）は `savedArticlesProvider` の再購読のみ（§5.4・§6） | §5.4、§6 |
...
| CLAUDE.md（司令塔。**未反映**。機械的な差し替え 1 語） | 「テスト方針」の純粋関数の列挙にある **`resolveHasArticles` を `resolveArticleCount` に差し替える**（§8 #61 で戻り値が `bool?` から `ArticleCountState` に変わったための改名。対象・件数（12 件）・置き場は変わらない）。T-23 の `shouldSyncOnResume` → `resolveResumeSync` と同じ扱いで、司令塔が本書の承認時に 1 語だけ更新する | §5.4、§7、§8 #61 |
...
**該当なし。** Q-1〜Q-6 は開発者の回答（2026-09-24）を受けてすべて §8 に移した（対応：Q-1 → #55、Q-2 → #56、Q-3 → #57、Q-4 → #58、Q-5 → #59、Q-6 → #60）。初稿の Q-1〜Q-6 は 2026-09-19 の回答で §8 #24〜#29 に移してある。

本書の反映にあたり新たに開発者の判断を要する事項は生じていない（§8 #61・#62・#63 は画面定義書 S-00 §8 #36・#38・§5.2 ST-17、S-01 §8 #30・#31・§5.1 ST-04・ST-19、S-02 §5 ST-06 の決定に追随して導けるもので、判断の余地が無い）。

## 10. タスク分割の目安

**本節での「品質ゲート」は `flutter analyze && dart analyze --fatal-infos && flutter test` の 3 コマンドを指す**（`riverpod_lint` は analyzer プラグインなので `flutter analyze` だけでは実行されない。§4.8。CLAUDE.md の「品質ゲート」も同じ 3 コマンドに更新済み。§8 #60）。**本節のタスク候補 ID（T-A〜T-G・T-P）は `/pm plan` 用の ID で、実タスク（`docs/tasks/T-nn.md`）とは別系統**（本文中の `T-17`・`T-23` のような `T-nn` は実タスクを指す。対応は `docs/tasks/README.md` が持つ）。**`T-H`〜`T-O` は D-05 §10 が使う**ため本書では欠番にし、本書の片付けタスクは D-05 の全タスク完了後に行うので末尾の **`T-P`** とする（D-05 §10 前文・§1「決めること」9 が「D-04 の T-F1・T-F2a・T-F2b の後に続く T-H〜T-O」と定めており、**D-05 §10 の表が実際に T-H1〜T-O を使っているので本書が譲る**。両書とも review 中だが、ID の割り当ては先に確定した D-05 側に合わせる）。
