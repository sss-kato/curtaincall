...
- **不変条件**（手順ではなく、実装が常に満たすべき性質。どちらも `dart analyze` では検出できないため設計書で固定する）：
  - **I-1：手順 1〜2 は `run()` の同期部分（最初の `await` より前）で行う。** `run()` を `async` にして先頭で `await` を挟んだり、`_current` への代入を `execute()` の完了後に回したりしない。`_current`（と `_pending`）が `run()` の呼び出しと同じマイクロタスクで立つことが、§5.6.3 の「重ならない保証」の**規則 2** の前提になっている
...
- **実行中（未完了）に `resyncIfNeeded()` を 2 回呼び、その実行を `failed` ありで完了させる** → `execute` の追加呼び出しは **1 回だけ**（2 本目は手順 2 で `_pending != null` を見て降りる）。2 本の `resyncIfNeeded()` はどちらも正常に完了する（短い間隔で背景化 → 復帰を 2 回行った場合。§5.6.3「重ならない保証」・§6）
