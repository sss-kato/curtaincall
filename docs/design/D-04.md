...
scope: app
features: [F-05, F-06, F-07, F-08, F-09, F-10, F-11]
risks: [R-6]
depends_on: [D-01]
...
| S-03/A-01（通知トグル）・§7.1（既定 ON） | `SettingsRepository.setNotificationEnabled`・`SyncPushSubscriptionsUseCase` | §4.4 / §5.6 |
...
| S-03/A-07（設定アプリへ） | `AppLifecycleSync` が復帰時に `pushPermissionStatusProvider` を invalidate | §5.3 |
...
flowchart TB
  ...
  subgraph APPL["features/*/application"]
    UC["SyncArticlesUseCase / SyncCoordinator / SyncPushSubscriptionsUseCase / RequestNotificationPermissionUseCase"]
  end
...
app/
    ...
    features/
      articles/
        ...
        application/
          ...
          sync_coordinator.dart         SyncCoordinator（同時実行の抑止・全体タイムアウト。§5.3）
  ...
  test/
    ...
    features/
      ...
      articles/application/sync_coordinator_test.dart      同時実行の抑止・全体タイムアウト・notificationTap の後追い（§7）
...
| `SocketException` | `FeedOfflineException`（DNS 解決失敗・接続拒否・機内モード。§8 #7）。`package:http` の `IOClient` は `SocketException` を `ClientException` に包むが、その例外型は `SocketException` を `implements` しているため `on SocketException` で先に捕捉できる |
...
| `schemaVersion != 1` | `FeedErrorException(FeedFailureReason.unsupportedSchema, schemaVersion: <配信の値>)`（D-01 §4.6）。`schemaVersion` が整数でなければ `malformed` |
...
abstract final class SettingKeys {
  static const String feedEtag = 'feed_etag';                     // §4.3。ArticleRepository の実装のみが読み書きする。SettingsRepository には get / set を追加しない（§8.1）
  static const String feedUnsupportedSchema = 'feed_unsupported_schema'; // §5.2 手順 0。同上（ArticleRepository の実装のみ）
...
| `FcmPushGateway`（フェーズ 5。§10 T-G） | `firebase_messaging`：`getNotificationSettings().authorizationStatus`（`provisional` は `authorized` 扱い）／`requestPermission(alert: true, badge: false, sound: true)`／`subscribeToTopic` / `unsubscribeFromTopic`／`getInitialMessage()`／`onMessageOpenedApp`。どちらも `parseNotificationTap(message.data)` で `NotificationTap` に写す（写像の規則は domain の純粋関数に閉じ、`FcmPushGateway` は呼ぶだけ。§7）。フォアグラウンド受信時も通知を表示する（`setForegroundNotificationPresentationOptions(alert: true, badge: false, sound: true)`。S-01/ST-15 はフォアグラウンドでも同じ動作） |
...
@Riverpod(keepAlive: true)
SyncCoordinator syncCoordinator(Ref ref) => SyncCoordinator(
      useCase: ref.watch(syncArticlesUseCaseProvider),
...
- **入力**：`SyncTrigger`（契機。手順 0 の抑止判定と `forceReload` の決定に使う）
- **依存**：`ArticlesFeed`、`ArticleRepository`
- **ステップ**：
  0. **対応外スキーマの抑止**（§8 #32）：`trigger` が `launch` または `foreground` で、`await articles.unsupportedSchemaVersion()` が非 null なら、取得せずに `SyncFailed(SyncFailureReason.unsupportedSchema)` を返す（毎回 200 KB を取り直さない）。`pullToRefresh`・`retry`・`notificationTap` は抑止せず取得に進む（ユーザーの明示操作は常に試す）
  1. `etag = await articles.feedEtag()`
  2. `result = await feed.fetch(etag: etag, forceReload: trigger == SyncTrigger.notificationTap)`（§8 #31）。`FeedOfflineException` → `SyncOffline` を返す。`FeedErrorException(reason)` → `SyncFailed(reason)` を返す（DB の記事には触れない）。ただし `reason == unsupportedSchema` のときは `articles.setUnsupportedSchemaVersion(e.schemaVersion)` を先に行う（同じバージョンを次回の `launch` / `foreground` で抑止するため）
  ...
  7. `plan = FeedApplyPlan(inserts, updates, refreshes, etag: fetched.etag)` を `articles.applyFeed(plan)` に渡す。`DriftArticleRepository.applyFeed` は次を **1 トランザクション**で行う：
     ...
     6. `settings` の `feed_etag` を `plan.etag` で upsert（null なら行を削除）。`feed_unsupported_schema` の行を削除する（対応するスキーマを受け取れたので抑止を解く）
...
- **出力**：`SyncResult`
- **失敗時**：手順 2 の分類どおり `SyncOffline` / `SyncFailed`。手順 7 の例外（drift のエラー）はトランザクションが戻された上で `SyncFailed(SyncFailureReason.storage)`。手順 0・1・2 の `setUnsupportedSchemaVersion` で drift が例外を投げた場合も `SyncFailed(storage)`。UseCase は例外を投げない（`SyncController` が結果型を状態に写すだけで済む。§8 #10）
...
責務を 3 つに分ける（§8 #33）。**同時実行の抑止と全体タイムアウト**は Provider に触れない application のクラス `SyncCoordinator` が持ち（UseCase テストと同じ方法で検証できる）、`SyncController` は結果を `SyncStatus` に写すだけ、`AppLifecycleSync` は契機の起動順序だけを持つ。

```dart
// lib/features/articles/application/sync_coordinator.dart
/// SyncArticlesUseCase の実行を 1 つに絞る（S-01 §8 #12）。Riverpod・Flutter に依存しない。
/// SyncArticlesUseCase は final にせず、テストでは継承したスタブ（execute を Completer で制御）を渡す（§7）
final class SyncCoordinator {
  SyncCoordinator({required SyncArticlesUseCase useCase, required Duration overallTimeout});

  /// 実行中でなければ useCase.execute(trigger) を始めて返す。
  ...
  /// 各実行には overallTimeout（feedTimeout + 5 秒）を掛け、超えたら SyncFailed(SyncFailureReason.timeout) で完了させる。
  /// useCase が例外を投げた場合（規約上は投げない）は SyncFailed(SyncFailureReason.storage) に写す。例外は再スローしない
...
- **入力**：`SyncTrigger`
- **ステップ**（`run`）：
  1. 実行中の Future があり `trigger != notificationTap` → それを返す
  2. 実行中の Future があり `trigger == notificationTap` → 実行中の契機が `notificationTap` ならそれを返す。そうでなければ「実行中の完了 → `run(notificationTap)`」を連結した Future を予約して返す（予約は 1 つまで。2 つ目以降は予約済みの Future を返す）
  3. 実行中でなければ `useCase.execute(trigger)` に `.timeout(overallTimeout, onTimeout: () => const SyncFailed(SyncFailureReason.timeout))` を掛け、`catchError` で `SyncFailed(storage)` に写し、完了時に実行中の Future をクリアする（`whenComplete`）
- **出力**：`SyncResult`
...
class SyncController extends _$SyncController {
  ...
  /// SyncCoordinator.run の結果を SyncStatus に写す。抑止・タイムアウトは Coordinator が行う
  Future<SyncResult> sync(SyncTrigger trigger) async {
    final coordinator = ref.read(syncCoordinatorProvider);
    state = state.copyWith(inProgress: true);
    final result = await coordinator.run(trigger);
    if (!coordinator.isRunning) {
      state = SyncStatus(inProgress: false, lastResult: result, sequence: state.sequence + 1);
    }
    // ログ：SyncFailed / SyncOffline は logger.w、成功は logger.i（契機・件数）
    return result;
  }
}
```

`sync` は同時に複数回呼ばれても `SyncCoordinator` が 1 つの実行に絞るため、`inProgress` は「Coordinator が実行中か」と一致する（後追いの `notificationTap` が予約されている間も `isRunning == true` で `inProgress` は下りない）。`SyncController` は Provider の配線と状態の写しだけを持つためテストしない（§7）。
...
PushSubscriptionSyncResult? _lastSubscription; // 直近の購読同期の結果。failed があれば復帰時に再実行（§8 #37）
...
Future<void> _syncSubscriptions() async {
  _lastSubscription = await ref.read(syncPushSubscriptionsUseCaseProvider).execute();  // 例外は投げない（§5.6）
}
...
void didChangeAppLifecycleState(AppLifecycleState next) {
  ...
  if (_lastSubscription?.hasFailure ?? false) unawaited(_syncSubscriptions());          // 3. 直近の購読同期に failed があれば再実行（§8 #37）
}
```

`_onLaunch` の 3 手順は逐次（2 を await してから 3）で、1 だけを await しない。復帰時の 3 手順はすべて await せず独立に始める（互いに依存しない）。D-05 の通知トグル（S-03/A-01）による購読同期は `AppLifecycleSync` を経由せず D-05 が直接呼ぶため、その結果は `_lastSubscription` に反映されない（トグル操作で失敗した団体は次の復帰では再実行されず、次のトグル操作か次回起動で同期される。§8 #37）。
...
final class FeedStatus {
  const FeedStatus({required this.hasArticles, required this.inProgress, required this.lastResult, required this.launchPending});
  final bool? hasArticles;
  final bool inProgress;
  final SyncResult? lastResult;
  /// 起動時の取得（launch）がまだ完了していない = lastResult == null かつ配置画面が取得を伴う（§8 #36）。
  /// 取得を伴わない画面（S-02）は false を渡す
  final bool launchPending;
  // == / hashCode は 4 フィールドで定義（§8 #24）
}

/// [feed] は feedStatusProvider（hasArticles・inProgress・lastResult・launchPending）
ListStatus resolveListStatus(FeedStatus feed, {required bool hasVisible});

/// [syncsOnLaunch] = 配置画面が取得を伴う（S-01 は true、S-02 は false）
@Riverpod(keepAlive: true)
FeedStatus feedStatus(Ref ref, {required bool syncsOnLaunch}) {
  ...
  return FeedStatus(
    ...
    launchPending: syncsOnLaunch && sync.lastResult == null,
  );
}
...
| false | false | null / `SyncSucceeded` | false | — | `empty` | false | false | false | ST-12（取得成功で 0 件、または取得を伴わない画面 = S-02 で `lastResult == null`） |
...
- 起動直後の最初のフレームは `sync(launch)` が `initState` の post-frame callback で始まるため `inProgress == false`・`lastResult == null`・`hasArticles == null` である。1 行目（未確定）と 2 行目（`launchPending`）により、この間に出るのは E-20（ST-10）であり、E-22（ST-12）は一瞬も出ない。`lastResult == null` で `empty` になるのは取得を伴わない画面（S-02。`launchPending = false`）だけ
...
| `formatPublishedDate` | `core/ui/article/published_date_format.dart` | `DateTime local`、`DateTime now` | **S-00 §7.3 の書式に従う純粋関数**（書式を本書に転記しない）。暦日の比較は `local` と `now` の年月日で行う。未来日（`local` が `now` より後の暦日）は特別扱いせず、同年なら `M/d`、それ以外は `yyyy/M/d`（「明日」は出さない）。時刻は出さない |
...
| ケース | 期待する振る舞い | 担当する層 |
|---|---|---|
| 機内モード・DNS 解決失敗・接続拒否（`SocketException`、および `SocketException` を `implements` する `http.ClientException`） | `SyncOffline`。DB に触れない。取得済みの記事があれば ST-13、無ければ ST-16 | infrastructure（`HttpArticlesFeed`）→ application |
...
| `schemaVersion: 2`（対応外） | `SyncFailed(unsupportedSchema)`。端末内の記事・既読・保存を消さない（D-01 §4.6）。`settings.feed_unsupported_schema = '2'` を記録し、同じ値を記録中は `launch` / `foreground` の取得を行わない（毎回 200 KB を取り直さない。`feed_etag` は更新されないため 304 にならない）。`pullToRefresh` / `retry` / `notificationTap` は常に取得する。対応する `schemaVersion` を受け取れた時点（`applyFeed` 成功）で行を削除する。ST-15 が 3 秒出るだけでアプリ更新の必要が伝わらない点は S-00 の所管として §8.1 に申し送る（§8 #32） | infrastructure → application |
...
| 未来の `publishedAt`（サイト側の日付誤り） | 「明日」等は出さず、同年なら `M/d`、それ以外は `yyyy/M/d`（§5.9） | presentation（`formatPublishedDate`） |
...
テストの単位は UseCase（CLAUDE.md）。Widget テスト・個別クラスのユニットテストは書かない（例外は本節末尾の純粋関数 6 つだけ。§8 #28）。`SyncCoordinator`（application）は UseCase を包む application のクラスなので UseCase と同じ扱いでテストする（`SyncArticlesUseCase` をスタブに差し替え、`Future` の完了を手動で制御する）。差し替えるのは外部 I/O（HTTP・FCM・アセット）で、drift は `NativeDatabase.memory()` の**実物**を使う（`DriftArticleRepository` の SQL、特に保存済み記事の保護とトランザクションを UseCase 経由で検証するため）。HTTP は `package:http` の `MockClient` で `HttpArticlesFeed` を実物のまま通し、`ArticlesFileCodec` の検証（D-01 §7.3「読み取り（D-04）」）も UseCase から到達させる。ネットワークへは一切アクセスしない。`Firebase`・`FcmPushGateway`・`bootstrap`・`SyncController`・Widget はテストしない。
...
| `SyncCoordinator` | **同時実行の抑止**：実行中に `run(pullToRefresh)` を 2 回呼ぶと UseCase の `execute` は 1 回だけ呼ばれ、3 つの Future が同じ結果で完了する／完了後に呼ぶと再び `execute` が呼ばれる／**全体タイムアウト**：`execute` が完了しないと `overallTimeout` 経過後に `SyncFailed(timeout)` で完了し、その後は新しい `run` が実行できる（`FakeAsync`）／`execute` が例外を投げると `SyncFailed(storage)` で完了し再スローしない／**通知タップの後追い（§8 #31）**：`launch` 実行中に `run(notificationTap)` を呼ぶと、`launch` の完了後に `execute(notificationTap)` が 1 回呼ばれ、その結果で完了する／実行中にさらに `run(notificationTap)` を呼んでも後追いは 1 回のまま／`notificationTap` 実行中の `run(notificationTap)` は後追いせず実行中の Future を返す／`isRunning` は後追いの完了まで true | `SyncArticlesUseCase` のスタブ（`Completer` で完了を制御）、`FakeAsync` |
...
| `resolveListStatus`（`articles/presentation/list_status.dart`） | §5.4 の判定表 8 行 × 4 出力（`full`・`refreshing`・`offlineBanner`・`errorNotice`）／**起動直後**：`hasArticles = null`・`inProgress = false`・`lastResult = null`・`launchPending = true` → `loading`（`empty` にならない。§8 #36）／`hasArticles = false`・`lastResult = null`・`launchPending = false`（S-02）→ `empty`／`hasVisible = false` で `FullView.empty` になる行 | `test/features/articles/presentation/list_status_test.dart` |
| `formatPublishedDate`（`core/ui/article/published_date_format.dart`） | S-00 §7.3 の各書式（今日・昨日・同年・前年以前）／端末タイムゾーンで日付が変わる境界（UTC の同じ瞬間が `now` の日付をまたぐ場合）／年をまたぐ「昨日」（1/1 の `now` に対する 12/31）／**未来日**（翌日・翌年）は「明日」等にならず `M/d` / `yyyy/M/d`。`now` と対象日時は引数で与える | `test/core/ui/article/published_date_format_test.dart` |
...
| 7 | オフライン（S-00/ST-13・ST-16）の判定は `SocketException`（および `SocketException` を `implements` する `http.ClientException`）の捕捉で行い、`SocketException` を包まない `ClientException`（途中切断・リダイレクト上限・不正な応答行）とタイムアウトはエラー（ST-14・ST-15）に分類する。接続状態を監視するパッケージは入れない | `ClientException` を無条件にオフラインにする／`connectivity_plus` で失敗後に接続状態を照会する／タイムアウトもオフラインにする | S-00 §5.2 の区別は「ネットワーク不通」か「ネットワークはあるが失敗」かで、`dart:io` の `SocketException`（DNS 失敗・接続拒否・機内モード）がそれに対応する。`http` の `IOClient` はこれを `SocketException` を `implements` した `ClientException` に包むため `on SocketException` で捕捉できる。それ以外の `ClientException` は接続が成立した後の失敗（Pages の不正応答）で、Wi-Fi 接続中に「オフラインです」を出すのは誤り。タイムアウトは回線が細い場合にも起きるためオフラインとは言えない。接続監視は依存が増え、Wi-Fi 接続済みでインターネットに出られない状態を正しく判定できない（レビュー第 1 周 M-3） |
...
| 32 | 対応外の `schemaVersion` を検知したら `settings.feed_unsupported_schema` に値を記録し、同じ値を記録中は `launch` / `foreground` の自動取得を行わない（`pullToRefresh` / `retry` / `notificationTap` は行う）。対応する版を反映できた時点で記録を消す | 抑止せず毎回取得する／同じ値でも 1 日 1 回は取得する | 恒久化した不一致（app 更新前）では `feed_etag` が更新されないため 304 にならず、起動・復帰のたびに 200 KB を取り直す。抑止しても、ユーザーの明示操作（Pull to Refresh）と app 更新後の最初の取得（新しい app は対応版を受け取れるので記録が消える）で回復する。記録の単位を「値」にすることで、配信側が別の版に変わったときは自動で再試行される。案内文言（アプリ更新の必要）は S-00 の所管なので §8.1 に申し送る。開発者の指示により推奨案を採用（2026-09-19。レビュー第 1 周 M-4） |
| 33 | 同時実行の抑止・全体タイムアウト・通知タップの後追いは application の `SyncCoordinator`（Provider・Flutter に依存しない）に置き、`SyncController`（presentation）は結果を `SyncStatus` に写すだけにする。フォアグラウンド復帰の遷移判定は純粋関数 `shouldSyncOnResume` に切り出す | `SyncController`（Riverpod Notifier）に抑止とタイムアウトを持たせる | 「実行中なら実行中の Future を返す」「`feedTimeout + 5 秒` で諦める」「後追いは 1 回」は振る舞いの規則であり、Notifier に置くと ProviderContainer を組んだ Widget テスト相当になりテストしない宣言（§7）と矛盾する。application に置けば UseCase と同じ方法（スタブ + `FakeAsync`）で検証できる。`SyncController` は配線と写しだけになるためテスト対象外のままでよい（レビュー第 1 周 M-10） |
...
| 36 | `resolveListStatus` の入力に `hasArticles` の未確定（`null`）と `launchPending`（起動時の取得が未完了）を持ち、どちらかが成り立てば `FullView.loading`（ST-10）を返す。`lastResult == null` で `empty` になるのは取得を伴わない画面（S-02）だけ | 未確定を `hasArticles = false` に丸め、`lastResult == null` を `empty` にする | `sync(launch)` は最初のフレーム後に始まるため、最初のビルドは `inProgress = false`・`lastResult = null`・件数未確定で、丸めると判定表の `empty` 行に落ちて E-22 が一瞬出る。S-00/ST-12 の発生条件に「起動直後」は無く、ST-10 が「初回起動の直後」を担う。未確定と `launchPending` を明示すれば、最初のフレームから E-20 になり画面仕様と一致する（レビュー第 1 周 M-16） |
| 37 | `AppLifecycleSync` は直近の購読同期の結果を保持し、`failed` が 1 件以上あればフォアグラウンド復帰時に `SyncPushSubscriptionsUseCase` を再実行する | 起動時だけ同期し、失敗は次回起動に任せる／復帰のたびに毎回同期する | 機内モードで起動すると 5 団体すべて `failed` になり、iOS はアプリを終了させないため次回起動が数日〜数週間来ず、その間通知が届かない（F-07・要件 §5「ベストエフォート」の範囲を超える）。復帰のたびに同期するのは失敗が無いときに無駄で、`failed` があるときだけなら回復に必要な分に限られる（レビュー第 1 周 M-14） |
...
| D-05 | `SettingsRepository` に `browser` / `unreadFilter` の get・set・watch を追加する。キーと既定値は `SettingKeys`（§4.4）。未読フィルタは S-01 §8 #19 のとおり永続化する。**`feed_etag`・`feed_unsupported_schema` は `ArticleRepository` の実装だけが読み書きし、`SettingsRepository` に get / set を追加しない**（記事の反映と同じトランザクションで書く規則を守るため。§8 #15・#32） | §4.4 |
...
| D-05 | `feedStatusProvider` は `syncsOnLaunch` を引数に取る。ホーム（S-01）は `true`、保存（S-02）は `false` を渡す（S-02 では `lastResult == null` が ST-12 になる。§8 #36） | §5.4 |
| D-05 | `SyncFailed(unsupportedSchema)` は他の失敗と同じく ST-14 / ST-15 で表示する（本書は文言を持たない）。S-00 側で案内文言（アプリの更新が必要）が追加されたら、`lastResult` の `reason` で分岐して出す | §6、§8 #32 |
| D-05 | 通知トグル（S-03/A-01）後の `SyncPushSubscriptionsUseCase` の結果は `AppLifecycleSync` の再実行対象（§8 #37）に入らない。トグル操作で `failed` になった団体は次のトグル操作か次回起動で同期される（`AppLifecycleSync` に結果を渡す経路は作らない） | §5.3 |
| S-00（次回 reopen 時） | §5.1 ST-03 の発生条件と §7.5 の表示条件を「`SyncArticlesUseCase` が更新を検知した後に記事を開いていない」趣旨に補正する。新着は `updatedAt` があってもバッジを付けない（§8 #11、開発者確認 2026-09-19）。あわせて、対応外の `schemaVersion` を検知したとき（§8 #32）に ST-15 の 3 秒表示だけではアプリ更新の必要が伝わらないため、E-24 / E-25 に「アプリの更新が必要です」相当の変種（文言は S-00 が決める）を追加するか判断する | §5.5、§6 |
| D-01（変更不要。確認事項） | D-01 §5.2 app 判定表の「更新：両方有りで値が異なる」を、本書は「配信の `updatedAt` が端末より新しい」に厳密化して実装する（§8 #30）。値が異なるが端末より古い場合（CDN の古いエッジ）は「変化なし」。D-01 の表の表現を「新しくなった」に揃えるかは D-01 を次に触る機会に判断する。D-01 §7.3 の「差分反映」テストの意味は変わらない | §5.2 |
