...
| D-05 | **T-M（実タスク T-33）の目視に 1 項目を足す**：D-04 §10 T-F1 の「再現手順 P」＋ (2')（ST-17 のまま背景化 → 復帰で `launch`・`foreground` を 2 回失敗 → DB を読める状態に戻して E-24 を押す）を実施し、一覧へ移った直後に E-25 がさかのぼって出ないことを確認する（S-01 §8 #30 (c)）。**T-F3 では行わない**：T-F3 の完了時点ではホーム画面が T-H3 のスタブで、E-24・一覧・E-25 のいずれも描画されないため観測できない（描くのは T-M） | §10、§5.4.4 |
