...
app/
  ...
  lib/
    ...
    app/
      ...
      root_tabs.dart            S-00/E-01〜E-04。A-01・A-02・ST-20。tabControllerProvider（CupertinoTabController を公開。§5.8）
...
| `articles` の行の寿命 | 配信に含まれる間は `in_feed = true`。配信から外れたら、保存済みなら `in_feed = false` で残し、未保存なら削除する（§5.2 手順 8）。保存を解除した記事が `in_feed = false` なら、その時点で D-05 が削除する（S-02 §7.3「解除したとき」。§8.1）。`in_feed = false` の行はユーザーが保存を解除するまで残り、**件数の上限は設けない**（F-06「保存記事は自動削除の対象外」。1 件ずつの明示操作でしか増えないため実用上は数千件が上限で、`findAll()`（§5.2 手順 6）の読み込みは数十ミリ秒で収まる） |
...
`logger` パッケージ（CLAUDE.md。`print` 禁止）。`buildOverrides()`（`core/di/bootstrap_overrides.dart`。§5.1）が 1 つ生成して `loggerProvider` に override する。レベルは debug ビルドで `Level.debug`、release で `Level.warning`。domain・application は `Logger` を import しない（UseCase は結果型で失敗を返す）。ログを出すのは infrastructure（`HttpArticlesFeed`・`DriftArticleRepository`・`NoopPushGateway`・`FcmPushGateway`）と presentation（`SyncController`・`AppLifecycleSync`）。記事本文・通知本文・サムネイルの内容はログに出さない（URL・id・件数・例外の型とメッセージのみ）。presentation は `ref.read(loggerProvider)` を State 生成時（`initState`）に 1 度読んで保持する（`Logger` は keepAlive の基盤オブジェクトなので、保持した参照を `await` 後に使ってよい。§5.3）。
...
final class SyncCoordinator {
  ...
  /// 各実行には overallTimeout（feedTimeout + 5 秒）を掛け、超えたら SyncFailed(SyncFailureReason.timeout) で完了させ（events にも SyncCompleted(trigger, SyncFailed(timeout)) を 1 回流す）、その実行の世代を無効にする（§8 #39）。
...
- **入力**：`SyncTrigger`
- **内部状態**：`_current`（実行中の Future と契機。null = 実行中なし）、`_reserved`（後追い `notificationTap` の予約。`Completer<SyncResult>` 1 つまで）、`_generation`（int。実行を始めるたびに +1。タイムアウトで諦めたとき・`dispose()` でも +1）、`_timedOutGeneration`（int?。全体タイムアウトで諦めた実行の世代。その実行の `SyncFailed(timeout)` を `events` に流すために保持する。`dispose()` では変えない）、`_disposed`（bool。`dispose()` 後は `events` に流さない）
- **ステップ**（`run`）：
...
- **`_start(trigger)`**：
  ...
  3. `future = execute(trigger, isCancelled: isCancelled).timeout(overallTimeout, onTimeout: () { _generation += 1; _timedOutGeneration = gen; return const SyncFailed(SyncFailureReason.timeout); }).catchError((_) => const SyncFailed(SyncFailureReason.storage))`（`onTimeout` で `_generation` を進めることで、裏で生き続ける `execute` の `isCancelled()` が true になり、`applyFeed` の直前で破棄される。§5.2。Dart の `Future.timeout` は元の処理を中断しない。`_timedOutGeneration = gen` は、タイムアウトの結果 `SyncFailed(timeout)` 自体は手順 5 で `events` に流すため（流さないと `inProgress` が true のまま戻らない）。`.timeout()` の戻り値 `future` は 1 度しか完了しないので、裏の `execute` が後から完了しても手順 5 は再び走らない）
  4. `_current = (trigger, future)`
  5. `future.then((result) { if ((gen == _generation || gen == _timedOutGeneration) && !_disposed) events に SyncCompleted(trigger, result); _onFinished(); })`（`_disposed` は `dispose()` で true。閉じたコントローラに add しない。同様に手順 2 の `SyncStarted` も `_disposed` なら流さない。`gen == _timedOutGeneration` はタイムアウトの結果を流すための条件で、`dispose()` は `_timedOutGeneration` を変えないため破棄後は `!_disposed` で遮断される）。`_onFinished`：`_disposed` なら `_current = null` にするだけで後追いを始めない（`_reserved` は `dispose()` が既に完了させて null にしている。§8 #39）。`_disposed` でなく `_reserved` があれば **同じ同期処理の中で** `final c = _reserved; _reserved = null; _start(notificationTap).then(c.complete);` を行い（`_current` を空にせず次の実行に置き換える = スロットが空く瞬間が無い。§8 #39）、無ければ `_current = null`
  6. `future` を返す
...
- **全体タイムアウト時の通知**：`events` には `SyncCompleted(trigger, SyncFailed(timeout))` が 1 回流れる（`_timedOutGeneration`。`SyncController` の `inProgress` が false に戻り、E-21 が消える）。実行 1 回につき `SyncStarted` / `SyncCompleted` が 1 回ずつという `events` の契約はタイムアウト時も保たれる
...
class SyncController extends _$SyncController {
  ...
  void _onEvent(SyncEvent event) {
    ...
    // SyncSucceeded.staleDiscarded が非 null（後退防止で 200 を破棄した。§5.2 手順 4）のときは logger.w に
...
late final Logger logger;                     // Logger は keepAlive の基盤オブジェクトなので await 後に使ってよい（§4.9）
// 直近の購読同期の結果は PushSubscriptionCoordinator.lastResult が持つ（D-05 のトグル経路の結果も含む。§8 #37・#41）

void initState() {
  super.initState();
  logger = ref.read(loggerProvider);          // State 生成時に 1 度だけ読んで保持する（§4.9）
...
Future<void> _syncSubscriptions() async {
  try {
    ...
    logger.w('購読同期に失敗', error: e, stackTrace: s);   // run() は execute の例外（CompanyRepository.loadAll の StateError）を再スローする（§5.6「失敗時」）。Error も含めて捕捉して記録するだけ
  }
}
...
Future<void> _resyncSubscriptionsIfFailed() async {
  ...
  try {
    ...
    logger.w('購読同期に失敗', error: e, stackTrace: s);   // pending / run() は execute の例外（CompanyRepository.loadAll の StateError）で完了する（§5.6「失敗時」）。Error も含めて捕捉して記録するだけ
  }
}
```
...
final class PushSubscriptionCoordinator {
  ...
  /// execute が例外を投げた場合（規約上は投げない。CompanyRepository.loadAll の StateError のみ）は、その実行を待つ run() の Future を同じ例外で完了させる（結果型に写さない）。
...
- 通知タップでホームへ切り替える操作（S-01/ST-15）は D-05 が `CupertinoTabController.index = 0` で行えるよう、コントローラを `tabControllerProvider`（`keepAlive`。`lib/app/root_tabs.dart` に置く。§3.2）で公開する
...
- **E-14**：`formatPublishedDate(article.publishedAt.toLocal(), now)`
- **E-15**：`Image.network(thumbnail, fit: BoxFit.cover, cacheWidth: (表示幅 × MediaQuery.devicePixelRatioOf(context)).round())` を `AspectRatio(1)` + `ClipRRect` で描く。`cacheWidth` でデコード後のサイズを表示サイズに揃え、大きな OGP 画像 100 件で `ImageCache` が溢れないようにする（表示幅は S-00 §3.2 の E-15 の幅）。`loadingBuilder` / `errorBuilder` と `thumbnail == null` でプレースホルダー（ST-05・ST-06）。画像はメモリ内キャッシュ（`ImageCache`）だけで、ディスクに書くパッケージ（`cached_network_image` 等）は使わない（CLAUDE.md。§8 #16）
- **既読（ST-02）の見た目**：S-00 §7.4 のとおり（色・太さ・不透明度を本書で再定義しない）
...
| `SyncCoordinator` | `test/features/articles/application/sync_coordinator_test.dart` | `同時実行の抑止`・`全体タイムアウト`・`通知タップの後追い`・`完了通知`・`dispose` | `execute` のスタブ関数（`Completer` で完了を制御。呼び出し回数と受け取った `isCancelled` を記録）、`FakeAsync` |
| `PushSubscriptionCoordinator` | `test/features/notifications/application/push_subscription_coordinator_test.dart` | `直列化`・`失敗`・`pending` | `execute` のスタブ関数（`Completer` で完了を制御） |
| `SyncPushSubscriptionsUseCase` | `test/features/notifications/application/sync_push_subscriptions_use_case_test.dart` | `購読の同期` | `FakePushGateway`、in-memory drift（`SettingsRepository` 実物）、`AssetCompanyRepository` 実物（`rootBundle`。`TestWidgetsFlutterBinding.ensureInitialized()`） |
...
- `applyFeed` が途中で例外を投げる（`ArticleSyncRepository` を実物のラッパで包み 2 件目で投げる）→ `SyncFailed(storage)` で DB が変わらず、`feed_etag` も更新されない
