...
**ただし `countRetryRequestedProvider` はホーム（`articleCountProvider`）専用で、S-02/A-05 からは呼ばない。** 保存画面は同じ**考え方**（押した後の最初の失敗は直前値があっても `unavailable` に写す）を `saved/presentation` 側の自前のフラグで実装する（D-05。§8.1）。理由は 2 つ：(1) A-05 が再購読するのは `savedArticlesProvider` だけなので `articleCountProvider` の `AsyncValue` は変化せず、ホームの `CountRetryRequested` の `ref.listen`（変化でしか発火しない）が下ろす契機を持てない。`retryRequested` が true のまま残ると、その後にホームで**受動的な**件数エラーが起きたときに 0-b が効いて、100 件の一覧を持つホームが ST-17（E-24）に落ちる。(2) **写し方が逆**：S-02 はやり直し中も `unavailable` のまま（E-24 を出したままにし E-20 を出さない。S-02 §5 ST-06・§6 A-05）だが、ホームは再購読中を `pending`（→ ST-10）に写す。加えて `countRetryRequestedProvider` は `articles/presentation` にあり、`saved/presentation` からの import は CLAUDE.md の禁止事項にあたる。
