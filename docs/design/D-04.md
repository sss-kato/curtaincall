...
判定表（上から順に最初に一致した行を採る）。**この表はホーム（S-01）にだけ適用する**（保存画面の `content` / `empty` は D-05 が `hasVisible` から直接組み立てる。§8 #51）。**凡例**：`任意` ＝ その列を参照しない（どの値でも同じ行に一致する）。**`launchPending` は `lastResult == null` の別名**（§5.4.1 の `FeedStatus` の getter）**なので独立した列には置かず、`lastResult` の値で書く**：
...
**(3) 取得のやり直しに失敗しても、読み出しが成功していれば一覧（または ST-12・ST-14・ST-16）へ通常どおり移る**（新しい件数が届いた時点で `shouldClearRetryRequested` が true になり `retryRequested` は false に戻る）。§8 #63 の「直前値を保つ」限定は**受動的な失敗**についての規則で、利用者が明示的に押した A-12 / A-08 / A-05 はその例外（押した時点で画面に一覧は出ていない）。**復帰した後も限定は効き続ける**：一覧に戻った端末で件数 Stream が受動的にエラー終了しても、`retryRequested` は既に false なので分岐 1 が直前値を使い、一覧は覆われない。下ろし損ねるとこの限定が壊れるため、下ろす条件は `shouldClearRetryRequested` として切り出し、§7 のテーブル駆動テストで「早すぎる下ろし」「下ろし損ね」の両方を検出する。
...
| **同じ状態（ST-14 / ST-16）で A-12 を押し、件数の読み直しが再び失敗した** | 押下で `retryRequested == true` になっているため、再購読の飛行中は `pending` → ST-10（E-20）、エラーで終われば**直前値 0 が残っていても** `unavailable` → ST-17（S-01/ST-19）へ移る（S-00 §6 A-12・S-01 §6 A-08 の移り先と一致）。`ref.invalidate` 自体は直前値を捨てない（`copyWithPrevious`）ため、この移動は `retryRequested` によってのみ成立する（§5.4・§8 #63） | presentation（`resolveArticleCount`）/ D-05 |
...
| 件数 Stream の最初の値が値もエラーも返らないまま止まった（SQLite のロック待ち） | `pending` が続き E-20（ST-10）のまま。**再試行の動線が無いため ST-17 より回復手段が乏しい**が、表示は変えない。`AppLifecycleSync` が `articleCountStallLogDelay`（10 秒）後に 1 度だけ `logger.w` を残す（§5.3・§5.4）。SQLite のロック競合は既定で `SQLITE_BUSY`（＝エラー）として返るため、この状態に入るには drift / isolate 側の不具合が要る（§8 #66） | app（`AppLifecycleSync`）/ presentation |
| 取得が成功して 100 件を反映した直後、件数 Stream の更新が `SyncStatus` より遅れた（初回起動） | 判定表 3 行目（`empty` × `hasVisible = true` → `content`）で一覧を出す。E-22（ST-12）は一瞬も出ない（S-00 §8 #4。§5.4・§8 #65） | presentation（`resolveListStatus`）/ D-05 |
| 記事を取得済みの端末で `watchCount()` が一過性の I/O エラーで終了した（`database is locked` 等） | **直前の件数を使う**（`hasValue: true` を優先）。ST-17 にも全面表示にもならず一覧を覆わない（**一度一覧または空表示が出た後は直前の表示を保ち、利用者への知らせも出さない**。S-00 §5.2 ST-17 の限定・S-01 §5.1 ST-19。§8 #63）。障害は `AppLifecycleSync` の `listenManual` が 1 回だけ `logger.w` に残す（§4.9・§5.3・§5.4） | app（`AppLifecycleSync`）/ presentation |
| 件数は読み出せたが、一覧の中身を組み立てられなかった（D-05 の一覧 Stream が値を得ずにエラー。行の写像の失敗など） | D-05 が `hasVisible: false` を渡し、判定表の `present` × `hasVisible = false` の行で `FullView.empty` ＝ ST-12（S-01/ST-04）。`unavailable`（ST-17）にも全面のエラー表示にもならず、失敗そのものを知らせる表示も増やさない（S-01 §5.1 ST-04「件数と直近の取得結果によらず本状態になる」。§5.4） | presentation / D-05 |
| 記事件数 Stream が**値を 1 度も得る前に**エラーで終了した（起動時の DB ロック・migration の一過性失敗） | `resolveArticleCount` が `unavailable` → ST-17（E-24「記事を表示できませんでした」）。E-24 の再試行（S-00/A-12）は **`CountRetryRequested.retry()` だけ**を行う（`sync()` は呼ばない。S-00 §6 A-12）。再購読の実行中は `pending` → ST-10（E-20。S-01 §5.1 ST-02）、DB が正常に戻っていれば件数が届いて一覧に戻り、再び失敗すれば ST-17 に戻る（何度でも再試行できる）。**再購読が無いとアプリを再起動するまで ST-17 から抜けられない**（§5.4。§8.1 の D-05 宛）。**A-12 の後の再購読が値もエラーも返さないまま滞留した場合はログに残らない**（`_countStallTimer` は `initState` の 1 回きり。§8 #66）が、D-05 が A-12 の押下を `logger.i` に 1 行残すため、「押した直後から E-20 が続く」ことで滞留と切り分けられる（§8.1 の D-05 宛） | presentation / D-05 |
...
**純粋関数の直接テスト（例外）**：状態判定・並び順・日時書式・ライフサイクル判定・タブ再タップ判定・通知ペイロードの写像の純粋関数 10 個は UseCase を経由しないため、CLAUDE.md「テスト方針」の例外として関数ごとにテーブル駆動テストを置く（§8 #28。`shouldClearRetryRequested` の 1 件だけ CLAUDE.md へ未反映で、§8.1 に依頼を積んである）。対象は「入出力が値だけで、I/O・Provider・Widget に触れない関数」に限り、それ以外の個別クラステストは引き続き書かない。**`AsyncValue` を入力に取る 3 つ**（`resolveArticleCount`・`shouldLogCountError`・`shouldClearRetryRequested`）**は入力を `copyWithPrevious` で組み立てる**ため、Riverpod の直前値の意味論が変わればここで落ちる。一方、**`ref.invalidate` が実際に再購読を起こして通知するかは framework の挙動で、本書では検証しない**（目視の完了条件にも置かない。§8 #69・§10 T-F3）。
...
| 65 | 判定表に **`count == empty` × `hasVisible == true` → `FullView.content`** の行を 3 行目として足す（§5.4） | 足さない（従来。`empty` の行は `hasVisible` を見ない）／`lastResult` が `SyncSucceeded` で `inserted + updated > 0` のときは件数の次の値が届くまで `loading` に留める | 件数 Stream（drift の `watchCount()`）と `SyncStatus`（`SyncCompleted`）は**別経路で更新される**ため、初回起動で 100 件を反映した直後に `empty` × `SyncSucceeded` × `launchPending: false` が一瞬成立し、E-22「記事がありません」が描画されうる（S-00 §8 #4「起動直後に E-22 を出さない」に反する）。`hasVisible` を見る案は判定表の入力だけで閉じ、`SyncSucceeded` の中身（件数）に依存しないため写像が単純になる。一覧に行があるのに全面の空表示を重ねないという規則は S-00 §5.2 の排他の考え方とも一致する（reopen 後・第 1 周 [M-4]） |
...
| CLAUDE.md（司令塔。**反映済み**。2026-09-24） | 「テスト方針」の純粋関数の列挙は **`resolveArticleCount`**（`resolveHasArticles` からの改名。§8 #61）を含む 12 件に更新済みで、本書の改訂と同じ機会に差し替えられている（`shouldLogCountError`・`resolveAnchorId` も含む） | §5.4、§7、§8 #61 |
| CLAUDE.md（司令塔。**未反映**。1 語の追記） | 「テスト方針」の純粋関数の列挙（現在 12 件）に **`shouldClearRetryRequested`** を足して 13 件にする（`features/articles/presentation/list_status.dart`。`retryRequested` を下ろす条件。§8 #63 の限定が復帰後も効き続けることを守る唯一の自動テスト）。**本書 §7 の列挙は 10 個に更新済み。** 追記はこの 1 語だけで、他の文言は変えない | §5.4、§7、§8 #63 |
...
2026-09-24 の回答で解決した分はすべて §8 に移してある（対応：Q-1 → #55、Q-2 → #56、Q-3 → #57、Q-4 → #58、Q-5 → #59、Q-6 → #60。reopen 後・第 1 周で挙げた Q-1（恒久的な DB 障害からの逃げ道）は **#68**。初稿の Q-1〜Q-6 は 2026-09-19 の回答で §8 #24〜#29 へ）。そのほかの決定（§8 #61〜#67）は画面定義書 S-00 §8 #31・#36・#38・§5.2 ST-17、S-01 §5.1 ST-02・ST-04・ST-19・§8 #23・#30・#31、S-02 §5 ST-06 の決定と CLAUDE.md の規約から導けるもので、判断の余地が無い。
