...
| DB の open に失敗（破損） | 最初のクエリで例外。`SyncFailed(storage)` と `articleCountProvider` のエラー。`resolveArticleCount` が `unavailable` になり、**`lastResult` を問わず ST-17**（判定表 1 行目。S-00 §8 #36）。取得が 304 や 0 件成功で終わっていても ST-12（「記事がありません」）にはならない。自動修復（ファイル削除）はしない（既読・保存を黙って消さない）。回復は E-24 の再試行（S-00/A-12）＝ `articleCountProvider` の再購読だけ。**DB ファイルが恒久的に壊れている場合は A-12 を何度押しても毎回エラーになり ST-17 から抜けられず、回復手段はアプリの削除・再インストール（既読・保存も失われる）だけになる**。`shouldLogCountError` は「1 障害 1 行」なので 2 回目以降の失敗はログに残らないが、D-05 が A-12 を押した事実を `logger.i` に 1 行残すため、押した回数だけは切り分けの材料になる（§8.1 の D-05 宛）。設定画面からのデータ初期化は将来の逃げ道として §9 Q-1 に挙げる | infrastructure / presentation |
...
2026-09-24 の回答で解決した分はすべて §8 に移してある（対応：Q-1 → #55、Q-2 → #56、Q-3 → #57、Q-4 → #58、Q-5 → #59、Q-6 → #60。初稿の Q-1〜Q-6 は 2026-09-19 の回答で §8 #24〜#29 へ）。本改訂で新たに 1 件を挙げる。

**[Q-1] 恒久的な DB 障害からの逃げ道（設定画面からのデータ初期化）を設けるか**

- 背景：DB ファイルが恒久的に壊れると、件数の読み出しが毎回失敗して ST-17（E-24 + 再試行）から抜けられない。A-12 を何度押しても回復せず、**現在の回復手段はアプリの削除・再インストールだけ**（既読・保存も失われる）。2 回目以降の失敗はログにも残らない（`shouldLogCountError` の 1 障害 1 行。§5.4・§6）
- 選択肢：
  - **(A) 設けない（現状のまま）。** 本書・D-05 とも変更なし。再インストールで回復する旨も画面には出さない
  - (B) 設定画面（S-03）に「データの初期化」を足す。DB ファイルを削除して作り直し、既読・保存・設定を失う代わりに ST-17 から回復できる。**画面定義書 S-03 の変更（要素・操作・確認ダイアログの文言）が先に必要**で、D-05 に UseCase が 1 つ増える
  - (C) ST-17 の E-24 に「アプリを削除して入れ直すと回復します」の補足文を足す。**S-00 §4.3 E-24 の文言変更（画面定義書の変更）が必要**で、実装は文言のみ
- **推奨：(A)**。理由：DB の恒久破損は iOS では稀（ストレージ障害・強制終了の重なり）で、要件にも復旧手段の記載が無い（要件 §5・F-05・F-06 は既読・保存を失わないことを求めており、(B) はそれを自ら捨てる操作を増やす）。(C) は画面定義書の reopen を要する割に、利用者が取れる行動は (A) と同じ
- 影響範囲：(A) なら無し。(B) は S-03（reopen）・D-05（UseCase・画面）・本書 §4.2（DB の作り直し規則）。(C) は S-00（reopen）と `FullErrorView` の文言

そのほかに開発者の判断を要する事項は生じていない（§8 #61〜#67 は画面定義書 S-00 §8 #31・#36・#38・§5.2 ST-17、S-01 §5.1 ST-02・ST-04・ST-19・§8 #23・#30・#31、S-02 §5 ST-06 の決定と CLAUDE.md の規約から導けるもので、判断の余地が無い）。
