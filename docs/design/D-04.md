...
Future<void> _syncSubscriptions() async {
  try {
    ...
    logger.w('購読同期に失敗', error: e);   // run() は execute の例外を再スローする（§5.6「失敗時」）。捕捉して記録するだけ
  }
}
...
Future<void> _resyncSubscriptionsIfFailed() async {
  ...
  try {
    ...
    logger.w('購読同期に失敗', error: e);   // pending / run() は execute の例外で完了する（§5.6「失敗時」）。捕捉して記録するだけ
  }
}
```
...
| T-F2a | 共通 Widget（状態表示） | `core/ui/status/full_loading_view.dart`・`refreshing_header.dart`・`empty_view.dart`・`offline_banner.dart`・`full_error_view.dart`・`transient_notice.dart`。計 6 | `lib/core/ui/status/` | T-A | `flutter analyze` 警告ゼロ（新規テストは無し。Widget テストは書かない。§7）。`core/ui/status` が Flutter SDK 以外を import しないこと。T-B1・T-F1・T-F2b と並行可（触るディレクトリが互いに素） |
