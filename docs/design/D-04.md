...
| S-00/ST-12 | 状態 | `ListStatus.full == FullView.empty`（**記事件数を読み出せたうえで**表示対象 0 件。`ArticleCountState.empty` / `present` のときだけ成立し、読み出せないとき（`unavailable`）は ST-17 に落ちる。S-00 §8 #36。文言は配置画面 = D-05） | §5.4 |
...
`unavailable` は `articleCountProvider` が**値を 1 度も得ないままエラーで終了**した場合（DB の一時ロック・migration の一過性失敗・破損）に成立し、`lastResult` が何であっても判定表の先頭行で拾われる（S-00 §8 #36「読み出せないときは ST-12 にしない」）。**この状態から抜ける経路は Provider の再購読だけ**で、再購読の実行中は `resolveArticleCount` が `pending` を返すため ST-10（E-20）になり（S-01 §5.1 ST-02・§8 #31）、失敗すれば再び `unavailable` に戻って何度でも再試行できる。再購読が無いと、DB が正常に戻ってもアプリを再起動するまで全面表示が一覧を覆い続ける（要件 §5「オフライン時：取得済みの記事は閲覧可能」に反する）。
...
| 記事を取得済みの端末で `watchCount()` が一過性の I/O エラーで終了した（`database is locked` 等） | **直前の件数を使う**（`hasValue: true` を優先）。ST-17 にも全面表示にもならず一覧を覆わない。障害は `AppLifecycleSync` の `listenManual` が 1 回だけ `logger.w` に残す（§4.9・§5.3・§5.4） | app（`AppLifecycleSync`）/ presentation |
...
| D-05 | **ホーム（S-01）の** S-00 §5.2 の判定は `resolveListStatus(ref.watch(feedStatusProvider), hasVisible: ...)`（引数なしの Provider。§8 #51）を使い、画面側で再実装しない。`hasVisible` は団体タブ・未読フィルタ・保存の有無で絞った後の件数 > 0。E-25（ST-15）は `syncControllerProvider` の `sequence` の変化を `ref.listen` し、`lastResult is SyncFailed` かつホームが表示中（`tabControllerProvider.index == RootTab.home.index` かつ `ModalRoute.isCurrent`）のときだけ `TransientNotice` を 3 秒出す。表示中でなければ捨てる（S-00 §8 #25）。**次の 2 つでも出さない（どちらも捨てる。後から遡って出さない）**：(1) その時点の `ListStatus.full == FullView.unavailable`（ST-17 の間。S-01 §8 #30 (c)）、(2) `lastResult` が `SyncFailed(suppressed: true)`（取得そのものを行っていない。S-00 §8 #31・S-01 §8 #23。§8 #44・#59）。**index の数値リテラル（`0`）を書かない**（§5.8「index の数値をコードに散らさない」） | §5.4、§5.8 |
...
本書の反映にあたり新たに開発者の判断を要する事項は生じていない（§8 #61・#62 は画面定義書 S-00 §8 #36・#38、S-01 §8 #30・#31 の決定に追随して導けるもので、判断の余地が無い）。
