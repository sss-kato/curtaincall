...
| S-00/A-12 | 操作 | `FullErrorView.onRetry` コールバック。**再試行は 2 系統**（S-00 §6 A-12）で、D-05 が全面表示の種別（`ListStatus.full`）で分岐する：`error` / `offline`（ST-14 / ST-16）→ `SyncController.sync(SyncTrigger.retry)` **と** `markRequested()` + `ref.invalidate(articleCountProvider)` の両方（取得のやり直し + 端末の読み出しのやり直し）。`unavailable`（ST-17）→ `markRequested()` + `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）だけを行い、**`sync()` を呼ばない**（取得は行わない）。**どちらの系統も `markRequested()` + invalidate を行う**理由（`ref.invalidate` は直前値を捨てないため、明示的な再試行であることを `retryRequested` として写像に渡さないと ST-17 へ移れない）は §5.4 の表と §8 #63 | §5.3 / §5.4 / §5.9 |
...
| S-01/A-08（再試行。ST-19 からの再試行を含む） | ST-07・ST-08 からは `SyncTrigger.retry` と `articleCountProvider` の再購読（+ `markRequested()`）、ST-19（= S-00/ST-17）からは再購読のみ（+ `markRequested()`。`sync()` を呼ばない）。どちらも `retryRequested` を立てるため、読み出しに失敗すれば ST-19 へ移れる（S-01 §6 A-08 の移り先に ST-19 が含まれる。`ref.invalidate` だけでは直前値が残って移れない。§8 #63）。分岐は S-00/A-12 の行と §5.4 | §5.3 / §5.4 |
    ...
    features/
      articles/
        ...
        presentation/
          ...
          list_status.dart              FeedStatus・ArticleCountState・resolveListStatus（**ホーム専用**）・resolveArticleCount・shouldLogCountError、feedStatus / articleCount / countRetryRequested の 3 Provider（§5.4。ListStatus・FullView は core/ui/status/list_status.dart から import する。§8 #51・#61・#63）。
...
- `retryRequested` は **A-12 / A-08 の押下で true** にし、**次に件数が届いた時点（`isLoading` でも `hasError` でもない決着した `AsyncData`）で false** に戻す。**下ろす処理と `feedStatusProvider` の再計算のどちらが先に走っても表示は変わらない**（`retryRequested == true` でも決着した `AsyncData` はその値で `present` / `empty` に写すため。下のコード片の 0-c）。保持は `countRetryRequestedProvider`（下記）で、押す（`markRequested()` を呼ぶ）のは D-05（§8.1）
...
/// 判定そのものは resolveArticleCount のテーブル駆動テストが retryRequested の真偽で網羅する
@Riverpod(keepAlive: true)
class CountRetryRequested extends _$CountRetryRequested {
  @override
  bool build() {
    ref.listen(articleCountProvider, (previous, next) {
      // 決着した AsyncData（飛行中でもエラーでもない）が届いた時点で下ろす。
      // AsyncError は直前値を引き継いで hasValue: true になりうるため hasError も見る（§5.4 本文）
      if (next case AsyncValue(hasValue: true, isLoading: false, hasError: false)) {
        state = false;
      }
    });
    return false;
  ...
  /// D-05 が A-12 / A-08 の押下で呼ぶ（同じ同期処理で ref.invalidate(articleCountProvider) も行う。§8.1）
  void markRequested() => state = true;
}
...
|---|---|---|---|
| ST-14（取得の失敗 × 記事 0 件） | `error` | `SyncController.sync(SyncTrigger.retry)` **と** `markRequested()` + `ref.invalidate(articleCountProvider)`（と D-05 が持つ一覧 Stream の Provider）の両方 | S-00 §6 A-12（移り先に ST-17 を含む） |
| ST-16（オフライン × 記事 0 件） | `offline` | 同上 | 同上 |
| ST-17（記事を読み出せない） | `unavailable` | `markRequested()` + `ref.invalidate(articleCountProvider)`（と一覧 Stream の Provider）**だけ**。`sync()` は呼ばない | S-00 §6 A-12「取得は行わない」、S-01 §6 A-08、S-02 §8 #1 |

**2 系統の違いは `sync(retry)` を伴うかどうかだけで、`markRequested()` と `ref.invalidate` はどちらも行う。** ST-14 / ST-16 でもこの 2 つを行う理由：(1) **これが無いと回復できない経路がある**。記事 0 件の端末で件数 Stream が「0 を 1 度 emit した後」にエラー終了すると、`resolveArticleCount` は分岐 1（`hasValue: true`）で `empty` を返し続け（自動リトライは切ってあり（§8 #61）、`keepAlive: true` で破棄もされない）、`sync(retry)` だけで取得に成功しても件数は `empty` のままで ST-12 から抜けられない（§6 の該当行）。invalidate は**購読を張り直して新しい件数を得る唯一の手段**なので必須である。(2) S-00 §6 A-12 は ST-14 / ST-16 からの再試行の**移り先に ST-17 を挙げている**。ただし invalidate だけでは直前値が残る（上記のとおり `copyWithPrevious` が引き継ぐ）ため、`markRequested()` で `retryRequested` を立てることによって、**再購読が飛行中は `pending`（ST-10）・再び失敗したときは `unavailable`（ST-17、S-01 では ST-19）**へ移り、画面定義書の移り先の列挙と一致する。(3) 取得のやり直しに失敗しても、読み出しが成功していれば一覧（または ST-12・ST-14・ST-16）へ通常どおり移る（新しい件数が届いた時点で `retryRequested` は false に戻る）。§8 #63 の「直前値を保つ」限定は**受動的な失敗**についての規則で、利用者が明示的に押した A-12 / A-08 / A-05 はその例外（押した時点で画面に一覧は出ていない）。
...
| **件数 Stream が値（0 件）を得た後にエラー終了し、その後の再試行で取得に成功した**（記事 0 件の端末で ST-14 / ST-16 が出ている状態） | A-12 が `sync(retry)` **と** `markRequested()` + `ref.invalidate(articleCountProvider)` を行うため、取得で入った記事が件数の読み直しで反映され一覧に戻る（届いた時点で `retryRequested` は false に戻る）。invalidate しないと `resolveArticleCount` が直前値（0）を返し続けて `empty` のままになり、**ST-12「記事がありません」から抜けられず回復手段がアプリの再起動だけ**になる（§5.4「全面表示からの復帰」・§8 #63） | presentation / D-05 |
...
| 記事件数 Stream が**値を 1 度も得る前に**エラーで終了した（起動時の DB ロック・migration の一過性失敗） | `resolveArticleCount` が `unavailable` → ST-17（E-24「記事を表示できませんでした」）。E-24 の再試行（S-00/A-12）は **`markRequested()` + `ref.invalidate(articleCountProvider)` だけ**を行う（`sync()` は呼ばない。S-00 §6 A-12）。再購読の実行中は `pending` → ST-10（E-20。S-01 §5.1 ST-02）、DB が正常に戻っていれば件数が届いて一覧に戻り、再び失敗すれば ST-17 に戻る（何度でも再試行できる）。**再購読が無いとアプリを再起動するまで ST-17 から抜けられない**（§5.4。§8.1 の D-05 宛）。**A-12 の後の再購読が値もエラーも返さないまま滞留した場合はログに残らない**（`_countStallTimer` は `initState` の 1 回きり。§8 #66）が、D-05 が A-12 の押下を `logger.i` に 1 行残すため、「押した直後から E-20 が続く」ことで滞留と切り分けられる（§8.1 の D-05 宛） | presentation / D-05 |
...
| D-05 | **`FullErrorView.onRetry`（S-00/A-12）は `ListStatus.full` で 2 系統に分岐する**（§5.4 の表）：`error` / `offline`（ST-14 / ST-16）は `sync(SyncTrigger.retry)` **と** `ref.invalidate(articleCountProvider)`（D-05 が一覧の Stream Provider を持つならそれも invalidate）の**両方**、`unavailable`（ST-17）は **invalidate だけ**で `sync()` を呼ばない（S-00 §6 A-12「取得は行わない」、S-01 §6 A-08）。**2 系統の違いは `sync(retry)` を伴うかどうかだけで、invalidate はどちらも行う**：ST-14 / ST-16 で invalidate しないと、件数 Stream が値を得た後に死んだ端末では取得に成功しても `empty` のままで ST-12 から抜けられない（§6・§8 #63）。ST-17 で再購読しないと、DB が正常に戻ってもアプリを再起動するまで一覧に戻れない。**押した事実は `logger.i` に 1 行残す**（`shouldLogCountError` は同じ障害を再記録しないため、恒久的な DB 障害の切り分け材料が押下回数として残る。あわせて、A-12 後の再購読が滞留した場合は `_countStallTimer` が起動時の 1 回きりでログに残らないため、「押下ログの直後から E-20 が続く」ことが滞留の切り分け材料になる。§6・§8 #66）。保存画面（S-02/A-05）は `savedArticlesProvider` の再購読のみ（§5.4・§6） | §5.4、§6 |
| D-05 | **再試行の押下を示す `retryRequested` を立てる／下ろす責務は D-05 と本書で分ける**（§8 #63）。D-05 は S-00/A-12（→ S-01/A-08）の押下ハンドラで、`ref.invalidate(articleCountProvider)` と**同じ同期処理の中で** `ref.read(countRetryRequestedProvider.notifier).markRequested()` を呼ぶ（2 系統＝ `error` / `offline` / `unavailable` のすべてで呼ぶ）。**下ろすのは本書**（`countRetryRequestedProvider` が `articleCountProvider` を `ref.listen` し、決着した `AsyncData` が届いた時点で false に戻す。§5.4）。これが無いと `ref.invalidate` が直前値を捨てないため、ST-14 / ST-16 から A-12 を押して読み直しに再び失敗しても `empty`（ST-12）のままで、S-00 §6 A-12・S-01 §6 A-08 の移り先 ST-17 / ST-19 に到達しない。**保存画面（S-02/A-05）も同じ考え方**：押した後の最初の失敗は直前値があっても `unavailable`（E-24）に写し、やり直し中も `unavailable` のままにする（S-02 §5 ST-06・§8 #1。ST-06 は値を一度も得ていない状態なので直前値は通常存在しないが、規則を揃える） | §5.4、§8 #63 |
...
| CLAUDE.md（司令塔。**反映済み**。§8 #28） | 「テスト方針」の「Widget テスト・個別クラスのユニットテストは書かない」の直後に、純粋関数の直接テストを認める例外が 1 項目入っている（2026-09-23 確認）。列挙は本書 §7 の 9 つ（`resolveListStatus`・`resolveArticleCount`（CLAUDE.md の記載は改名前の `resolveHasArticles`。下の行で差し替えを依頼）・`shouldLogCountError`・`resolveReadDisplayState`・`compareArticles`・`formatPublishedDate`・`resolveResumeSync`・`shouldNotifyReselect`・`parseNotificationTap`）に D-05 §7 の 3 つ（`mergeSavedList`・`buildArticleCellModel`・`resolveAnchorId`）を加えた 12 件で、T-23 の改名（`shouldSyncOnResume` → `resolveResumeSync`）と T-23 で増えた 3 つも含めて最新。T-23・T-29 の dev-loop の時点で開発者の承認を得て司令塔が更新済み。**1 語の差し替え以外に本書から CLAUDE.md への依頼は無い** | §7 |
...
| CLAUDE.md（司令塔。**反映済み**。2026-09-24） | 「テスト方針」の純粋関数の列挙は **`resolveArticleCount`**（`resolveHasArticles` からの改名。§8 #61）を含む 12 件に更新済みで、本書の改訂と同じ機会に差し替えられている（`shouldLogCountError`・`resolveAnchorId` も含む）。**本書から CLAUDE.md への依頼は無い** | §5.4、§7、§8 #61 |
