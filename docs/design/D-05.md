...
final class AnchoredListController {
  ...
  Future<void> scrollToTop({required bool animated}) {
    ...
    _inFlight = completer;                                           // _run() を呼ぶ「前」に代入する（下記の理由）
    unawaited(_run(completer, animated: animated));                  // dart:async。完了・中断の出口は _run の finally（_finish）
    return completer.future;
  }

  /// 手順 1〜4。async メソッドの本体は最初の await まで同期実行され、await に到達しなければ finally も
  /// 同じ同期の区間で走る。そのため「async メソッドを呼んで戻り値を _inFlight に代入する」形にすると、
  /// await に到達せず finally へ入る経路（target == null の早期 return など。手順 3 の await を通らずに
  /// 抜ける分岐を後から足した場合を含む）で finally が代入より先に走り、_inFlight が完了済みの Future の
  /// まま残留して以後の scrollToTop() がすべて無視される。
  /// Completer を呼び出し側で先に置くこの形なら、経路に依らず（同期で finally に着いても）残留しない。
...
abstract interface class _AnchoredListTarget {
  set scrollingToTop(bool value);                  // State の _scrollingToTop を setState を使わない素の代入で書く（手順 1・4。
                                                   // attach() が initState から呼ぶため setState では assert に抵触する。下記フィールドの説明）
...
- **`_AnchoredListViewState` が持つ状態**（すべて private フィールド。これ以外の状態を持たない）：
  - `String? _anchorId`：アンカーの id。**null は「アンカー無し」**で、`items` が空のとき**だけ**この値になる。**逆（`items` が空なら `_anchorId == null`）は据え置き中には成り立たない**（下記「形態の決め方」1 がこの向きを見ない理由）
  - `int _anchorIndex`：`_anchorId` を決めた時点での `items` 内の index（`_anchorId == null` のときは `0`）。**`_anchorId` が新しい `items` に無い場合の保険**として `center` の位置を決めるときだけに使う（下記「『直前の形態を保つ』の実体」）。**アンカーが新しい `items` に残っているときはこの古い index を使わず、新しい `items` の中でのアンカーの位置を引き直す**（古い index に固定すると、アンカーより上への差し込みで `center` が別の記事に変わり表示が飛ぶ）
  - `bool _scrollingToTop`：`scrollToTop()` の開始から完了まで true（**State が持ち**、`Controller` が `_AnchoredListTarget.scrollingToTop` のセッター経由で書く。読むのは State の `didUpdateWidget` と手順 0 の再試行だけ）。**書き込みは `setState` を使わない素の代入**（`build` の出力に影響せず、かつ `attach()` が新しい State の `initState` から（進行中の要求の引き継ぎとして）このセッターを呼ぶため。`initState` は親の build フェーズ中に走るので、`setState` で書くと `markNeedsBuild` が build 中に走って `setState() or markNeedsBuild() called during build.` の assert に抵触し debug ビルドが落ちる）
  - `bool _lastFormWasAnchored`：**直前の `build()` が実際に採った形態**（アンカー形なら true）。`build()` の末尾で毎回代入する（`setState` を使わない素の代入。次の `build()` の入力にしかならず再 build を起こさない。同じ `items` で `build()` をやり直しても同じ値になる）。**「いま実際にどちらの形態で描かれているか」を知る唯一の判定材料**で、`_anchorId` や `_anchorIndex` から形態を推測しない。用途は 3 つ：据え置き中に「直前の形態を保つ」（下記「形態の決め方」2・5）、旧レイアウトの形態による `H` の経路の選択（下記「アンカーの更新」手順 3）、`ScrollController` の listener が「既に標準形か」を判定して復帰処理を飛ばすかどうか（下記「先頭に戻ったときの標準形への復帰」）
  - `bool _pendingItemsChanged` と `int _pendingFrames`：ガード 0 で据え置いた更新の再試行に使う（下記「アンカーの更新」手順 0）
  - `Completer<void>? _pendingScrollToTop`：**「次に `_laidOutPosition` が確定したフレームで `jumpTo(0)` する」要求の保持**（非 null = 保持中）。立てるのは「先頭スクロール要求」手順 3（レイアウト未確定）と手順 0 の諦めの 2 か所だけ（下記）
...
- **形態の決め方**（`build()`。**上から順に評価し、最初に該当した行で確定する**。ここに書いた 5 行以外の分岐を作らない）：
  ...
  - **「直前の形態を保つ」の実体**：`_lastFormWasAnchored == false` なら標準形。true ならアンカー形のまま、`final i = items.indexWhere((e) => idOf(e) == _anchorId);` を求めて `center` の位置を次のとおり決める。
    - **`i >= 0`（アンカーが新しい `items` に残っている）→ その `i`。** アンカー形の `pixels` は `center` の先頭からの距離なので、**`center` が同じ記事であり続ける限り `correctPixels` 無しで表示位置が完全に保たれる**。据え置いたフレームでアンカーより上に n 件差し込まれた場合、ここで古い `_anchorIndex` に固定すると `center` が n 件手前の記事に変わり、`pixels` は据え置きで動かないため表示が数百 px 飛ぶ。しかも次フレームの再試行は「据え置きの間に見た目がずれた分は戻さない」（下記「アンカーの更新」手順 0）ので、**飛んだ位置がそのまま残る**（S-01 §7.4「表示中の記事の画面上の位置を保つ」と §10 T-M の目視項目に反する）
    - **`i == -1`（アンカーが新しい `items` に無い。5 の経路と、据え置き中にアンカー自体が消えた場合）→ `_anchorIndex.clamp(0, items.length - 1)`。** 基準にする記事が残っていないため、位置の手掛かりは旧 index しかない（次フレームの再試行で実測に基づいて選び直す）。**この `_anchorIndex` は据え置きを始めた時点の index で、据え置き中にアンカーより上へ差し込まれた件数は反映されない**（`build()` から状態を書かない方針のため、据え置き中に引き直した `i` を `_anchorIndex` へ書き戻さない）。差し込みぶんだけ手前の記事が `center` になるが、**これはアンカー自体が消えたときの保険であり、次フレームの再試行が実測でアンカーを選び直して置き換える**（2 フレーム待っても確定しなければ `_resetAnchorToFirst()` + `jumpTo(0)` で先頭に倒れる。下記「アンカーの更新」手順 0）。1 により `items` は非空なので上限は 0 以上で、`clamp` は必ず成立する。上限を `items.length` にすると `center` の `SliverList` が要素 0 個になり、`maxScrollExtent == 0`・`minScrollExtent == -(全高)` の下で保持していた正の `pixels` が範囲外になって `goBallistic` が**一覧の末尾**へ整定する
  - **index 0（標準形）へ落とすのは `_resetAnchorToFirst()` だけ**で、`build()` は `correctPixels` を伴わない形態変更を起こさない（起こすと §5.11 が「位置が飛ぶ」と明記した経路そのものになる）。手順 0 で再試行を諦めるときも `_resetAnchorToFirst()` を通す（下記）
...
- **共通の手続き 4 つ**（形態を変える処理・先頭判定・フレーム後のやり直しはすべてこの 4 つを通す）：
  ...
  - **`void _resetAnchorToFirst()`**：標準形へ戻す唯一の手順。次の順で行う。
    ...
    - **据え置き（`_pendingItemsChanged`）もここで解除する。** 標準形へ倒した時点で保持すべき位置が無くなり、据え置いた更新をやり直す意味が無いため。解除しないと「形態の決め方」2 が直前のアンカー形を保ち続け、`_anchorId` が先頭なのにアンカー形で描かれて `jumpTo(0)` がアンカー位置に着く
  ...
  - **フレーム後の処理（`_scheduleFrameCallback()` / `_onPostFrame()`）**：`addPostFrameCallback` を張るのはこの 1 か所だけで、**据え置きの再試行（手順 0）と保持した先頭スクロール（手順 3）の両方を 1 本のコールバックで処理する**（待ち合わせのループを 2 か所に書かない）。回数の上限は**据え置きの再試行にだけ**掛かる（ファイル private 定数 `static const int _maxPendingFrames = 2;`。根拠「controller の差し替えは 1 フレームで解消する」（T-28 の実測）はこの定数の宣言に 1 度だけ書く）。
...
- **先頭に戻ったときの標準形への復帰**（ユーザーのスクロール）：`ScrollController` の listener で `_isAtTop()`（上記「共通の手続き」）を検知したら（ドラッグ中でも）、**既に標準形なら何もしない**（復帰処理はアンカー形のときだけ走る。標準形でスクロールのたびに `setState` しない）。アンカー形なら上記 `_resetAnchorToFirst()` を呼ぶ。**「既に標準形か」の判定は `_lastFormWasAnchored`（直前の `build()` が実際に採った形態）で行い、`_anchorId` から「形態の決め方」を再評価しない。** 据え置き中（「形態の決め方」2）は両者が一致せず、`_anchorId` が新 `items` の先頭になっていても実際はアンカー形で描かれているため、`_anchorId` で判定すると「標準形」と誤認して `_resetAnchorToFirst()` を呼ばず、`leadingSlivers` が出ていないまま「引っ張っても Pull to Refresh が始まらない」フレームが残る。以後はオーバースクロールが `CupertinoSliverRefreshControl`（標準形なので `leadingSlivers` が出ている）に届き、そのまま引き続ければ Pull to Refresh が発火する（F-10）。標準形へ戻す判定は `items` が変わらなくても走る
...
- **先頭スクロール要求**（`AnchoredListController.scrollToTop(animated:)`。S-01/A-03・A-09・A-10・ST-15、S-02/A-04。実装の形は上記コードの `_run`）：
  ...
  2. `_target.restoreToStandardForm()`：**`_resetAnchorToFirst()` を呼ぶだけ**（標準形なら `_anchorId` が既に先頭なので実質何も変わらない）。`_laidOutPosition` が null（レイアウト未確定。`PageView` の非表示ページが `requestAll()` を受けた直後など）のときは `_resetAnchorToFirst()` の手順 2 が飛ぶため、**形態の付け替えだけを行い `correctPixels` は行わない**。標準形になったことで次のレイアウトでは `minScrollExtent == 0`・`0` が先頭になるが、**`pixels` は付け替え前の値のまま残る**（先頭には着かない）ので、手順 3 が `0` へ動かす
  3. `_target.scrollToTopWhenLaidOut(animated:)`（State 側。上記インターフェース）：
     ...
     - **`_laidOutPosition` が null（レイアウト未確定）なら要求を捨てず、`_requestScrollToTopOnLayout()` で `_pendingScrollToTop` に保持して `Future` を返す**（`_scrollingToTop` は立てたまま。`scrollToTop()` の `Future` は保持が消化されるまで完了しない）。消化は上記「フレーム後の処理」が行い、**確定した最初のフレームで `_resetAnchorToFirst()` → `jumpTo(0)`**（`animated` は無視して常に `jumpTo`。旧レイアウトの実測を要さないため待つだけでよい）。**回数の上限は設けない**（据え置きの再試行（手順 0）の上限 2 フレームは、`correctPixels` に旧レイアウトの実測が要るという位置保持側の制約であって、先頭へ動かすだけの `jumpTo(0)` には掛からない）。諦めて途中の位置を残すと、S-01/A-03（§8 #5「スクロール位置は先頭に戻す」）と §8 #25 が `requestAll()` で排除したはずの「タブ切替で絞り込み後の一覧の途中が見える」状態が残るため、要求を保持し続ける方を採る
  ...
  - `scrollToTop()` の実行中にもう一度呼ばれたら、**`AnchoredListController._inFlight` に保持した進行中の `Completer` の `Future` をそのまま返す**（二重の `animateTo`・二重の保持を作らない）。State 側は「上記のフィールド以外の状態を持たない」ため、合流の判定先は `Controller` に置く（`animated` の値が前回と違っても、進行中の要求に合流する。先頭へ戻るという結果は同じ）。**`_inFlight` は `Completer` で表し、`_run()` を呼ぶ前に代入して、完了・中断の出口（`_finish()`）で「null に戻してから `complete()`」する。** `async` メソッドの戻り値を後から代入する形にすると、**`await` に到達せず `finally` へ入る経路**（`target == null` の早期 return など。上記コードでは手順 3 の `await` を通らずに抜ける分岐がこれに当たる）で `finally` が代入より先に走り、`_inFlight` が完了済みのまま残留して**以後そのページが二度と先頭に戻らない**。`Completer` を先に置く形なら**経路に依らず**（`await` の有無や同期完了かどうかに関わらず）残留しないため、この形を採る（上記コードのコメント）
- **アンカーの更新**（`didUpdateWidget` で `items` が変わったとき。**`listEquals(oldWidget.items, widget.items)` が true なら、アンカーの再計算も `correctPixels` も行わない**。星のタップで別の記事の `isSaved` が変わった・`minuteClockProvider` で親が再 build した等、表示位置に関係ない再 build で位置を触らない。`ArticleListItem`・`SavedArticle` は値の等価性を持つ。§4.1）：
  0. **`position` に触れる前のガード**：**`_laidOutPosition == null`**（`hasClients == false`、または `hasContentDimensions == false`）なら、アンカーの再計算も形態の変更も `correctPixels` も**行わず据え置く**。`State.didUpdateWidget` は `Scrollable` が新しい `ScrollController` を受け取るより**前**に走るため、controller を差し替えた直後のフレームでは新しい controller が `hasClients == false` になる。ここで標準形へ落とすと `correctPixels` を伴わない形態変更になり位置が飛ぶ（T-28 の実測で `dy` が `-20` から `+280` へずれた）。
     - **据え置きの記録**：`_pendingItemsChanged` が **false のときだけ `_pendingFrames = 0`** にしてから `_pendingItemsChanged = true` にし、`_scheduleFrameCallback()` を呼ぶ（上記「フレーム後の処理」。コールバックは同時に 1 本）。**据え置き中にもう一度据え置くときはカウンタをリセットしない。** リセットすると、`items` が短時間に続けて変わる経路（drift が `articles` / `read_states` / `saved_articles` の 3 テーブル更新で続けて再発火する）で `_pendingFrames` が 2 に達さず、下記の「諦め」が走らないまま待ち続ける
     - **再試行**：`_onPostFrame()` の据え置きの段（上記コード）が、`_laidOutPosition` が非 null のとき `_retryDeferredItemsChange()` で**手順 1〜4 をそのフレームの実測でやり直す**（同メソッドの最後に `_pendingItemsChanged = false`）。まだ null なら `_pendingFrames++` して同じコールバックを張り直す
     ...
     - **`_pendingFrames` が `_maxPendingFrames`（2）に達したら諦める**（controller の差し替えは 1 フレームで解消することを T-28 で実測しており、2 フレーム待っても確定しないページは画面に出ていない）。諦める処理は **`_resetAnchorToFirst()`**（標準形へ揃える。`_laidOutPosition` が null なので `correctPixels` は行われず**位置保持は放棄する**。据え置きもここで解ける）→ **`_requestScrollToTopOnLayout()`**（次に確定したレイアウトで `jumpTo(0)`）の 2 つだけ。揃えずに「形態の決め方」2・5 の据え置き状態を残すと、以後 `didUpdateWidget` が走らないまま `items` が縮んだときに `center` が一覧の末尾付近になり、そのページへスワイプしたユーザーに末尾が見える。さらに、標準形へ揃えただけでは `pixels` がアンカー基準の値のまま `minScrollExtent == 0` で解釈され、`items` が大きく縮んでいると `applyContentDimensions` が**末尾**に収めてしまう（同じ見え方に戻る）ため、**着地点を先頭に倒す**（`jumpTo(0)` は旧レイアウトの実測を要さないので確定さえすれば実行できる。「先頭スクロール要求」手順 3 と同じ仕組み）。諦めるのは画面に出ていないページに限られる（§6）
...
| `items` の変更と `ScrollController` の差し替えが同一フレームで起きた（ページの再構築） | `_laidOutPosition == null`（`didUpdateWidget` は `Scrollable` が新 controller を受け取る前に走るので `hasClients == false`）なのでアンカーの更新も補正も見送り、**`addPostFrameCallback` で次フレームにやり直す**（やり直しでは `resolveAnchorId` を呼ばず、実測した `visibleId` を直接アンカーにする。最大 `_maxPendingFrames`（2）フレーム待ち、確定しなければ**標準形に揃えて先頭へ倒す**。§5.11 手順 0）。`ScrollPosition` は差し替えでも再利用されるため `pixels` の起点は変わらず、アンカーはそのまま有効 | core/ui |
| 据え置いたフレームでアンカー記事自体が新 `items` から消えていた | `build()` は「形態の決め方」2（据え置き中は直前の形態を保つ）により**形態を変えずに**描く（アンカー形なら `center` を `_anchorIndex.clamp(0, items.length - 1)` に置く。上限が `items.length` だと `center` が要素 0 個になり `goBallistic` で一覧の末尾へ飛ぶ）。`correctPixels` を伴わない形態変更（index 0 = 標準形への転落）を起こさない。次フレームのやり直しで、そのときビューポートに見えている記事を基準にアンカーを選び直して補正する（§5.11 手順 0・「形態の決め方」） | core/ui |
| 据え置いたフレームで新 `items` が 0 件になった（未読フィルタ ON で最後の未読を開いて戻った・保存の全件解除が確定した回と同じフレーム） | 「形態の決め方」**1**（`items.isEmpty` を `_anchorId` より先に見る）で**標準形 + `emptyView`** になる。据え置き中は `_anchorId` が非 null のまま残るため、1 を `_anchorId == null` で書くと `_anchorIndex.clamp(0, -1)` が `ArgumentError` を投げて `build()` ごと落ち、落ちなくても `emptyView` の置き場が無く空白画面になる。位置保持は次フレームのやり直しに任せる（§5.11「形態の決め方」） | core/ui |
| 据え置いたフレームでアンカーが新 `items` の先頭になった（アンカーより上の記事がまとめて消えた） | 「形態の決め方」**2** で止まり、**形態を変えずに**描く（3 の標準形に落とさない）。据え置き中は `correctPixels` を行えないため、補正なしでアンカー形→標準形に切り替わると `pixels` の基準（負の `minScrollExtent`）が変わって位置が飛ぶ。次フレームのやり直しで補正つきに揃える（§5.11「形態の決め方」） | core/ui |
| 据え置いたフレームでアンカーより上に記事が差し込まれた（アンカーは新 `items` に残っている） | 「形態の決め方」2 がアンカー形を保ち、`center` を **`_anchorId` の新しい `items` での index** に置くので、`correctPixels` 無しで表示位置が保たれる（アンカー形の `pixels` は `center` の先頭からの距離で、`center` が同じ記事なら画面位置は変わらない）。古い `_anchorIndex` に固定すると `center` が差し込み件数ぶん手前の記事になって表示が飛び、再試行は「据え置きの間に見た目がずれた分は戻さない」ため飛んだまま残る（§5.11「『直前の形態を保つ』の実体」） | core/ui |
| 据え置きが 2 フレーム続けて起きた（drift が `articles` / `read_states` / `saved_articles` の更新で短時間に再発火） | 2 回目以降は **`_pendingFrames` をリセットしない**ので上限（2）が伸びず、諦めの処理が必ず走る。コールバックは `_frameCallbackScheduled` で同時に 1 本に保ち、冒頭で `_pendingItemsChanged` が false なら据え置きの段を飛ばす（§5.11 手順 0・「フレーム後の処理」） | core/ui |
| 据え置きの再試行を 2 フレーム待っても `_laidOutPosition` が確定しなかった | 位置保持を放棄して **`_resetAnchorToFirst()` で標準形に揃え**（`correctPixels` は行えない）、続けて **`_pendingScrollToTop` を立てて次に確定したレイアウトで `jumpTo(0)`** する。据え置きのアンカーを残すと `items` が縮んだときに `center` が末尾付近になり、標準形へ揃えただけでも `pixels` がアンカー基準のまま残って `applyContentDimensions` が末尾に収めうるため、**着地点を先頭に倒す**（§5.11 手順 0） | core/ui |
...
**`AnchoredListView` の代わりの担保**（§5.11・§10 T-H3・T-M）：`AnchoredListView`（Widget）自体は自動テストを持たない方針を維持する。**アンカーの付け替え規則だけは純粋関数 `resolveAnchorId` に切り出して直接テストする**（下記「純粋関数の直接テスト」。§8 #33）。残るレイアウト補正（`H` の優先順・`_ExtentCompensatingSliver`・2 形態の切替）は、**依存している Flutter SDK 内部の前提と、それを確認した Flutter のバージョンを `core/ui/list/anchored_list_view.dart` 冒頭の library doc に列挙して残す**ことで担保する（Flutter を上げたときの再確認の手掛かりがこれしか無いため。挙動の確認は T-M の目視項目）。初版（Flutter 3.47.4 stable で実機確認）の列挙は次の 8 項目：
...
- `SchedulerBinding.addPostFrameCallback` は**自分ではフレームを要求しない**（次に作られるフレームの後に 1 度だけ走る）。レイアウトの確定を待つ再試行（据え置きの再試行・保持した先頭スクロール）を毎フレーム張り直しても、描画が空回りしない（§5.11「フレーム後の処理」）
