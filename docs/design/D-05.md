...
| S-00/A-12 | 操作 | `FullErrorView.onRetry` を `ListStatus.full` で **2 系統**に分ける（D-04 §5.4.4 の表・§8.1）。**どちらの系統でも** `ref.read(countRetryRequestedProvider.notifier).retry()` と `ref.invalidate(homeArticlesProvider)` を行い、`error` / `offline` だけが加えて `sync(SyncTrigger.retry)` を呼ぶ（`unavailable` は取得しない） | §5.6 |
...
| S-00/ST-17 | 状態 | ホームは `FullView.unavailable`（D-04 §5.4.3 の判定表 10 行の先頭行）→ S-01/ST-19。保存画面は `savedListControllerProvider` の `AsyncError`（値なし）から `FullView.unavailable` を直接組み立てる → S-02/ST-06 | §5.1 / §5.5 |
...
| S-01/A-08 | 操作 | S-00/A-12 と同じ 2 系統。ST-07・ST-08（`offline` / `error`）から押したときは `retry()` + `homeArticlesProvider` の invalidate + `sync(SyncTrigger.retry)`、ST-19（`unavailable`）から押したときは `retry()` + `homeArticlesProvider` の invalidate だけ（取得は行わない） | §5.6 |
      ...
      articles/
        ...
        presentation/
          home_screen.dart                  S-01/E-01〜E-03・E-10（ナビゲーションバー・未読フィルタ・上部タブ・PageView・帯・全面表示）。
                                            同じ feature の list_status.dart（D-04 所管）から feedStatusProvider・resolveListStatus・
                                            countRetryRequestedProvider（A-08 の retry()）を使う（§5.1・§5.6）
...
final screenStatus = _screenStatus(
  ref.watch(feedStatusProvider),                                          // D-04 §5.4.1・§5.4.2（引数を取らない。D-04 §8 #51）
  ref.watch(homeArticlesProvider(companyId: null).select(_allTabOf)),
);

// home_screen.dart の private な関数。build と §5.6 の listener が共有する（二重実装を避ける）
typedef _AllTab = ({bool settled, bool hasVisible});
_AllTab _allTabOf(AsyncValue<HomeArticles> v) =>
    (settled: v.hasValue || v.hasError, hasVisible: v.valueOrNull?.items.isNotEmpty ?? false);

ListStatus _screenStatus(FeedStatus feed, _AllTab allTab) => allTab.settled
    ? resolveListStatus(feed, hasVisible: allTab.hasVisible)
...
**`requireValue` を読まない。** `AsyncError` で前の値が無いとき（`articles` の 1 行の `published_at` / `category` が壊れて `ArticleRowMapper` が写像で例外を投げた等）に `requireValue` は `StateError` を投げ、`HomePage.build` が例外で落ちる。このとき `articleCountProvider`（`SELECT COUNT(*)`）は成功しうるので `feed.count == ArticleCountState.present` のまま `hasVisible == false` となり、D-04 §5.4.3 判定表の最終行（10 行目）で `FullView.empty` になる（件数が読めている以上 `unavailable` にはならない。S-00 §8 #36 の ST-17 は「件数そのものが分からない」ときの状態）。文言は `totalInTab == 0` の側（S-01 §5 ST-04）を出す（§6）。分岐は `AsyncData` / `AsyncError` の状態クラスではなく `hasValue` / `hasError` のフラグで書く（D-04 §5.4.2「Riverpod 3 の自動リトライは切る」により、`articleCountProvider` 以外の Provider ではリトライ中が `AsyncLoading(hasError: true)` として現れるため）。
...
- **`HomePage` が自分で出す全面表示は `FullLoadingView`（自ページの一覧が未確定 = 上の `_` の分岐）だけ**で、`error` / `offline` / `unavailable` の全面表示は出さない（再試行 A-08 の動線は `HomeScreen` が持つ。§5.6）。`HomeScreen` と `HomePage` は同じ `feed` を使うが `hasVisible` が異なる（画面は「すべて」ページ、ページは自ページ）ため、**判定表 3 行目の過渡状態では `HomeScreen` が `content`・ある団体ページが `error` / `offline` / `loading` という食い違いが 1 フレーム起こりうる**。このとき `HomePage` は `EmptyView` を渡さずに一覧（0 件なので空の領域）をそのまま描く。次のフレームで件数 Stream が追いつけば通常の表示に戻るため、専用の分岐は持たない（§8 #38）
- `screenStatus.full == FullView.empty`（取得成功で 0 件。D-04 §5.4.3 の判定表 8 行目）のときは `HomeScreen` は `PageView` を出し、`HomePage` も `hasVisible = false`（記事が 0 件なので `items` は空）で `empty` になり、`totalInTab == 0` なので ST-04 が出る。全面表示用の `empty` Widget は持たない
...
**読み出しに失敗したときの扱い**（`SavedScreen.build`。S-02 §5 ST-06・§8 #11。§6）：`savedListControllerProvider` が `AsyncError`（値を 1 度も得ずにエラーで終わった）のときは **`FullView.unavailable`（S-00/ST-17 = S-02/ST-06）**に写し、`FullErrorView(kind: FullErrorKind.unavailable, onRetry: …)` を E-02 の領域全体に出す（文言・アイコン・色は `FullErrorView` が `kind` から決める。D-04 §5.9・§8 #62。本書は持たない）。**ST-02（空）には写さない**（保存した記事が消えたように見え、F-06 と食い違う。S-02 §8 #11）。**ログは `SavedListController` の listener（下記 `build()`）がエラーの到着ごとに 1 回だけ出すので、`SavedScreen.build` では出さない**（`build` の `switch` で出すと再 build のたびに記録される。§5.1 の `HomePage` と同じ方式）。`SizedBox.shrink()` は `AsyncLoading`（初回値が未着）のときだけで、エラーで空白が恒久化しない。**値を持ったまま後からエラーになった場合は直前の一覧を出したままにする**（`hasValue: true` の分岐が先に一致する。ホームと同じ考え方。D-04 §5.4.2）。
...
FullErrorView(
  kind: kind,
  onRetry: () {
    // 共通（どちらの系統でも行う。D-04 §5.4.4）
    _logger.i('再試行を押した（full=${screenStatus.full}）');   // 押下の事実を 1 行（D-04 §8.1・§6）
    ...
    // 系統ごと：取得を伴うのは error / offline だけ（S-01/ST-07・ST-08）
    if (screenStatus.full != FullView.unavailable) {            // S-01/ST-19 では取得しない（S-00 §6 A-12「取得は行わない」）
...
ref.listen(syncControllerProvider, (prev, next) {
  ...
  // 判定は §5.1 と同じ関数に同じ入力を与えて作る（build のローカル変数 screenStatus を閉じ込めない。
  //  listener は HomeScreen の再 build より先に呼ばれうるため、1 つ前の値で判定してしまう）
  final status = _screenStatus(
    ref.read(feedStatusProvider),
    _allTabOf(ref.read(homeArticlesProvider(companyId: null))),  // hasVisible は「すべて」ページ（§8 #38）
  );
...
| `homeArticlesProvider` が `AsyncError`（DB 破損・`ArticleRowMapper` の写像失敗） | `hasVisible = false`・**`totalInTab == 0` 扱い**で判定する（`requireValue` を読まない。読むと `StateError` で `HomePage.build` が落ちる。§5.1）。`ref.listen` でエラー到着ごとに 1 回 `logger.w`。`articleCountProvider` も失敗していれば `count == unavailable` で **ST-19（S-00/ST-17。全面 E-24 + 再試行）**（D-04 §5.4.3 の判定表の先頭行・§6「DB の open に失敗」）、**件数クエリだけ成功していれば `count == present`・`hasVisible == false` で `empty`**（S-01 §5 ST-04 の文言）になる。**再試行（S-01/A-08 = S-00/A-12）は系統を問わず `countRetryRequestedProvider.retry()`（＝ `articleCountProvider` の invalidate）と `homeArticlesProvider`（family 全ページ分）の invalidate を行って Stream を張り直す**ので、DB が正常に戻れば再起動せずに一覧へ戻れる（§5.6。D-04 §5.4.2・§5.4.4・§8.1）。件数だけ成功している経路（ST-04 と同じ表示）は S-01 §5 に対応する状態が無いため §8.1 に申し送る | presentation |
...
| 値を持った後（一覧を表示中）に `savedArticlesProvider` がエラーになった | 直前の一覧を出したままにする（`hasValue: true` の分岐が先に一致する。ホームと同じ考え方。D-04 §5.4.2）。ST-06 にはしない | presentation |
...
| ホームの再試行の 2 系統（S-00/A-12・S-01/A-08。§5.6） | 判定（`resolveListStatus`・`resolveArticleCount`・`shouldClearRetryRequested`）は D-04 §7 の直接テスト。**両系統で `retry()` と一覧の invalidate を呼び、`sync(retry)` だけを分岐させる配線**は §10 T-M の目視項目 19 |
...
| 37 | 再試行（S-01/A-08 = S-00/A-12）は**両系統で** `ref.read(countRetryRequestedProvider.notifier).retry()` と `ref.invalidate(homeArticlesProvider)` を呼び、**分岐するのは `sync(SyncTrigger.retry)` を伴うかどうかだけ**にする。押下は `logger.i` に 1 行残す。`articleCountProvider` を直接 invalidate しない | `unavailable` の系統だけ `ref.invalidate(articleCountProvider)` を呼ぶ（2026-09-24 以前の本書）／`retry()` と `ref.invalidate(articleCountProvider)` を本書で 2 行に分けて書く | **D-04 §5.4.4 が「2 系統の違いは `sync(retry)` を伴うかどうかだけ」と確定した**（D-04 §8 #63・§8.1）。旧案だと、記事 0 件の端末で件数 Stream が 0 を 1 度流した後にエラー終了した場合、ST-14 / ST-16 から取得に成功しても `resolveArticleCount` が分岐 1 で `empty` を返し続け ST-12 から抜けられない（§6）。`retry()` は「`retryRequested = true`」と「`articleCountProvider` の invalidate」を 1 メソッドにまとめた不変条件の保証（片方だけだと下ろす契機が二度と来ない）なので、本書で 2 行に分けると保証が崩れる。押下のログは `shouldLogCountError` が `hasError → hasError` を記録しないことの補いで、恒久的な DB 障害の切り分け材料になる（D-04 §6・§8 #66） |
| 38 | `HomeScreen` が `resolveListStatus` に渡す `hasVisible` は、**「すべて」ページ（`homeArticlesProvider(companyId: null)`）の `items` が 1 件以上か**にする（値もエラーも未着なら `resolveListStatus` を呼ばず `FullView.loading`）。`true` の固定値を渡さない | `hasVisible: true` の固定（2026-09-24 以前の本書）／`hasVisible: false` の固定／選択中の団体ページの `items` から取る | D-04 §5.4.3 の判定表が 10 行になり、**3 行目（`count == empty` × `hasVisible == true` → `content`）が `count == empty` の他のすべての行より先に一致する**（D-04 §8 #65）。`true` 固定だと記事 0 件の端末で取得が失敗しても `content` になり、S-01/ST-07・ST-08（全面 E-24）と ST-10・ST-12 が一度も出ず、さらに `errorNotice` が立って ST-14 と E-25 が同時に出る（S-00 §5.2 の排他規則に反する）。`false` 固定だと逆に、初回の 100 件反映で件数 Stream が `SyncStatus` より遅れる窓（D-04 §5.4.3 の 2 番目の注記）で全面の読み込み中表示が行のある一覧を覆い、3 行目を足した意味（S-00 §8 #4）が失われる。「すべて」ページは団体タブの絞り込みが無く `in_feed = true` の全記事を表す唯一のページなので、「件数 0 と一覧に行がある状態の食い違い」をそのまま表現できる。選択中ページから取ると、スワイプ中に `homeTabControllerProvider` が確定するまで前のページの値を使い、団体ごとに全面表示が切り替わる（S-01 §8 #13「上部タブは表示したまま全ページ同じ表示」に反する） |
