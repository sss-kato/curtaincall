...
| S-01/ST-03 | 状態 | 同 `refreshing == true` → `RefreshingHeader` を一覧の**先頭の Sliver**（`AnchoredListView.leadingSlivers`。S-00 §3.3「一覧の先頭。iOS 標準の引っ張り更新の位置」）に置く。出現・消失で表示中の記事がずれないよう `AnchoredListView` が高さの変化を相殺する（§5.11・§8 #28）。引っ張り中は `CupertinoSliverRefreshControl` の表示に任せる | §5.1 / §5.6 / §5.11 |
...
| 依存元 | 依存先 | 理由 |
|---|---|---|
| `browser/domain`（`ArticleOpener`） | `settings/domain`（`BrowserChoice`） | `open(Uri, BrowserChoice)` の引数。値オブジェクトのみ |
    ...
    features/
      articles/
        ...
        presentation/
          ...
          home_page.dart                    S-01/E-11 の 1 ページ（Column：RefreshingHeader（一覧の外）+ AnchoredListView（引っ張り更新 + 空表示））
...
| S-01/E-11 | `HomePage(companyId)`（`AutomaticKeepAliveClientMixin`。6 ページを生かしてスクロール位置を保つ。S-01/A-01「前回そのページを見ていた位置」）。中身は `Column`：上段に `RefreshingHeader`（`status.refreshing && !_pulling` のときだけ。一覧の**外**）、下段（`Expanded`）に `AnchoredListView`。`leadingSlivers` は `CupertinoSliverRefreshControl` だけ（§5.11・§8 #28） |
...
- `status.refreshing` かつ引っ張り中でない（`_pulling == false`）→ `Column` 上段に `RefreshingHeader`（D-04 §5.9）。一覧の Sliver にはしない（帯の出現・消失でスクロール位置が動かない。§5.11・§8 #28）
...
**エラーの扱い**（`SavedScreen.build`。§6）：`savedListControllerProvider` が `AsyncError` のときは `resolveListStatus(feed, hasVisible: false)` に渡し（ホーム §5.1 の `AsyncError` 行と同じ）、`logger.w` を残す。`SizedBox.shrink()` は `AsyncLoading` のときだけで、エラーで空白が恒久化しない。
...
return switch (saved) {
  ...
  AsyncError() => _buildList(resolveListStatus(feed, hasVisible: false), const []),   // §6。logger.w
...
- **`build()`**：`ref.listen(savedArticlesProvider, onData, onError: ...)` で `_fromDb` を更新して `_recompute()`。**`onError` では `state = AsyncError(error, stackTrace)` にして `logger.w`**（エラーを状態に写す。写さないと `_fromDb == null` のまま `AsyncLoading` が続き画面が空白のまま止まる。§6）。`ref.read(tabControllerProvider)` に `addListener` し（`ref.onDispose` で外す）、index が 1 から他へ変わったとき `_onLeave()`。`ref.watch` は使わず、状態は `_recompute()` と `onError` だけが書く（Notifier の再生成で保留を失わないため）。初期値 `AsyncLoading`
- **`_recompute()`**：`_fromDb == null` なら `AsyncLoading` のまま。そうでなければ `state = AsyncData(mergeSavedList(fromDb: _fromDb, pendingUnsave: _pendingUnsave))`
...
final class AnchoredListView<T> extends StatefulWidget {
  const AnchoredListView({
    ...
    this.leadingSlivers = const [], // CupertinoSliverRefreshControl だけを想定（高さが自発的に変わる Sliver は置かない。§8 #28）
...
- **構成（2 形態）**：
  - **標準形**（アンカーが `items` の先頭、または `items` が空）：`CustomScrollView(controller, slivers: [...leadingSlivers, SliverList(全要素)])`。`center` は指定しない（= 先頭の Sliver）。`CupertinoSliverRefreshControl` が前方成長側の先頭にあり、Pull to Refresh が通常どおり成立する。先頭 = `position.pixels == 0`
  - **アンカー形**（アンカーが先頭以外）：`CustomScrollView(controller, center: _centerKey, slivers: [...leadingSlivers, SliverList(アンカーより前の要素を逆順), SliverList(key: _centerKey, アンカー以降の要素)])`。`RenderViewport` は `center` より前に Sliver があると前方成長側の先頭に負の `overlap`（オーバースクロール）を渡さないため、この形態では `CupertinoSliverRefreshControl` は発火しない。ユーザーは先頭より下を見ているので引っ張れず、実害は無い。先頭 = `position.minScrollExtent`（逆成長側の高さぶん**負**）。`pixels == 0` はアンカー位置であって先頭ではない
...
- **空 → 非空・非空 → 空**：標準形（アンカー無し）。`emptySliver` を `SliverFillRemaining(hasScrollBody: false)` で置く（引っ張り更新は `leadingSlivers` に残るので S-01 §8 #14 を満たす）
- **`leadingSlivers` と位置保持**（§8 #28）：位置補正の契機は `items` の変化だけなので、`leadingSlivers` の高さが自発的に変わると標準形の一覧はその高さぶん上下にずれる。したがって `leadingSlivers` に置くのは `CupertinoSliverRefreshControl` だけとする（高さが変わるのはユーザーの引っ張り中と、引っ張り更新の完了に伴う既定の収縮のみで、いずれも Cupertino 標準の見え方）。`RefreshingHeader`（S-01/ST-03）と `OfflineBanner`（S-01/ST-06）は一覧の**外**に置く（`RefreshingHeader` は `HomePage` の `Column` 上段、`OfflineBanner` は `HomeScreen` の `HomeTabBar` と `PageView` の間。§5.1）。帯の出現・消失では一覧のビューポートが縦に伸縮するだけで、スクロール位置（`pixels`・アンカー・先頭判定）は変わらない。`leadingSlivers` はどちらの形態でも `slivers` の先頭（最上部）。アンカー形では `center` より前にあるため逆成長側に属し、並び（上から）は `leadingSlivers` → アンカーより前の要素 → アンカー以降
...
- テストは書かない（Widget。D-04 §7 と同じ方針）。アンカー選択の規則・2 形態の切替・`_scrollingToTop` の不変条件は上記を実装コメントに残す。Pull to Refresh の成立と先頭スクロールは T-M の目視項目（§10）
...
| `homeArticlesProvider` が `AsyncError`（DB 破損） | `hasVisible = false` として判定し `logger.w`。ホームは `hasArticles` も偽になるので ST-08 相当（D-04 §6「DB の open に失敗」） | presentation |
| `savedArticlesProvider` が `AsyncError`（DB 破損） | `SavedListController` の `ref.listen` の `onError` が `state = AsyncError` にする（`_fromDb` の更新だけでは `AsyncLoading` のまま `SizedBox.shrink()` が恒久化する）。`SavedScreen` は `AsyncError` を `resolveListStatus(feed, hasVisible: false)` に渡し `logger.w`（§5.5）。`hasArticles` が偽なら `empty`（E-22）、真なら `content` で 0 件の一覧。復旧は `savedArticlesProvider` の invalidate（次回起動）まで待つ | presentation |
...
| 取得中の帯（`RefreshingHeader`）・オフラインの帯（`OfflineBanner`）が出た／消えた | 一覧の外（`Column` 上段・`HomeTabBar` の下）に置くため、一覧のビューポートが縦に伸縮するだけで、スクロール位置（`pixels`・アンカー・先頭判定）は動かない。`leadingSlivers` は `CupertinoSliverRefreshControl` のみ（§5.1・§5.11・§8 #28） | presentation / core/ui |
...
| 28 | `AnchoredListView.leadingSlivers` に置くのは `CupertinoSliverRefreshControl` だけ。`RefreshingHeader`（S-01/ST-03）は `HomePage` の `Column` 上段（一覧の外）、`OfflineBanner`（S-01/ST-06）は `HomeScreen` の `HomeTabBar` と `PageView` の間に置き、Sliver にしない | (a) `RefreshingHeader` を `leadingSlivers` に置き、出現・消失時に `correctPixels(±h)` で相殺する／(b) 帯を一覧に重ねる（`Stack`） | 位置補正の契機が `items` の変化だけなので、標準形で leading の高さが変わると一覧が帯の高さぶんジャンプする（アンカー形では逆成長側にあるためずれず、形態で挙動が変わる。レビュー第 2 周 M-6）。(a) は帯の高さをレイアウト前に知る必要があり、D-04 §5.9 の `RefreshingHeader` の高さを本書が固定することになる。一覧の外に置けばビューポートが伸縮するだけで `pixels` もアンカーも変わらず、形態による非対称も無い。(b) は帯が先頭のセルを隠す。`OfflineBanner` は初版から一覧の外（§5.1）で、`RefreshingHeader` も同じ扱いに揃える。開発者の指示により推奨案を採用（2026-09-20） |
...
| 要件書（司令塔） | §5「データ保持」の但し書き（保存済み記事は 100 件の削除対象にしない）に「ただし保存済み記事は削除しない（保存を解除した時点で削除対象に戻る）」を補う提案。本書 §5.5（解除の確定時に `in_feed = false` の記事を削除）と D-04 §4.2 の根拠を要件書に持たせる。本文の変更は不要 | §5.5 / §8 #5 |
...
| T-H3 | app 側の骨格（3 画面のスタブ・時計・位置保持一覧） | `articles/presentation/home_screen.dart`・`saved/presentation/saved_screen.dart`・`settings/presentation/settings_screen.dart`（ナビゲーションバーだけのスタブ）、`lib/app/minute_clock.dart`、`core/ui/list/anchored_list_view.dart`（`AnchoredListView` + `AnchoredListController`。§5.11）。改修：`lib/app/root_tabs.dart`（3 画面を載せる）。新規 5 + 改修 1 | `lib/app/`、`lib/core/ui/list/`、`lib/features/*/presentation/`（各 1 ファイル） | T-F1、T-F2a、T-F2b | `flutter analyze && flutter test`。シミュレータで 3 タブに各画面のタイトルが出る。`core/ui/list` が Flutter SDK 以外を import しない。T-H1・T-H2 と並行可 |
