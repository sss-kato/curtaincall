...
並行できる組：T-H1・T-H3（T-H2 は T-H1 の後）、T-I・T-J・T-K・T-L（`core/di` は別ファイル。`test/helpers/` の共有ヘルパ（`seed_articles.dart`・`collect_stream.dart`）は T-H1 が作り終えているため衝突しない）。T-N は T-M の後（`article_cell_model_builder.dart` に依存）。T-O は T-M・T-N と並行可（表の依存欄のとおり T-M に依存しない）。T-Q は `lib/core/ui/list/` だけを触るため T-M・T-N・T-O と並行可。

**D-04 §10 の `T-P`（D-04 の反映後の片付け）との関係**：`T-P` は本書の **T-H1〜T-O** とは別系統で、**T-H1〜T-O の完了後**に走る（D-04 §10 前文の「D-05 の全タスク完了後」は T-H1〜T-O を指す。`T-Q` は含まない。D-04 側の文言の限定は §8.1 の D-04 宛で申し送る）。`T-P` は `lib/app/root_tabs.dart`・`lib/app/app_lifecycle_sync.dart`・`core/ui/status/list_status.dart` を触り、本書の `T-M`（`app_lifecycle_sync.dart`・`home_screen.dart`）・`T-N`（`saved_screen.dart` の `ListStatus` の import）と同じファイルに当たるため、**`T-P` と `T-M`・`T-N` を並行しない**。本書の `T-Q` は `lib/core/ui/list/` だけを触り `T-P` と重ならないため、**`T-Q` と `T-P` は並行してよい**（§8.1「実装（片付け）」）。
