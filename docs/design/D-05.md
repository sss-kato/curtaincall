...
final class AnchoredListController {
  ...
  Future<void> scrollToTop({required bool animated}) {
    ...
    unawaited(_run(completer, animated: animated).whenComplete(() => _finish(completer)));   // dart:async
  ...
  Future<void> _run(Completer<void> completer, {required bool animated}) async {
    try {
      ...
      if (identical(_inFlight, completer)) _target?.scrollingToTop = false;   // 手順 4
    }
  }
...
abstract interface class _AnchoredListTarget {
  set scrollingToTop(bool value);                  // State の _scrollingToTop を書く（手順 1・4）
...
- **進行中の表し方（`_inFlight` と `_scrollingToTop` の同期）**：「`scrollToTop()` が実行中」という事実は `Controller` の `_inFlight`（合流用）と State の `_scrollingToTop`（`didUpdateWidget` がアンカー形に入らない不変条件）の 2 つで表れるが、**`_inFlight != null` ⟺ そのとき attach されている State の `_scrollingToTop == true`** を常に保つ。保つ手立ては上記コードの 3 か所だけで、ここ以外で `_scrollingToTop` を書かない：
  ...
  - `_run` のループ：`await` の間に State が張り替わったら（`identical(_target, target)` が false）、**新しい State で手順 1 からやり直す**（旧 State に張った要求を宙吊りにしない）。旧 State の `scrollToTopWhenLaidOut()` の `Future` は旧 State の `dispose` が完了させる（下記「先頭スクロール要求」手順 3）
...
- **`_AnchoredListViewState` が持つ状態**（すべて private フィールド。これ以外の状態を持たない）：
  ...
  - `int _anchorIndex`：`_anchorId` を決めた時点での `items` 内の index（`_anchorId == null` のときは `0`）。**直前の形態を保って描くとき**（下記「形態の決め方」2・5）の `center` の位置決めだけに使う
  - `bool _scrollingToTop`：`scrollToTop()` の開始から完了まで true（**State が持ち**、`Controller` が `_AnchoredListTarget.scrollingToTop` のセッター経由で書く。読むのは State の `didUpdateWidget` と手順 0 の再試行だけ）
  - `bool _lastFormWasAnchored`：**直前の `build()` が実際に採った形態**（アンカー形なら true）。`build()` の末尾で毎回代入する（`setState` を使わない素の代入。次の `build()` の入力にしかならず再 build を起こさない。同じ `items` で `build()` をやり直しても同じ値になる）。**据え置き中に「直前の形態を保つ」ための唯一の判定材料**で、`_anchorId` や `_anchorIndex` から形態を推測しない（下記「形態の決め方」2・5、「アンカーの更新」手順 3）
...
- **形態の決め方**（`build()`。**上から順に評価し、最初に該当した行で確定する**。ここに書いた 5 行以外の分岐を作らない）：
  ...
  2. **`_pendingItemsChanged`（ガード 0 で据え置き中）→ 直前の形態を保つ**（下記）。据え置き中は `correctPixels` を行えないため、**形態も `center` の基準も動かさない**。`_anchorId` が新 `items` の先頭になっていても（上の記事がまとめて消えた場合）2 で止め、`correctPixels` を伴わない標準形への転落（＝位置が飛ぶ経路）を作らない。補正つきの付け替えは次フレームの再試行に任せる
  ...
  - **「直前の形態を保つ」の実体**：`_lastFormWasAnchored == false` なら標準形。true ならアンカー形のまま `center` を **`_anchorIndex.clamp(0, items.length - 1)`** の位置に置く（1 により `items` は非空なので上限は 0 以上で、`clamp` は必ず成立する）。上限を `items.length` にすると `center` の `SliverList` が要素 0 個になり、`maxScrollExtent == 0`・`minScrollExtent == -(全高)` の下で保持していた正の `pixels` が範囲外になって `goBallistic` が**一覧の末尾**へ整定する
...
- **構成（2 形態）**：
  ...
  - **アンカー形**（上記 4、および 2・5 で `_lastFormWasAnchored == true` のとき）：`CustomScrollView(controller, center: _centerKey, slivers: [_ExtentCompensatingSliver(key: _headerKey, child: headerSliver), SliverList(アンカーより前の要素を逆順), SliverList(key: _centerKey, アンカー以降の要素)])`。**`leadingSlivers` は `slivers` に入れない。** `center` より前の Sliver は `GrowthDirection.reverse` でレイアウトされるが、`CupertinoSliverRefreshControl` の `RenderSliver.performLayout` は `assert(constraints.growthDirection == GrowthDirection.forward)` を持つため、逆成長側に置くと **debug ビルドで assert に抵触して落ちる**（「発火しないだけ」では済まない。Flutter 3.47.4 で実測。§8 #21）。この形態ではユーザーは先頭より下を見ており引っ張れないので、出さなくても実害は無い（先頭に戻れば下記「復帰」で標準形になり、そこで引っ張り更新が成立する）。先頭 = `position.minScrollExtent`（逆成長側の高さぶん**負**）。`pixels == 0` はアンカー位置であって先頭ではない
...
- **先頭スクロール要求**（`AnchoredListController.scrollToTop(animated:)`。S-01/A-03・A-09・A-10・ST-15、S-02/A-04。実装の形は上記コードの `_run`）：
  ...
  4. `_target.scrollingToTop = false`。手順 1〜3 を `try/finally` で包み、`finally` で必ず戻す（`animateTo` はユーザーのドラッグで中断されても `Future` は完了する。中断後は標準形のままなので、そのまま引っ張れば `CupertinoSliverRefreshControl` が発火する。§6）。`finally` の時点で attach 先が別の State に張り替わっている・`detach()` 済みなら書かない（`identical(_target, target)` で判定。上記コード）
...
- `AnchoredListView` 自体のテストは書かない（Widget。D-04 §7 と同じ方針）。ただし**アンカーの付け替え規則は `resolveAnchorId`（`core/ui/list/anchor_resolution.dart`）に純粋関数として切り出し、テーブル駆動の直接テストを持つ**（§7・§8 #33）。`_AnchoredListViewState` はこれを呼ぶだけにし、走査規則を State 側に書かない。2 形態の切替・`_scrollingToTop` の不変条件・`_ExtentCompensatingSliver` の補正条件（保持値の更新時点を含む）は上記を実装コメントに残す。とくに次の 4 つは、**書き換えると自動テストも品質ゲートも検出できないまま壊れる**ため、理由をコードのすぐ横に残す：(a) `_inFlight` を `Completer` で表し `_run()` を呼ぶ前に代入すること（`async` の本体は最初の `await` まで同期実行されるため）、(b) 「形態の決め方」1 が `items.isEmpty` を見ること（`num.clamp` は `upperLimit < lowerLimit` で `ArgumentError`）、(c) 据え置き中（`_pendingItemsChanged`）は `_lastFormWasAnchored` の形態を保つこと、(d) `_pendingScrollToTop` には回数上限を設けないこと。加えて、**依存している Flutter SDK 内部の前提と、それを確認した Flutter のバージョンをファイル冒頭の library doc に列挙する**（自動テストが無いため、Flutter を上げたときの再確認の手掛かりがこれしか無い。列挙する項目は §7）。Pull to Refresh の成立と先頭スクロールは T-M の目視項目（§10）
