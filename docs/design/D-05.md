...
| S-01/ST-04 | 状態 | `FullView.empty` かつ `HomeArticles.totalInTab == 0` → `EmptyView`（S-01 §5.1 ST-04 の文言） | §5.1 / §5.2 |
| S-01/ST-05 | 状態 | `FullView.empty` かつ `totalInTab > 0`（未読フィルタ ON で未読 0 件）→ `EmptyView`（S-01 §5.1 ST-05 の文言） | §5.1 / §5.2 |
...
| S-01/ST-13 | 状態 | `PageView` のドラッグ。下線は `PageController.page` の小数部で補間する（`HomeTabBar` が `_pageController` を `ListenableBuilder` で購読して再 build する）。**`PageController` に client が無いとき**（全面表示中で `PageView` がツリーに無い・初回 build）は `page` を読まず選択中タブの index を使う（§5.1「下線位置の入力」・§6） | §5.1 |
...
| S-02/A-05 | 操作 | `FullErrorView.onRetry` → **`ref.invalidate(savedArticlesProvider)` だけ**（`sync()` も `countRetryRequestedProvider.retry()` も呼ばない。本画面は取得を伴わず件数も読まない。S-02 §8 #1・D-04 §5.4.2）。`SavedListController` はやり直しの間も `AsyncError` のままで、新しい `AsyncData` が届いたときだけ一覧に戻る | §5.5 |
    ...
    features/
      articles/
        ...
        presentation/
          ...
          home_status.dart                  homeListStateOf・homePageListStateOf・resolveHomeStatus（純粋関数。
...
typedef HomeListState = ({bool settled, bool hasVisible});

/// 画面全体（HomeScreen）の判定に与える状態。
/// hasVisible は **未読フィルタで絞る前**の件数（totalInTab > 0）から取る（§8 #38）。
/// items から取ると、件数 Stream が 0 のまま固着した端末で未読フィルタ ON かつ全件既読のときに
/// 「記事を 100 件持っているのに全面 E-24」になる（§6）。
HomeListState homeListStateOf(AsyncValue<HomeArticles> v) =>
    (settled: v.hasValue || v.hasError, hasVisible: (v.valueOrNull?.totalInTab ?? 0) > 0);

/// ページ（HomePage）の判定に与える状態。
/// hasVisible は **未読フィルタで絞った後**の表示対象（items）から取る。
/// ここを totalInTab にすると、未読フィルタ ON で全件既読のページが content になり
/// S-01/ST-05（「未読の記事はありません」）が一度も出なくなる（§8 #38）。
HomeListState homePageListStateOf(AsyncValue<HomeArticles> v) =>
    (settled: v.hasValue || v.hasError, hasVisible: v.valueOrNull?.items.isNotEmpty ?? false);

/// 2 つの入力から ListStatus を作る唯一の規則。HomeScreen・HomePage・§5.6 の listener が共有する。
ListStatus resolveHomeStatus(FeedStatus feed, HomeListState s) => s.settled
...
を計算し、`screenStatus.full`（`loading` / `error` / `offline` / `unavailable` なら全面表示。`content` / `empty` なら `PageView`）と `screenStatus.offlineBanner`（S-00/E-23）を使う。`empty`（S-01/ST-04・ST-05）と `refreshing`（ST-03）は各 `HomePage` が自ページの `homeArticlesProvider` で判定する（下記「ページの状態判定」）。

**`homeListStateOf` と `homePageListStateOf` の違いは `hasVisible` の由来（絞り込み前／後）だけ**で、`settled` の決め方も判定規則（`resolveHomeStatus`）も共通にする。判定表のどの行に落ちるかを変えるのは入力だけで、規則そのものは 1 つしか無い（§8 #38・#39）。
...
| S-01/E-10 | `PageView.builder(controller: _pageController, physics: BouncingScrollPhysics)`。`_pageController` は `HomeScreen` の `initState` で **1 度だけ** `PageController(initialPage: ref.read(homeTabControllerProvider))` として生成し、`dispose` で破棄する（`build` 内で生成すると再 build のたびにページが初期値へ戻る）。`initialPage` を Provider から取るのは、未起動から通知タップで起動したとき（S-01/ST-15）に `HomeScreen` の初回 build より前に `selectCompany()` が済んでいる場合があるため（通常の起動では 0。§5.7）。ページ数 = 1 + companies。`onPageChanged` → `homeTabControllerProvider.notifier.select(i)`（A-02。ただし `_pendingTabIndex != null` のときは無視する。下記「通知由来の変化を取りこぼさない」）。`homeTabControllerProvider` の変化を `ref.listen` し、**`_pageController.page?.round() != next`**（= タブのタップ・スワイプ以外の変化 = 通知由来の `selectCompany`）なら、`_animating == false` のとき `jumpToPage(next)`、`_animating == true` のとき `_pendingTabIndex = next`（アニメーションの完了時に反映する。下記）（§5.7・§8 #13）。`_animating`・`_pendingTabIndex` は `HomeScreen` の `State` のフィールド（`bool _animating = false;`・`int? _pendingTabIndex;`）で、`build` の出力に影響しない（listener の条件にしか使わない）ため **`setState` を使わない素の代入**で書く |
...
**タブのタップ（A-01）**：`_animating = true` → `select(index)` → `index` が現在の隣なら `animateToPage`。隣でなければ Material `TabBarView` と同じワープ（アニメーションの間だけ目的ページを現在ページの隣に並べ替えて `animateToPage` し、完了後に並びを戻して `jumpToPage`）。**`_animating = true` の代入から完了処理までを `try` / `finally` で囲み、`finally` で (1) ワープの並べ替えの復元、(2) `_pendingTabIndex` の反映（下記）、(3) `_animating = false` の順に行う**（`animateToPage` の `Future` はドラッグで中断されたときも完了するが、例外で終わる経路や State が作り直される経路でフラグと並べ替えを下ろし損ねると、以後すべての通知タップが `jumpToPage` しなくなり、ワープ用の並びが残ったまま固定される）。`dispose` でも同じ `finally` 相当の後始末を行う（`mounted` を見て `jumpToPage` は呼ばない）。`index == 現在` なら A-10（§5.6）。スワイプ（A-02）は `onPageChanged` から `select` を呼ぶだけで、listener は上の条件（`_pageController.page?.round() != next`）が成立しないため何もしない。

**通知由来の変化を取りこぼさない**（S-01/ST-15。§6）：`selectCompany()`（§5.7 手順 3-4）は A-01 のアニメーション中にも届きうる。その場合 listener は `_pendingTabIndex = next` に保持するだけで `jumpToPage` せず、上の `finally` の (2) で `_pendingTabIndex` が非 null なら `jumpToPage(_pendingTabIndex!)`（非 null を直前に確認しているための `!`）→ `select(_pendingTabIndex!)` → `_pendingTabIndex = null` とする。**`_pendingTabIndex != null` の間は `onPageChanged` からの `select` を呼ばない**（アニメーションの完了で発火する `onPageChanged` が、利用者のタップ先で通知の選択を上書きしてしまうため。S-01/ST-15 は「通知タップ後は対象の団体タブが最初のフレームから一致する」）。

**下線位置の入力**（S-01/ST-13。§6）：`HomeTabBar` の下線は、**`_pageController.hasClients && _pageController.positions.length == 1` のときだけ `_pageController.page`（小数部で補間）を使い、そうでなければ `ref.watch(homeTabControllerProvider).toDouble()`（選択中タブの index）を使う**。`PageController.page` は `PageView` がツリーに無いと `positions` が空で、`assert(positions.isNotEmpty)`（debug）／`StateError`（release）になる。**全面表示中（`screenStatus.full` が `loading` / `error` / `offline` / `unavailable`）は `PageView` をツリーに置かない**ため必ずこの経路に入り、起動直後の S-01/ST-02（`count == pending` → `FullView.loading`）でも通る。`PageView` を出す経路でも**最初のフレームは `HomeTabBar` の build がレイアウトより前**なので同じ状態になる。`page` の変化（ドラッグ中の補間）で `HomeTabBar` を再 build する契機は、`HomeTabBar` が `_pageController` を `ListenableBuilder`（`listenable: _pageController`）で購読して作る（`PageController` は `ScrollController` なので `Listenable`）。`_pageController` を渡すのは `HomeScreen` で、`HomeTabBar` は client の有無を自分で確かめてから `page` を読む（例外にしない）。
...
// 判定規則は HomeScreen と同じ resolveHomeStatus。違いは hasVisible の由来だけ（ページは未読フィルタ適用後の items）。
// 前の値を持つ AsyncLoading（未読フィルタ切替で Stream を作り直した直後）は settled が true のまま前の値で判定され、
// 全面ローディングを挟まない（S-01/A-03）。値もエラーも未着のときだけ FullView.loading（§8 #11）。
// 値を持ったままエラーになった場合は hasValue が優先され、直前の一覧で判定する（§6）。
final status = resolveHomeStatus(feed, homePageListStateOf(home));
```

**`requireValue` を読まない。** `AsyncError` で前の値が無いとき（`articles` の 1 行の `published_at` / `category` が壊れて `ArticleRowMapper` が写像で例外を投げた等）に `requireValue` は `StateError` を投げ、`HomePage.build` が例外で落ちる。このとき `articleCountProvider`（`SELECT COUNT(*)`）は成功しうるので `feed.count == ArticleCountState.present` のまま `hasVisible == false` となり、D-04 §5.4.3 判定表の最終行（10 行目）で `FullView.empty` になる（件数が読めている以上 `unavailable` にはならない。S-00 §8 #36 の ST-17 は「件数そのものが分からない」ときの状態）。文言は `totalInTab == 0` の側（S-01 §5.1 ST-04）を出す（§6）。分岐は `AsyncData` / `AsyncError` の状態クラスではなく `hasValue` / `hasError` のフラグで書く（D-04 §5.4.2「Riverpod 3 の自動リトライは切る」により、`articleCountProvider` 以外の Provider ではリトライ中が `AsyncLoading(hasError: true)` として現れるため）。
...
ref.listen(homeArticlesProvider(companyId: null), (prev, next) {
  ...
  if (!next.hasError || !next.hasValue) return;   // 値なしのエラーは全面表示 + A-08 の動線があるので触らない
  if (_articlesReloaded) return;                  // 同じ障害では 1 回だけ（無限 invalidate を避ける）
...
- `_articlesReloaded` は `HomeScreen` の `State` の `bool` フィールド（初期 false。`build` の出力に影響しないため `setState` を使わない素の代入）。**`_retry()`（§5.6）でも false に戻す**（利用者が明示的に押したときは常にやり直せる）
- **値ありエラーだけを契機にする**のは、値なしエラーでは判定表が `unavailable` / `empty` / `error` / `offline` に落ちて A-08 の動線が出る（あるいは S-01 §5.1 ST-04 の「件数は読めたが一覧を組み立てられなかった」表示になる）ためで、そこに自動の張り直しを重ねない（§6・§8.1 の S-01 宛 (a)）
- **1 回で止める**のは、`ref.invalidate` の後に張り直した Stream も直前値を引き継いだうえで同じ行の写像に失敗すると、再び値ありエラーになって listener が再入し、invalidate のループになるため（`_articlesReloaded` が立っている間は何もしない）。張り直しても直らない障害は、次の健全な値（別の同期で問題の行が消えたとき）まで古い一覧を表示したままになる
...
- 張り直しの事実は上の `logger.w` に 1 回残す（`HomePage` の `ref.listen` が残すエラー本体のログとは別。恒久的な障害では 1 回しか出ないため、ログの有無で「1 回試して直らなかった」ことが分かる）

- `status.full == FullView.empty` のとき `EmptyView` に渡すアイコン・文言：`totalInTab == 0` なら S-01 §5.1 ST-04、そうでなければ ST-05（未読フィルタ ON で未読 0 件）。`EmptyView` は `AnchoredListView.emptyView` に渡し（`empty` 以外は `null`）、`AnchoredListView` が `SliverFillRemaining` に置くので引っ張り更新できる（S-01 §8 #14。§5.11.1）
...
- **`HomePage` が自分で出す全面表示は `FullLoadingView`（自ページの一覧が未確定 = `settled == false`）だけ**で、`error` / `offline` / `unavailable` の全面表示は出さない（再試行 A-08 の動線は `HomeScreen` が持つ。§5.6）。`HomeScreen` と `HomePage` は同じ `feed` を使うが `hasVisible` の由来が異なる（画面は「すべて」ページの `totalInTab`、ページは自ページの `items`）ため、**判定表 3 行目の過渡状態では `HomeScreen` が `content`・ある団体ページが `error` / `offline` / `loading` という食い違いが 1 フレーム起こりうる**。このとき `HomePage` は `EmptyView` を渡さずに一覧（0 件なので空の領域）をそのまま描く。次のフレームで件数 Stream が追いつけば通常の表示に戻るため、専用の分岐は持たない（§8 #38）
- **`FullView.empty` が成立する経路は 2 つある**（§8 #38）。(1) 判定表 8 行目（`count == empty` × `hasVisible == false`。取得成功で端末に記事が 0 件）：`HomeScreen` は `PageView` を出し、`HomePage` も `items` が空・`totalInTab == 0` で `empty` になり **ST-04**（「記事がありません」）を出す。(2) 判定表 10 行目（`count == present` × `hasVisible == false`）：`HomeScreen` 側では「すべて」ページの `totalInTab == 0`（件数は読めているが `in_feed = true` の一覧が組み立てられなかった。§6）、`HomePage` 側では未読フィルタ ON で自ページの未読が 0 件になった場合が典型で、後者は `totalInTab > 0` なので **ST-05**（「未読の記事はありません」）を出す。**`empty` なら常に ST-04、ではない。** どちらの経路でも `HomeScreen` は `PageView` を出すため画面全体の見え方は変わらず、文言の出し分けは `HomePage` の `totalInTab` だけが決める。全面表示用の `empty` Widget は持たない
...
**S-02/A-05（再試行）**：`FullErrorView.onRetry` は **`ref.invalidate(savedArticlesProvider)` だけ**を行う。`sync()` も `articleCountProvider` の invalidate も呼ばない（本画面は取得を伴わず、記事件数も読まない。S-02 §8 #1・§6 A-05）。ホーム（S-01/A-08・S-00/A-12）の 2 系統（D-04 §5.4.4「全面表示からの復帰（再試行の 2 系統）」）に対し、**取得を持たない画面の第 3 の経路**として本書が持つ。
...
| `homeArticlesProvider` が `AsyncError`（DB 破損・`ArticleRowMapper` の写像失敗） | `hasVisible = false`・**`totalInTab == 0` 扱い**で判定する（`requireValue` を読まない。読むと `StateError` で `HomePage.build` が落ちる。§5.1）。`ref.listen` でエラー到着ごとに 1 回 `logger.w`。`articleCountProvider` も失敗していれば `count == unavailable` で **ST-19（S-00/ST-17。全面 E-24 + 再試行）**（D-04 §5.4.3 の判定表の先頭行・§6「DB の open に失敗」）、**件数クエリだけ成功していれば `count == present`・`hasVisible == false` で `empty`**（S-01 §5.1 ST-04 の文言）になる。**再試行（S-01/A-08 = S-00/A-12）は系統を問わず `_retry()` が `countRetryRequestedProvider.retry()`（＝ `articleCountProvider` の invalidate）と `homeArticlesProvider`（family 全ページ分）の invalidate を行って Stream を張り直す**ので、DB が正常に戻れば再起動せずに一覧へ戻れる（§5.6。D-04 §5.4.2・§5.4.4・§8.1）。件数だけ成功している経路（ST-04 と同じ表示）は **S-01 §5.1 ST-04 の発生条件**（「件数は読み出せたが一覧の中身を組み立てられなかったとき」）に反映済みで、本書の扱いと一致する | presentation |
| `homeArticlesProvider` の Stream が**値を流した後に**エラー終了した（100 件表示できている端末で、同期後に 1 行だけ `published_at` の写像に失敗する記事が入った等） | Riverpod が直前値を引き継ぐので `hasValue == true` のままで `resolveHomeStatus` は `content` を返し、**全面表示にならない＝ A-08 の再試行ボタンが出ない**。Stream は終了済みなので Pull to Refresh が成功しても一覧は古いままになる。これを塞ぐため `HomeScreen` の `ref.listen` が **`hasError && hasValue` を検知したら 1 度だけ `ref.invalidate(homeArticlesProvider)` で張り直す**（`_articlesReloaded` で同じ障害の間は 1 回に限り、健全な値が届いたときと `_retry()` で解く。§5.1「一覧 Stream が値を得た後にエラー終了したときの張り直し」）。張り直した先で**値なし**のエラーになれば通常どおり判定表に落ち、A-08 の動線（`unavailable` / `error` / `offline`）か S-01 §5.1 ST-04 の表示になる。上の「1 行だけ写像に失敗する」例のように張り直しても直らない障害では、次の健全な値まで古い一覧を表示したままになる（`logger.w` に記録が残る） | presentation |
...
| 記事件数 Stream が 0 を 1 度流した後にエラー終了し（`count` が `empty` に固着）、その後の取得で記事が 100 件入った端末で、未読フィルタ ON にして全件を既読にした | `HomeScreen` の `hasVisible` は**未読フィルタ適用前の `totalInTab`（100 > 0）**なので true になり、判定表 3 行目で `content` → `PageView` が出る（各 `HomePage` は `items` が 0 件・`totalInTab > 0` で S-01/ST-05）。`hasVisible` を `items` から取ると false になり、次の取得が失敗した時点で判定表 7 行目に落ちて**記事を 100 件持っている端末に全面 E-24（ST-08）が出て一覧が消える**（§5.1・§8 #38） | presentation |
...
| タブのタップのアニメーション中（`animateToPage` の 300ms）に通知タップが届いた | listener は `_pendingTabIndex` に保持し、アニメーション完了の `finally` で `jumpToPage` → `select` する。保持している間は `onPageChanged` からの `select` を呼ばない（利用者のタップ先が通知の選択を上書きしない。S-01/ST-15。§5.1「通知由来の変化を取りこぼさない」） | presentation |
| タブのアニメーションがドラッグで中断された・完了前に State が作り直された | `_animating = false`・ワープの並べ替えの復元・`_pendingTabIndex` の反映はすべて `try` / `finally`（`dispose` でも同等の後始末）で行うため下ろし損ねない。下ろし損ねると以後すべての通知タップが `jumpToPage` せず、ワープ用の並びが残ったまま固定される（§5.1「タブのタップ（A-01）」） | presentation |
...
| `HomeScreen` の `hasVisible` を `homeListStateOf`（`totalInTab`）から与える配線（§8 #38） | 規則そのものは上記 `resolveHomeStatus` / `homeListStateOf` の直接テストが覆う（`items` から取る実装に戻すと `homeListStateOf` のケースが落ちる）。`HomeScreen` がその関数を `select` に渡していることは §10 T-M の目視項目 22 |
| `PageController` に client が無いときの下線（§5.1「下線位置の入力」） | 値の写像ではなく Widget の配線のため直接テストできない。§10 T-M の目視項目 23（起動直後・全面表示中に落ちないこと） |
| 一覧 Stream が値を得た後にエラー終了したときの 1 回だけの張り直し（§5.1） | Provider の配線のためテストしない。§10 T-M の目視項目 24 |
...
| 39 | ホームの状態判定の規則は `lib/features/articles/presentation/home_status.dart` の 3 つの純粋関数（`homeListStateOf`・`homePageListStateOf`・`resolveHomeStatus`）に 1 箇所だけ置き、`HomeScreen.build`・`HomePage.build`・§5.6 の listener がこれを呼ぶ。**`resolveListStatus` の直接呼び出しと `AsyncValue` の `switch` を画面側に書かない**。テーブル駆動の直接テストを §7 に持つ | `HomeScreen` の private 関数（`_screenStatus` / `_allTabOf`）と `HomePage.build` の `switch` に同じ規則を 2 つの書き方で持つ（2026-09-24 第 1 周の改訂）／`HomePage` も `HomeScreen` の関数を private のまま使い回す（同一ファイルに寄せる） | 2 つは「入力（「すべて」ページ／自ページ）が違うだけで同一の規則」で、#38 で `hasVisible` を実値に変えたことにより完全に一致した重複になった。**片方だけを直す事故（まさに #38 が直した種類の事故）が型検査でもテストでも落ちない。** §5.6 の listener は「同じ関数に同じ入力を与える」形で既に重複を避けており、`HomePage` だけがその原則の外にあった。切り出せば #38 の規則に自動テストが付き（§7。`hasVisible` に固定値を渡す実装へ戻す回帰が検出できる）、`HomePage` を `home_screen.dart` の private に依存させずに済む。置き場は実装ファイルと同じ相対パスの `test/features/articles/presentation/home_status_test.dart` で、**CLAUDE.md の既存の許容パス（`test/features/<feature>/{domain,presentation}/`）に収まるため規約の改訂は不要**（純粋関数の例示への追記だけ §8.1 の CLAUDE.md 宛に足す） |
| 40 | 一覧 Stream（`homeArticlesProvider`）が**値を流した後に**エラー終了したときは、`HomeScreen` の `ref.listen`（「すべて」ページ）が `hasError && hasValue` を検知して **1 度だけ** `ref.invalidate(homeArticlesProvider)` で張り直す。回数は `_articlesReloaded` で 1 回に限り、健全な値が届いたときと `_retry()` で解く | (a) 何もしない（2026-09-24 第 1 周の改訂まで）／(b) `SyncSucceeded` を受けた時点で一覧 Provider が `hasError` なら invalidate する／(c) 値ありエラーでも全面表示（`FullView.error`）に倒して A-08 の動線を出す | (a) は `hasValue == true` のままなので `content` になり、**全面表示にならないので A-08 の再試行ボタンが出ず、`homeArticlesProvider` を invalidate する経路が画面に 1 つも無くなる**。Stream は終了済みなので以後 Pull to Refresh が成功しても一覧は古いままで、再起動以外に回復手段が無い（D-04 §8 #63 が `retry()` で塞いだ「件数 Stream が値を得た後に死んだ端末では取得に成功しても抜けられない」と同型の穴）。(b) は取得が走らない端末（オフラインが続く・S-02 のように取得を持たない経路）で契機が来ず、`SyncSucceeded` を待つぶん復帰が遅い。(c) は S-01 §5.1 ST-07・ST-08 の発生条件（直近の取得結果で決まる）に反し、取得は成功しているのにエラー表示が出る。回数を 1 回に限るのは、張り直した Stream も直前値を引き継いで同じ行で失敗すると listener が再入して invalidate のループになるため |
...
| CLAUDE.md（司令塔）（**一部反映済み**。未反映分は本文の (d) の 1 件） | **反映済み（2026-09-24 確認）**：(a) 純粋関数の例示の `mergeSavedList`・`buildArticleCellModel`・**`resolveAnchorId`**（§8 #33）、(b) ディレクトリ規約の `features/browser/`（§8 #8。開発者確認済み 2026-09-20）・`core/ui/list/` と、`core/database/` の説明への「2 つ以上の feature の infrastructure が共有する述語・写像（`article_row_mapper.dart`）」（§3.1・§8 #32）、(c) 純粋関数テストの置き場の列挙（「テスト方針」の許容パスと `app/` の `test/` ツリーの両方）への `test/core/ui/list/`（§3.2・§7・§8 #33）、`test/helpers/` の契約テストの例外（§8 #36）。**未反映は (d) の 1 件**：「テスト方針」の純粋関数の例示に **`resolveHomeStatus`・`homeListStateOf`・`homePageListStateOf`**（§8 #39）を足す。**置き場の列挙（`test/features/<feature>/{domain,presentation}/`）には既に収まっているため、規約の改訂は例示への追記だけで足りる**（例示が無くても `T-M` の実装は規約に反しない） | §7 / §3.1 / §3.2 |
