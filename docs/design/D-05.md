...
final class AnchoredListController {
  ...
  /// 手順 1〜4。async メソッドの本体は最初の await まで同期実行されるため、
  /// 「async メソッドを呼んで戻り値を _inFlight に代入する」形にすると、await が 1 つも無い経路
  /// （animated: false かつレイアウト確定済み。S-01/A-03 の requestAll()）で finally が代入より先に走り、
  /// _inFlight が完了済みの Future のまま残留して以後の scrollToTop() がすべて無視される。
  /// Completer を呼び出し側で先に置くこの形なら、同期完了の経路でも残留しない
...
- **形態の決め方**（`build()`。**上から順に評価し、最初に該当した行で確定する**。ここに書いた 5 行以外の分岐を作らない）：
  ...
  - **「直前の形態を保つ」の実体**：`_lastFormWasAnchored == false` なら標準形。true ならアンカー形のまま、`final i = items.indexWhere((e) => idOf(e) == _anchorId);` を求めて `center` の位置を次のとおり決める。
    ...
    - **`i == -1`（アンカーが新しい `items` に無い。5 の経路と、据え置き中にアンカー自体が消えた場合）→ `_anchorIndex.clamp(0, items.length - 1)`。** 基準にする記事が残っていないため、位置の手掛かりは旧 index しかない（次フレームの再試行で実測に基づいて選び直す）。1 により `items` は非空なので上限は 0 以上で、`clamp` は必ず成立する。上限を `items.length` にすると `center` の `SliverList` が要素 0 個になり、`maxScrollExtent == 0`・`minScrollExtent == -(全高)` の下で保持していた正の `pixels` が範囲外になって `goBallistic` が**一覧の末尾**へ整定する
...
| `animated: false` の先頭スクロール（A-03 の `requestAll()`）の直後に `animated: true` の要求が来た（下部タブ再タップ） | **新しい要求として手順 1 から走る。** `_inFlight` は `Completer` で表し `_run()` を呼ぶ前に代入して出口で「null に戻してから `complete()`」するため、`await` が 1 つも無い同期完了の経路でも残留しない（`async` メソッドの戻り値を後から代入する形にすると `finally` が代入より先に走って残留し、以後そのページが二度と先頭に戻らない。§5.11「先頭スクロール要求」） | core/ui |
