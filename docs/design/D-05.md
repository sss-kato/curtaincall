...
| S-01/ST-03 | 状態 | 同 `refreshing == true` → `RefreshingHeader`（一覧の**外**、`HomePage` の `Column` 上部に置く。§8 #28。引っ張り中は `CupertinoSliverRefreshControl` の表示に任せる） | §5.1 / §5.6 |
...
| S-02/ST-01 | 状態 | `resolveListStatus(feedStatusProvider(syncsOnLaunch: false), hasVisible: entries.isNotEmpty)` が `content`。`savedArticlesProvider` の初回値が届くまでは何も出さない（`SizedBox.shrink()`。読み込み表示も空表示も出さない）。`AsyncError` は `hasVisible: false` で判定する（§5.5・§6） | §5.5 |
| S-02/ST-02 | 状態 | 同 `empty`（`savedArticlesProvider` の値が届いた後で 0 件）→ `EmptyView`（S-02 §5 ST-02 の文言）。初回値が届く前は出さない。`CupertinoSliverRefreshControl` を置かない | §5.5 |
