...
final screenStatus = resolveListStatus(feed, hasVisible: true);          // hasVisible を true に固定 → full は loading / error / offline のいずれか（empty は出ない）
```

を計算し、`screenStatus.full`（`content` 以外なら全面表示）と `screenStatus.offlineBanner`（S-00/E-23）を使う。`empty`（S-01/ST-04・ST-05）と `refreshing`（ST-03）は各 `HomePage` が自ページの `homeArticlesProvider` で判定する（下記「ページの状態判定」）。`resolveListStatus` の `full` は `hasVisible` を `empty` の判定にしか使わない（D-04 §5.4）ため、`true` に固定しても `loading` / `error` / `offline` の判定はページごとの判定と一致する。
...
| 全面表示 | `screenStatus.full != FullView.content` のとき（`loading` / `error` / `offline`。`empty` は出ない）は `PageView` の代わりに `FullLoadingView` / `FullErrorView(kind, onRetry)` をページ領域全体に出す（S-01 §8 #13。上部タブは表示したまま） |
...
`TransientNotice.show(context, message:)` の呼び出し API は D-04 T-F2a で実装された `TransientNotice`（D-04 §5.9）の公開メソッドに従う。static な `show` が無ければ、`HomeScreen` が `Overlay.of(context).insert(OverlayEntry(builder: (_) => TransientNotice(message: ...)))` で挿入し、3 秒後（D-04 §5.9）に `entry.remove()` する（`Timer` は `HomeScreen.dispose` で破棄する）。
...
final class AnchoredListController {
  ...
  /// 引数の型は同ファイル内の private インターフェース _AnchoredListTarget（「標準形へ復帰する」操作だけを持つ。
  /// _AnchoredListViewState<T> が implements する。型引数 T に依存しない）。1 つの Controller に同時に 1 つの State だけ
